...
static void operation_temp(void){
  ...
  // Store in memory
  // If the buffer is full && has not reached 60C yet, then overwrite from array 0
  // If it has reached 60C, then save from array 0, and no overwrite.
  if (temp_data_count<DATA_BUFFER_SIZE){
    temp_data_stored[temp_data_count] = temp_data;
    temp_data_count = temp_data_count + 1;
  }
  else if (temp_data_count==DATA_BUFFER_SIZE && high_temp_triggered==0){
    temp_data_count = 0;
    temp_data_stored[temp_data_count] = temp_data;
  }
