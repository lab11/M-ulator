 ...
 *    Changed default parameters for faster configuration
 *    Using LNT WUPT to wake up system for the LNT delays
 *    Reset LNT AFE and DBE when GOC trigger is used to interrupt LNT timer
 *
 *  v6.4.1:
 *    Extend STATE_COLLECT and STATE_WAIT beacons with an 11-bit SYS_ID
 *
 ******************************************************************************************/

#define VERSION_NUM 0x640

#include "huffman_encodings.h" 
#include "../include/PREv21E.h"
#include "../include/PREv21E_RF.h"
#include "../include/SNTv5_RF.h"
#include "../include/PMUv12_RF.h"
...
// LNT delay states
#define LNT_DELAY_STOP  0x0
#define LNT_DELAY_START 0x1

volatile uint16_t XO_TO_LNT_WUPT_SHIFT = 15;
volatile uint16_t xo_to_lnt_wupt_mplier = 1988; // We approximate the ratio between clk_lrc and xo
volatile uint16_t min_lnt_wupt_time = 100; // Minimum sleep time for LNT WUP, we want to make sure we don't miss the wakeup request
volatile uint16_t lnt_delay_state = LNT_DELAY_STOP; // initialize to run main code body
volatile uint32_t lnt_stop_wait_time = (1 << 15); // default wait timer after lnt_stop is 1s
volatile int32_t lnt_delay_sleep_margin = (1 << 12); // about a 1/8th of a second
...
volatile uint32_t PMU_ACTIVE_SETTINGS[7] = {0x01000100, 
                                            0x01000100, 
                                            0x01000100, 
                                            0x01000100, 
                                            0x01000100, 
...
volatile uint32_t PMU_RADIO_SETTINGS[7] = {0x01000100, 
                                           0x01000100, 
                                           0x01000100, 
                                           0x01000100, 
                                           0x01000100, 
...
volatile uint32_t PMU_SLEEP_SETTINGS[7] = {0x01000100, 
                                           0x01000100, 
                                           0x01000100, 
                                           0x01000100, 
                                           0x01000100, 
...
void set_lnt_wup_timer(uint8_t en, uint32_t sleep_time) {
    lntv1a_r40_t lntv1a_r40 = LNTv1A_R40_DEFAULT;
    lntv1a_r41_t lntv1a_r41 = LNTv1A_R41_DEFAULT;
    lntv1a_r42_t lntv1a_r42 = LNTv1A_R42_DEFAULT;
    if(en) {
        lntv1a_r42.WUP_THRESHOLD = sleep_time & 0xFFFFFF;
        mbus_remote_register_write(LNT_ADDR, 0x42, lntv1a_r42.as_int);
        lntv1a_r41.WUP_THRESHOLD_EXT = sleep_time >> 24;
        mbus_remote_register_write(LNT_ADDR, 0x41, lntv1a_r41.as_int);
        lntv1a_r40.WUP_ENABLE = 0x01;
        lntv1a_r40.WUP_LC_IRQ_EN = 0x01;
        lntv1a_r40.WUP_AUTO_RESET = 0x01;
        lntv1a_r40.WUP_ENABLE_CLK_SLP_OUT = 0x00;
        mbus_remote_register_write(LNT_ADDR, 0x40, lntv1a_r40.as_int);
    }
    else {
        lntv1a_r40.WUP_ENABLE = 0x00;
        mbus_remote_register_write(LNT_ADDR, 0x40, lntv1a_r40.as_int);
    }
}
...
static void pmu_set_active_clk(uint32_t setting) {
    ...
    uint16_t cclk_div = setting & 0xFF;
...
static void pmu_init() {
    ...
    pmu_reg_write(0x47, // use slow loop and active-mode PRC clk tracking
        ...
        | (2 << 8) // Clock Divider Tuning for SAR Charge Pump Pull-Up (3'h0: divide by 1 / 3'h1: divide by 2 / 3'h2: divide by 4 / 3'h3: divide by 8 / 3'h4: divide by 16)
...
//        ( (0 << 16) // [17:16]: If [17]=1, override SAR_EN_FAST_LOOP with [16]
//        | (0 << 14) // [15:14]: If [15]=1, override SAR_MAIN_LOOP_EN with [14]
//        | (0 << 12) // [13:12]: If [13]=1, override UPC_MAIN_LOOP_EN with [12]
//        | (0 << 10) // [11:10]: If [11]=1, override DNC_MAIN_LOOP_EN with [10]
//        | (0 << 8) // [9:8]: If [9]=1, override SAR_VCON_BOOST with [8]
//        | (0 << 6) // [7:6]: If [7]=1, override MAIN_CLK_RESETB with [6]
//        | (2 << 4) // [5:4]: If [5]=1, override UPC_MAIN_CLK_RESETB with [4]
//        | (0 << 2) // [3:2]: If [3]=1, override DNC_MAIN_CLK_RESETB with [2]
//        | (0     ) // [1:0]: If [1]=1, override EN_MAIN_CLK_OUT with [0]
//    ));
...
int main() {
    ...
    *NVIC_ISER = (1 << IRQ_WAKEUP | 1 << IRQ_GOCEP | 1 << IRQ_TIMER32 | 
    ...
    // v6.4.0 Moving this check outside of code body
    ...
    if(*SREG_WAKEUP_SOURCE & 1) {
        ...
        // }
        set_goc_cmd();
        lnt_delay_state = LNT_DELAY_STOP;   // if a GOC command is sent, make sure the code body is run
        reset_lnt_flag = 1;

    }

    // v6.4.0: This needs to happen on every wakeup
    // must read adc from sleep mode prior to adjusting PMU SAR ratio in active mode
    pmu_adc_read_latest();
    pmu_setting_temp_based(0);

    // turn off lnt wup timer
    set_lnt_wup_timer(0, 0);

    // v6.4.0 Optimized LNT delays
    if(lnt_delay_state == LNT_DELAY_STOP) {
        lnt_delay_state = LNT_DELAY_START;   // Make sure the code body is not run again within the same "wakeup", lnt_delay_state will be set back to STOP after lnt_start()
        ...
        // v6.4.0 Set up 1s timer
        uint32_t lnt_wait_target = xo_sys_time + lnt_stop_wait_time;
        ...
        if(goc_data_header == 0x00) {
            ...
            if(option2) {
                ...
                SYS_ID  = (goc_data_full >> 5) & 0x7FF;
            }

            // send alive beacon
            radio_data_arr[2] = (0x1 << 8) | ((SYS_ID & 0x7) << 5) | CHIP_ID;
            radio_data_arr[1] = (VERSION_NUM << 16) | read_data_batadc;
            radio_data_arr[0] = ((SYS_ID >> 3) << 16) | snt_sys_temp_code & 0xFFFF;
            send_beacon();
            ...
            if(goc_state == STATE_INIT) {
                ...
                if(day_count + epoch_days_offset >= end_day_count && xo_day_time_in_sec >= data_collection_end_day_time) {
                    ...
                    if((op_counter == 0) || (radio_debug && projected_end_time_in_sec >= next_radio_debug_time)) {
                        ...
                        if(mrr_send_enable) {
                            ...
                            radio_data_arr[2] = (0xDE << 8) | ((SYS_ID & 0x7) << 5) | CHIP_ID;
                            radio_data_arr[1] = xo_sys_time_in_sec;
                            radio_data_arr[0] = (read_data_batadc << 24) | ((SYS_ID >> 3) << 16) | (snt_sys_temp_code & 0xFFFF);
                            send_beacon();
                        }
                    }
                }
                ...
                if(projected_end_time_in_sec == next_beacon_time && mrr_send_enable) {
                    // send beacon
                    radio_data_arr[2] = (0xEF << 8) | ((SYS_ID & 0x7) << 5) | CHIP_ID;
                ...
                if(mrr_send_enable) {
                    while(1) {
                        ...
                        if(radio_unit_counter == 0) {
                            // v6.4.1: radio SYS_ID + CHIP_ID for identification
                            radio_data_arr[2] = (0xDA << 8) | ((SYS_ID & 0x7) << 5) | CHIP_ID;
                            radio_data_arr[1] = start_day_count;
                            radio_data_arr[0] = ((SYS_ID >> 3) << 16) | max_unit_count;
                        }
            ...
            *REG_MBUS_WD = 0; // Disables Mbus watchdog timer
            ...
            // LNT optimization parameters
            uint16_t N = goc_data_full & 0xFFFFF;
            uint16_t option = (goc_data_full >> 20) & 0xF;
            uint32_t data_out = 0;
            if(option == 0) {
                XO_TO_LNT_WUPT_SHIFT = N;
                data_out = XO_TO_LNT_WUPT_SHIFT;
            }
            else if(option == 1) {
                xo_to_lnt_wupt_mplier = N;
                data_out = xo_to_lnt_wupt_mplier;
            }
            else if(option == 2) {
                lnt_stop_wait_time = N;
                data_out = lnt_stop_wait_time;
            }
            else if(option == 3) {
                min_lnt_wupt_time = N;
                data_out = min_lnt_wupt_time;
            }
            else if(option == 4) {
                lnt_delay_sleep_margin = N;
                data_out = lnt_delay_sleep_margin;
            }

            radio_data_arr[2] = (0x20 << 8) | CHIP_ID;
            radio_data_arr[1] = 0;
            radio_data_arr[0] = data_out;
            send_beacon();
        }

        // if there is more than lnt_delay_sleep_margin time left, go to sleep using sleep clk
        // other just go to lnt_start
        update_system_time();
        int32_t time_remaining = lnt_wait_target - xo_sys_time;
        // mbus_write_message32(0xE1, time_remaining);
        if(time_remaining > 0 && time_remaining > lnt_delay_sleep_margin) {
            uint64_t temp = mult(time_remaining, xo_to_lnt_wupt_mplier);
            uint32_t sleep_time = temp >> XO_TO_LNT_WUPT_SHIFT;
            // mbus_write_message32(0xE5, sleep_time);

            if(sleep_time > min_lnt_wupt_time) {
                set_lnt_wup_timer(1, sleep_time);
                pmu_setting_temp_based(2);
                operation_sleep();
                while(1);
            }
    ...
    // if not running lnt_stop or finished lnt_stop, set system to run lnt_stop on next wakeup
    lnt_delay_state = LNT_DELAY_STOP;
    update_system_time();
