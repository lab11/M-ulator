...
static void operation_init (void) {
    ...
    if (!get_flag(FLAG_ENUMERATED)) {
        ...
        *REG_WUPT_CONFIG = *REG_WUPT_CONFIG & 0xFF3FFFFF; // WUP_ENABLE=0, WUP_WREQ_EN=0
        ...
        // Set the duration
        eid_set_duration(60);  // 500 is the default duration for *_mid (=-5 ~ 15C) and *_low (< -15C) temparatures.
...
static void simple_update(uint32_t seg) { 
    ...
    eid_enable_cp_ck(0x1, 0x0, 0x0);
    // Make selected segments black
    eid_enable_cp_ck(0x0, 0x0, seg&0x1FF);
}
...
static void local_wb_update(uint32_t seg) { 
    ...
    if (seg_b2w != 0) eid_enable_cp_ck(0x1, 0x1, (~seg_b2w & 0x1FF));
    // Perform White to Black
    if (seg_w2b != 0) eid_enable_cp_ck(0x0, 0x0, seg_w2b & 0x1FF);
...
#define WAIT_TIME_MIN 30
#define DEF_THRESHOLD WAIT_TIME_MIN*90000       // 90000 is 1 min
...
int main(void) {
    ...
    *SCTR_REG_CLR_WUP_SOURCE = 1;
    ...
    if (!get_flag(FLAG_INITIALIZED)) {
        operation_init();
        eid_update_global(/*seg*/DISP_NONE);
    }
    else {
        uint32_t goc_raw = *GOC_DATA_IRQ;
        uint32_t goc_header = goc_raw >> 24;
        uint32_t goc_data = goc_raw & 0xFFFFFF;
        *GOC_DATA_IRQ=0;

        /* Global Update (1 segment) */
        if (goc_header == 0x00) {
            if (goc_data == 0xE)
                eid_update_global(/*seg*/0x0);
            else if (goc_data == 0xF)
                eid_update_global(/*seg*/0x1FF);
            else 
                eid_update_global(/*seg*/0x1<<goc_data);
        }
        /* Local Update (1 segment) */
        else if (goc_header == 0x01) {
            if (goc_data == 0xE)
                eid_update_local(/*seg*/0x0);
            else if (goc_data == 0xF)
                eid_update_local(/*seg*/0x1FF);
            else
                eid_update_local(/*seg*/0x1<<goc_data);
        }
        /* Global Update (pattern) */
            ...
            if (goc_data == 0xE)
                ...
                eid_update_global(/*seg*/goc_data&0x1FF);
        }
        /* Local Update (pattern) */
            ...
            if (goc_data == 0xE)
                ...
                eid_update_local(/*seg*/goc_data&0x1FF);
        }
        /* Configuration - Duration */
        else if (goc_header == 0x04) {
            eid_set_duration(goc_data);  // 500 is the default duration for *_mid (=-5 ~ 15C) and *_low (< -15C) temparatures.
        }
    }
...
//        operation_init();
//        reset_disp();
//        snt_set_wup_timer(/*auto_reset*/1, /*threshold*/DEF_THRESHOLD);
//        operation_sleep();
//    }
//    else if (*GOC_DATA_IRQ==0x1) {
//        *GOC_DATA_IRQ=0;
//        reset_disp();
//        set_flag(FLAG_DONE, 0);
//        snt_set_wup_timer(/*auto_reset*/1, /*threshold*/DEF_THRESHOLD);
//        operation_sleep();
//    }
//    else if (!get_flag(FLAG_DONE)) {
//
//        uint32_t snt_curr = snt_read_wup_timer();
//
//        if (snt_curr > ((DEF_THRESHOLD>>1) + (DEF_THRESHOLD>>2))) {
//
//            // Make all segments/field white
//            eid_set_duration(1000);
//            eid_enable_cp_ck(0x1, 0x0, 0x0);
//            delay(2*DLY_1S);
//            eid_set_duration(500);
//            set_flag(FLAG_DONE, 1);
//            snt_set_wup_timer(/*auto_reset*/1, /*threshold*/0);
//            operation_sleep();
//        }
//        else {
//            snt_set_wup_timer(/*auto_reset*/1, /*threshold*/DEF_THRESHOLD - snt_curr);
//            operation_sleep();
//        }
//
//
