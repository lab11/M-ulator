...
union status_t {
    struct {
        unsigned display_update_skipped :   1;  
    };
    uint32_t value;
};
    ...
    //------------------------------------------
        ...
        //--------------------------------------
            ...
            if (eeprom_user_config.temp_meas_start_delay!=0) 
                ...
                status.display_update_skipped = 1;
    ...
    if (xt1_state == XT1_ACTIVE) {
        ...
        if (// If this is the very first sample (right after the start delay)
           ...
           || (sub_sample_cnt==eeprom_user_config.temp_meas_interval)) {
            ...
            // This is to reset new_seg with start_delay=0. Otherwise, new_seg may retain previous 'cross' mark or the broken sign (i.e., FLAG_USE_DEFAULT=1), if any.
            if (status.display_update_skipped) {
                curr_seg = DISP_PLAY;
                new_seg = DISP_PLAY;
                status.display_update_skipped = 0;
            }
