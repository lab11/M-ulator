...
// Version 2.03 (no_devel)
//------------------------
#define HARDWARE_ID 0x01005843  // XT1r1 Hardware ID
#define FIRMWARE_ID 0x0203      // [15:8] Integer part, [7:0]: Non-Integer part
//-------------------------------------------------------------------------------------------
// < UPDATE HISTORY >
...
union eeprom_misc_config_t {
    struct {
        ...
        unsigned pmu_adc_sar_margin         :   1;  // [  6] PMU_ADC_SAR_MARGIN (Valid only if __PMU_ADC_FIXED_MARGIN__ is disabled)
                                                    //          0x1: 8%
                                                    //          0x0: 7% (forced if __PMU_ADC_FIXED_MARGIN__ is enabled)
        unsigned pmu_adc_sar_hysteresis     :   1;  // [  7] PMU_ADC_SAR_HYSTERESIS (Valid only if __PMU_ADC_FIXED_HYSTERESIS__ is disabled)
                                                    //          0x1: 1 (forced if __PMU_ADC_FIXED_HYSTERESIS__ is enabled)
                                                    //          0x0: 0
    };
    uint32_t value;
};
...
union buffer_t {
    struct {
        unsigned sample_count   :   24; // [23: 0] Sample Count
        unsigned last_qword_id  :    8; // [31:24] Last Qword ID
...
volatile uint32_t buffer_valid;
    ...
    if (xt1_state == XT1_ACTIVE) {
        ...
        if (// If this is the very first sample (right after the start delay)
           ...
           || (sub_sample_cnt==eeprom_user_config.temp_meas_interval)) {
            ...
            #ifdef DEVEL
                mbus_write_message32(0x9C, snt_temp_val);
            #endif
            ...
            if (sample_type) {
                    ...
                    nfc_i2c_byte_write( /*e2*/   0,
                        ...
                        mbus_write_message32(0x9D, snt_temp_val);
...
static void update_system_configs(uint32_t use_default) {
    ...
    if (use_default) {
        ...
        //--- Other Configurations
        //eeprom_misc_config.pmu_adc_sar_hysteresis   =  1;
        //eeprom_misc_config.pmu_adc_sar_margin       =  0;
        //eeprom_misc_config.eid_disable_local_update =  1;
        //eeprom_misc_config.eid_crit_temp_threshold  =  0;
        //eeprom_misc_config.pmu_num_cons_meas        =  5;
        eeprom_misc_config.value = (1 << 7) | (0 << 6) | (1 << 5) | (0 << 4) | (5 << 0);
...
//  -2048~+2047 {111111, value&0xFFF}   18
//-----------------------------------------------
