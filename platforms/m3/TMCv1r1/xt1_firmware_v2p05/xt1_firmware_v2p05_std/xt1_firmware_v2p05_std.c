...
// Version 2.05 (standard)
//------------------------
#define HARDWARE_ID 0x01005843  // XT1r1 Hardware ID
#define FIRMWARE_ID 0x0205      // [15:8] Integer part, [7:0]: Non-Integer part
//-------------------------------------------------------------------------------------------
// < UPDATE HISTORY >
...
//                                                  0x0: Word Read operation has terminated with a timeout error
...
volatile uint32_t sample_type;      // If 1 (SAMPLE_RAW)   : store 16-bit Raw value.                                                                                                                              
                                    // If 0 (SAMPLE_NORMAL): store converted & compressed values.                                                                                                                 
...
static void set_system(uint32_t target) {
    ...
    //------------------------------------------
        ...
        //--------------------------------------
            ...
            // Display Play Sign 
            // If start_delay=0, the display does not need to be updated here. It will be updated in snt_operation() to show play+check or play+cross.
    ...
    if (xt1_state == XT1_ACTIVE) {
        ...
        if (// If this is the very first sample (right after the start delay)
           ...
           || (sub_sample_cnt==eeprom_user_config.temp_meas_interval)) {
            ...
            #ifdef DEVEL
                if (sample_type==SAMPLE_NORMAL) mbus_write_message32(0x9C, snt_temp_val);
            #endif
            ...
            // sample_type: SAMPLE_NORMAL
                ...
                // Update Valid Sample Bit (This writes into EEPROM, regardless of the Sample Buffer status)
            ...
            //--- If there is no 'SLASH', it means either 1) it was in XT1_PEND state (i.e., play sign only) -or- 2) FLAG_USE_DEFAULT=1 and start_delay=0 (i.e., the broken sign)
            //     If 1) is true, you do not need to do any further action here. But there is no harm to remove TICK and BACK_SLASH.
            //     If 2) is true, you need to remove the TICK and BACK_SLASH (i.e., the broken screen when using the default values)
            //                    Otherwise, the TICK and BACK_SLASH will stay there, generating TICK+CROSS display, when it is supposed to display CHECK.
            if (!get_bit(new_seg, SEG_SLASH)) {
                new_seg &= ~DISP_TICK;
                new_seg &= ~DISP_BACKSLASH;
            }
    ...
    if (cmd_chksum == ((cmd_raw+cmd_param)&0xFF)) {
        ...
        //-----------------------------------------------------------
            ...
            if (// Raw Sample
                ...
                || ( get_flag(FLAG_AES_KEY_SET) && get_flag(FLAG_CONFIG_SET) )
                ) {
        ...
        //-----------------------------------------------------------
            ...
            if (// Raw Sample
                (sample_type==SAMPLE_RAW)
                // Normal Sample requires 1) AES KEY is set -AND- 2) XT1 is in PEND or ACTIVE state
                || ( get_flag(FLAG_AES_KEY_SET) && ((xt1_state==XT1_PEND) || (xt1_state==XT1_ACTIVE)))
                ) {
...
static void update_system_configs(uint32_t use_default) {
    ...
    if (use_default) {
        ...
        //--- Other Configurations
        //eeprom_misc_config.ignore_first_sample_exc  =  0; // [8]
        ...
        eeprom_misc_config.value = (0 << 8) | (1 << 7) | (0 << 6) | (1 << 5) | (0 << 4) | (5 << 0);
