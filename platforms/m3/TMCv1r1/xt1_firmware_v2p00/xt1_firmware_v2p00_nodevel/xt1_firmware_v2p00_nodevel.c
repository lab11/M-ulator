...
// Version 2.00 (no_devel)
//------------------------
#define HARDWARE_ID 0x01005843  // XT1r1 Hardware ID
#define FIRMWARE_ID 0x0200      // [15:8] Integer part, [7:0]: Non-Integer part
//-------------------------------------------------------------------------------------------
// < UPDATE HISTORY >
    ...
    //-----------------------------------------
        ...
        // Commands requiring further operation
            ...
            // Command: NEWKEY
                ...
                if(nfc_i2c_word_read_volatile(/*e2*/0, /*addr*/EEPROM_ADDR_AES_KEY, /*nw*/4, /*ptr*/aes_key)) {
        ...
        // Normal User commands (START, STOP, RESET)
        else {
            set_system(/*target*/target);
            snt_skip_calib = 1;

            if (cmd==CMD_START) {
                aes_encrypt_eeprom(/*addr_begin*/EEPROM_ADDR_PRODUCT_INFO, /*addr_end*/EEPROM_ADDR_CMD-1);
            }
            else if (cmd==CMD_STOP) {
                aes_encrypt_eeprom(/*addr_begin*/EEPROM_ADDR_START_TIME, /*addr_end*/EEPROM_ADDR_CMD-1);
            }
        }
