...
//                                                  0x3: 'restart' is 1. Will use the previous values as they are.
...
//  -----------------------------------------------------------------------------------------
//  0x90    snt_temp_val                        Temperature Raw Code (EEPROM will NOT be written in this session)
//  0x91    snt_temp_val                        Temperature Raw Code (EEPROM WILL be written in this session)
...
volatile uint32_t temp_meas_config; // Temperature Measurement Configuration. A copy of cmd_param upon START command.
...
static void set_system(uint32_t target) {
    ...
    //------------------------------------------
        ...
        if (target==XT1_RESET) {
            ...
            set_system_state(/*msb*/7, /*lsb*/0, /*val*/(0<<7)|(0<<6)|XT1_RESET);
        ...
        //--------------------------------------
            ...
            set_system_state(/*msb*/1, /*lsb*/0, /*val*/XT1_PEND);
            // Display Play Sign
...
static void snt_operation (uint32_t update_eeprom) {
    ...
    *NVIC_ISER = (0x1 << (SNT_TARGET_REG_IDX+8));
    ...
    #ifdef DEVEL
        mbus_write_message32(0x90|update_eeprom, snt_temp_val);
    #endif
...
static void update_system_configs(uint32_t use_default) {
    ...
    if (use_default) {
        ...
        //--- VBAT and ADC
        eeprom_adc_t1           = 2534; // ( 2534 ) Temperature Threshold (Raw Code) for ADC.
        eeprom_adc_t2           = 2534; // ( 2534 ) Temperature Threshold (Raw Code) for ADC.
        eeprom_adc_t3           = 2534; // ( 2534 ) Temperature Threshold (Raw Code) for ADC.
        eeprom_adc_t4           = 2534; // ( 2534 ) Temperature Threshold (Raw Code) for ADC.
    ...
    //-----------------------------------------
        ...
        if ((target==0xDEAD)     // Command Read Fail
         || (target==0xF00D)     // Invalid Handshaking (REQ=0 or ACK=1 or ERR=1)
            ...
            // Go to Sleep. FIXME: How to handle if the next snt_threshold is too close?
            operation_sleep();
        }
