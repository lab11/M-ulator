...
uint32_t div (uint32_t dividend, uint32_t divisor) {
    ...
    uint32_t idx;
        ...
        mbus_write_message32(0xC0, divisor);
    #endif

    // Get the integer part
    while (dividend >= divisor) {
        result += (1 << (32-__NUM_INT_DIGITS__));
        dividend -= divisor;
    }

    // Get the non-integer part
    divisor = divisor >> 1;
    for (idx=0; idx<(32-__NUM_INT_DIGITS__); idx++) {
        if (dividend >= divisor) {
            result |= (0x1 << idx);
            dividend -= divisor;
        }
...
uint32_t mult_dec (uint32_t num_int, uint32_t num_dec) {
    #ifdef DEVEL
        mbus_write_message32(0xC2, num_int);
        mbus_write_message32(0xC2, num_dec);
    #endif

    uint32_t int_part = num_dec>>(32 - __NUM_INT_DIGITS__);
    uint32_t dec_part = (num_dec<<__NUM_INT_DIGITS__)>>__NUM_INT_DIGITS__;
    uint32_t result = 0;
    ...
    // Non-Integer part
    uint32_t insig = 0; // fixed-point, N=32
    uint32_t error_dec = 0; // error decimal part
    uint32_t error_int = 0; // error integer part
    insig = (num_int&0x1)<<31;
    num_int = num_int >> 1;
    while ((num_int!=0) && (dec_part!=0)) {
        if (dec_part&0x1) {
            result += num_int;
            if (insig>(0xFFFFFFFF-error_dec)) error_int++;
            error_dec += insig;
        }
        insig = ((num_int&0x1)<<31)|(insig>>1);
        num_int = num_int >> 1;
        dec_part = dec_part >> 1;
    }

    // round-up the error and get an integer value
    if ((error_dec>>31)&0x1) error_int++;

    // compensate the error
    result += error_int;

    #ifdef DEVEL
        mbus_write_message32(0xC3, error_int);
...
uint32_t mult (uint32_t num_a, uint32_t num_b) {
    #ifdef DEVEL
        mbus_write_message32(0xC4, num_a);
        mbus_write_message32(0xC4, num_b);
    #endif
//#ifdef USE_M0_MULT
//    return num_a * num_b;
//#else
    ...
    for(idx=0; idx<32; idx++) {
        if (num_a&0x1) result += num_b;
        num_a = num_a >> 1;
        num_b = num_b << 1;
    }
    #ifdef DEVEL
        mbus_write_message32(0xC5, result);
    #endif
    return result;
//#endif
}
...
void eid_init(uint32_t ring, uint32_t te_div, uint32_t fd_div, uint32_t seg_div){
    ...
    // Watchdog Threshold; based on the 94Hz clock; Max: 16777215 (49.5 hours)
    //__crsh_wd_thrshld__       =   360000;  // 64 min
    //__crsh_wd_thrshld__       =  8121600;  // 24 hours
    __crsh_wd_thrshld__         = 12182400;  // 36 hours
    //__crsh_wd_thrshld__       = 16243200;  // 48 hours
