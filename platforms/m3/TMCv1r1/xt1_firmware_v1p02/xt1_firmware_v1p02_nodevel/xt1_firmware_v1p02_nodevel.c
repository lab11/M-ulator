...
// Version 1.02 (no_devel)
//------------------------
#define HARDWARE_ID 0x01005843  // XT1r1 Hardware ID
#define FIRMWARE_ID 0x0102      // [15:8] Integer part, [7:0]: Non-Integer part
//-------------------------------------------------------------------------------------------
// < UPDATE HISTORY >
...
static void snt_operation (uint32_t update_eeprom) {
    ...
    *NVIC_ISER = (0x1 << (SNT_TARGET_REG_IDX+8));
    ...
    // Get the measurement and convert the raw data into temperature
    // NOTE: snt_temp_val is "10 x (T + 80)" where T is the actual temperature in celsius degree
    snt_temp_val= tconv(/* dout */ *SNT_TARGET_REG_ADDR, 
    ...
    #ifdef DEVEL
        mbus_write_message32(0x90|update_eeprom, (*SNT_TARGET_REG_ADDR<<16)|snt_temp_val);
    #endif
