...
// Version 1.04 (no_devel)
//------------------------
#define HARDWARE_ID 0x01005843  // XT1r1 Hardware ID
#define FIRMWARE_ID 0x0104      // [15:8] Integer part, [7:0]: Non-Integer part
//-------------------------------------------------------------------------------------------
// < UPDATE HISTORY >
...
//  0xA3    eid_duration                        EID duration has bee set before updating the display
//
//  -----------------------------------------------------------------------------------------
//  < NFC and EEPROM >
//  -----------------------------------------------------------------------------------------
...
//                                                              0x10: DISPLAY     
//                                                              0x1E: RESET       
//  0xB4    {0x0, value}                        GETSEC successfully replied
//                                                  [ 5: 0]: last_chunk_id
//  0xB4    {0x1, value}                        GETSEC successfully replied
//                                                  [24: 8]: Number of seconds since the last sample (num_seconds)
//                                                  [ 7: 0]: Value written into EEPROM_ADDR_CMD
//  0xB5    {0x0, value}                        GETSEC failed to reply
//                                                  [ 5: 0]: last_chunk_id
//  0xB5    {0x1, value}                        GETSEC failued to reply
//                                                  [24: 8]: Number of seconds since the last sample (num_seconds)
//                                                  [ 7: 0]: Value supposed to be written into EEPROM_ADDR_CMD
//
//  -----------------------------------------------------------------------------------------
//  < Math Functions >
//  -----------------------------------------------------------------------------------------
//  div()
//  0xC0    dividend (int) or divisor (int)     Pair of messages. The first one is dividend, the second one is divisor.
//  0xC1    result (custom FP)                  result = dividend / divisor
//
//  mult_dec()
...
//  0xC5    result (int)                        result = num_a * num_b
//
//  div_int()
//  0xC6    dividend (int) or divisor (int)     Pair of messages. The first one is dividend, the second one is divisor.
//  0xC7    result (int)                        result = dividend / divisor
//
//  -----------------------------------------------------------------------------------------
...
// Pre-defined Wakeup Intervals
    ...
    #define WAKEUP_INTERVAL_IDLE    5    // (Default: 1440) Wakeup Interval for XT1_IDLE (unit: minutes)
#else
    #define WAKEUP_INTERVAL_IDLE    1440    // (Default: 1440) Wakeup Interval for XT1_IDLE (unit: minutes)
#endif
    ...
    *NVIC_ICER = 0xFFFFFFFF;
    ...
    if (check_snt) {
        ...
        uint32_t dist_min = get_high_power_history()  
                            ...
                            : (snt_freq<<2);              // 4s
        uint32_t dist_max = (snt_freq<<18);               // 262144s (= 4369min = 73hrs)
...
static void set_system(uint32_t target) {
    ...
    //------------------------------------------
        ...
        if (target==XT1_RESET) {
            ...
            system_state = 0;
            set_system_state(/*msb*/7, /*lsb*/0, /*val*/(0<<7)|(0<<6)|(0<<5)|XT1_RESET);
        ...
        while (num_buffer_items!=0) {
            ...
            // STORE THE CONVERTED & COMPRESSED DATA
                ...
                if (bit_pos_end < 64512) { // NOTE: 64512 = (8192 - 128) * 8
                    ...
                    //--- Initialize wdata 
                    wdata = set_bits(wdata, 31, bit_offset, 0);
                    ...
                    while (remaining_bits>0) {
                        ...
                        wdata |= (((reverse_code<<bit_offset)&0xFF) << (nb<<3));
...
static void calibrate_snt_timer(uint32_t restart) {
        ...
        // XO is running
            ...
            if (temp_xo_val <= xo_val_curr) {
                ...
                if (restart||SLEEP_BYPASSED||get_flag(FLAG_SLEEP_BYPASSED)) {
                    ...
                    temp_snt_freq = mult_dec(/*num_int*/snt_freq, /*num_dec*/a);
                    ...
                    if (  (temp_snt_freq > (snt_freq - (snt_freq>>2)))
                       && (temp_snt_freq < (snt_freq + (snt_freq>>2)))
                       ) {
    ...
    // PASS
        ...
        //--- if A >= 1
        if (get_bits(/*var*/a, /*msb*/31, /*lsb*/30)>=1) {
            snt_duration = mult(/*num_a*/wakeup_interval_sec, /*num_b*/snt_freq)
                            + mult_dec(/*num_int*/snt_duration, /*num_dec*/a&0xBFFFFFFF);
        }
        //--- if A < 1
        else {
            snt_duration = mult(/*num_a*/wakeup_interval_sec, /*num_b*/snt_freq)
                            - mult_dec(/*num_int*/snt_duration, /*num_dec*/(~a)&0x3FFFFFFF)
                            - (snt_duration>>30);
        }
...
static void eid_update(uint32_t seg) {
    ...
    #ifdef DEVEL
        mbus_write_message32(0xA3, eid_duration);
    #endif
...
uint32_t nfc_check_cmd(void) {
    ...
    //-----------------------------------------------------------
        ...
        uint32_t snt_val = snt_read_wup_timer() - snt_threshold_prev;
        uint32_t num_seconds = div_int(/*dividend*/snt_val, /*divisor*/snt_freq);
        uint32_t last_chunk_id = ((comp_bit_pos-1)>>3)>>8;
        uint32_t data = (last_chunk_id<<25)|(num_seconds<<8)|(0x1<<6)|cmd;
        uint32_t num_try=10;

        // Write the required data and ack
        while (num_try!=0) {
            nfc_i2c_start();
            if (nfc_i2c_byte_imm(0xA6)) {
                nfc_i2c_byte(EEPROM_ADDR_CMD >> 8);
        ...
        #ifdef DEVEL
            if (num_try!=0) {
                mbus_write_message32(0xB4, (0x0<<31)|last_chunk_id);
                mbus_write_message32(0xB4, (0x1<<31)|(data&0x1FFFFFF));
            }
            else {
                mbus_write_message32(0xB5, (0x0<<31)|last_chunk_id);
                mbus_write_message32(0xB5, (0x1<<31)|(data&0x1FFFFFF));
            }
        #endif
    }
    //-----------------------------------------------------------
...
int main(void) {
    ...
    *NVIC_ISER = (0x1 << IRQ_TIMER32);
    ...
    if (WAKEUP_BY_SNT) {
        ...
        uint32_t update_eeprom;

        if (xt1_state==XT1_PEND) {
            cnt_update_eeprom++;
            update_eeprom = nfc_i2c_get_token(/*num_try*/30) && (cnt_update_eeprom >= EEPROM_UPDATE_INTERVAL);
            #ifdef DEVEL
            ...
            cnt_update_eeprom++;
            update_eeprom = nfc_i2c_get_token(/*num_try*/1) && (cnt_update_eeprom >= EEPROM_UPDATE_INTERVAL);
            #ifdef DEVEL
                mbus_write_message32((0xB<<4)|update_eeprom, cnt_update_eeprom);
            #endif
            if (update_eeprom) cnt_update_eeprom = 0;
        }
        else {
        ...
        // Command Successfully Read
            ...
            // Other Commands:
                ...
                // Command: HARD_RESET
                    ...
                    eid_update(/*seg*/DISP_PLUS|DISP_MINUS);
