...
#define BUFFER_SIZE             4096  // Buffer Size; It must be larger than EEPROM_UPDATE_INTERVAL
...
// Pre-defined Wakeup Intervals
    ...
    #define WAKEUP_INTERVAL_IDLE    43200    // (Default: 43200) Wakeup Interval for XT1_IDLE (unit: seconds)
#endif
...
volatile uint32_t max_sample_count;     // v1p15t Only: Maximum Sample Count. Once XT1 measures this number of samples, it automatically stops.
...
static void set_system(uint32_t target) {
    ...
    //------------------------------------------
        ...
        if (target==XT1_RESET) {
            ...
            cmd                      = CMD_NOP;
            sample_type              = 0;
            disp_refresh_interval    = 86400;   // Every 24 hrs
            eid_duration             = 100;
...
//                );
            ...
            if (xt1_state == XT1_ACTIVE)
                buffer_add_item(get_bits(buf_val, 31, 16));
...
//                    );
                ...
                if (xt1_state == XT1_ACTIVE)
                    buffer_add_item(eeprom_sample_count+1);
...
//                                        /*nb*/   nb);
                    ...
                    if (buf_temp_val > highest_temp) {
                        ...
                        if (xt1_state == XT1_ACTIVE) buffer_add_item(highest_temp);
                    }
                    if (buf_temp_val < lowest_temp) {
                        ...
                        if (xt1_state == XT1_ACTIVE) buffer_add_item(lowest_temp);
                    }
                    ...
                    if (buf_temp_val > eeprom_high_temp_threshold) {
                        ...
                        if (curr_act) {
                            if (curr_num != 0x3F) {
                                ...
                                if (xt1_state == XT1_ACTIVE) buffer_add_item(high_sample_id_status);
                            }
                        }
                        ...
                        if (curr_act) {
                            if (curr_num != 0x3F) {
                                ...
                                if (xt1_state == XT1_ACTIVE) buffer_add_item(low_sample_id_status);
                            }
                        }
                        // If this is a start of a new streak
                        else {
...
//        }
    ...
    //------------------------------------------------------------------------------------
    // NOTE: v1p15t does not refresh the display, most of the section below is commented out
    //------------------------------------------------------------------------------------
...
static void calibrate_snt_timer(uint32_t skip_calib) {
    ...
    //uint32_t wakeup_interval_sec = (wakeup_interval << 6) - (wakeup_interval << 2);
        ...
        mbus_write_message32(0x8A, wakeup_interval);
    #endif
    ...
    if (fail) {
        ...
        snt_duration = mult(/*num_a*/wakeup_interval, /*num_b*/snt_freq);
...
static void eid_update_configs(void) {
    ...
    #else
        // convert hr to sec (3600x)
        disp_refresh_interval =   (new_val << 11)   // 2048
                                + (new_val << 10)   // 1024
                                + (new_val <<  9)   //  512
                                + (new_val <<  4);  //   16
    #endif
...
static void update_system_configs(uint32_t use_default) {
    ...
    eid_crit_temp_threshold = eeprom_eid_crit_temp_threshold ?
                                600 :   // 0x1: -20C
                                550 ;   // 0x0: -25C
}
...
static void update_user_configs(void) {
    ...
    // max_sample_count = 10x(eeprom_high_temp_threshold-800)
    //                  = 10x(10x(threshold_in_c+80) - 800)
    //                  = 10x(10xthreshold_in_c + 800 - 800)
    //                  = 100xthreshold_in_c

    max_sample_count = eeprom_high_temp_threshold;
    max_sample_count -= 800;
    max_sample_count =   (max_sample_count<<3)
                       + (max_sample_count<<1);
    ...
    *NVIC_ISER = (0x1 << IRQ_TIMER32);
    ...
    if (WAKEUP_BY_SNT) {
        ...
        if (xt1_state==XT1_ACTIVE) {
            ...
            if ((max_sample_count!=0)&&(temp_sample_count<max_sample_count)) {

                mbus_write_message32(0x92, temp_sample_count);

                snt_operation(/*update_eeprom*/1);
                ...
                // Terminate (execute the stop command)
                system_status_at_stop = calc_system_status_at_stop();
                set_system(/*target*/XT1_IDLE);
                snt_skip_calib = 1;
            }
        }
        else {
