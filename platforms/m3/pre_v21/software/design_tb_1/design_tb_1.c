...
void cycle1 (void) {
    if (do_cycle1 == 1) {
        ...
        if (!get_flag(FLAG_XO_SUB_2) && !get_flag(FLAG_XO_SUB_1) && !get_flag(FLAG_XO_SUB_0)) {
            ...
            *REG_XO_CONF1 = 0x302701; // Un-power-gate XO Driver
            *REG_XO_CONF1 = 0x102701; // Un-isolate XO Driver
...
void cycle2 (void) {
    if (do_cycle2 == 1) {
        ...
        if (!get_flag(FLAG_SOFT_RESET)) {
            ...
            *REG_SYS_CONF =   (1 << 6)  // PUF_CHIP_ID_SLEEP
                            | (1 << 5)  // PUF_CHIP_ID_ISOLATE
                            | (0 << 4)  // ENABLE_SOFT_RESET
