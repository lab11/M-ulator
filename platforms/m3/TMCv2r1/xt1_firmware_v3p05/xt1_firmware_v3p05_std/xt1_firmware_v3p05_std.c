...
//                                                  0x00000003: Skipped eid_update_configs() due to hibernation
...
//  0xB5    addr                                dat_buf.addr
//  0xB6    data                                Encrypted data
//  0xB7    CRC                                 dat_buf.crc32
//
//  --- all_buffer_commit()
...
volatile union eeprom_user_config_0_t eeprom_user_config_0;
...
union cnt_buf_t {
    struct {
        ...
        unsigned meas_cnt       :   23; // [54:32] Measurement Count
        unsigned rsvd0          :    8; // [62:55] 
        unsigned valid          :    1; // [63] 
    };
    struct {
        unsigned sample_info    :   32; // [31: 0] Last Qword ID and Sample Count
        unsigned meas_info      :   32; // [63:32] Valid & Measurement Count
    };
    uint64_t value;
};
volatile union cnt_buf_t cnt_buf;
...
union dat_buf_t {
    struct {
        ...
        unsigned addr   :   16; // [47:32] EEPROM Address
        unsigned rsvd0  :   15; // [62:33]
        unsigned valid  :    1; // [63] Becomes 1 when updating buf_storage and dat_buf.crc32; Becomes 0 after writing buf_storage and dat_buf.crc32 in EEPROM.
    };
    uint64_t value;
};
volatile union dat_buf_t dat_buf;
...
static void operation_sleep (uint32_t check_snt) {
    ...
    if (meas.low_vbat) display_low_batt(); // NOTE: If status.hbr_flag=1, then eid_update_display() within display_low_batt() updates flags only; 
...
static void set_system(uint32_t target) {
    ...
    //--------------------------------------
        ...
        if (status.sample_type == SAMPLE_RAW) {
            ...
            eeprom_user_config_0.temp_meas_interval    = 1;      // 1 min
            eeprom_user_config_0.temp_meas_start_delay = 0;      // 0 min
...
static void meas_temp_adc (uint32_t go_sleep) {
    ...
    if (!meas.valid) {
        ...
        if (!status.hbr_flag) eid_update_configs();
        #ifdef DEVEL
            else {
...
static void snt_operation (void) {
    ...
    if (xt1_state == XT1_ACTIVE) {
        ...
        if (// If this is the very first sample (right after the start delay)
           ...
           || (sub_meas_cnt==eeprom_user_config_0.temp_meas_interval)) {

            // Increment the Measurement Count
            meas_cnt++;
            cnt_buf.meas_cnt = meas_cnt;
            cnt_buf.valid = 1;
                ...
                // sample_type: SAMPLE_NORMAL
                    ...
                    if (comp_sample_id==0) {
                        if (status.hibernating) {
                            ...
                            reverse_code = reverse_bits_ext(/*code*/0x0, /*count*/hbr_meas_cnt, /*value*/temp.val);
                        }
                        else {
                    ...
                    //--- For non-128*N-th samples)
                        ...
                        if (status.hibernating) {
                            ...
                            reverse_code = reverse_bits_ext(/*code*/0x20, /*count*/hbr_meas_cnt, /*value*/temp.val);
                        }
                        else {
                            uint32_t comp_result = tcomp_encode(/*delta*/temp.val - comp_prev_sample);
                            num_bits = comp_result >> 24;
                            reverse_code = comp_result&0xFFFFFF;
                        }
                    }
                    ...
                    comp_prev_sample = temp.val;
                        ...
                        mbus_write_message32(0x98, rsh64(/*var*/reverse_code, /*nb*/32));
                        mbus_write_message32(0x99, reverse_code&0xFFFFFFFF);
                    #endif
                    ...
                    sample_cnt++;
                    cnt_buf.sample_cnt    = sample_cnt;
                    cnt_buf.last_qword_id = last_qword_id;
                    ...
                    if (eeprom_avail) {
                        ...
                        // CASE II) Bits go across different qwords (bit_pos_start>>7 != bit_pos_end>>7)
                            ...
                            sub_qword(/*msb*/bit_pos_end&0x7F, /*lsb*/0, /*value*/rsh64(/*var*/reverse_code, /*nb*/(128-(bit_pos_start&0x7F))));
...
static void eid_update_display(uint32_t seg) {
    ...
    if ((seg != prev_seg) && !status.hbr_flag) {
        ...
        if(nfc_i2c_get_token(/*num_try*/30)) {
            ...
            #ifdef DEVEL
                mbus_write_message32(0xA0, (0x0<<28) | seg);
            #endif
        }
        #ifdef DEVEL
        else {
                mbus_write_message32(0xA0, (0x2<<28) | seg);
        }
        #endif
    }
    #ifdef DEVEL
...
static void update_user_configs(void) {
    ...
    nfc_i2c_word_read(/*e2*/0, /*addr*/EEPROM_ADDR_HIGH_TEMP_THRESHOLD, /*nw*/2, /*ptr*/ (volatile uint32_t *)&eeprom_user_config_0.value);
    ...
    #ifdef DEVEL
        mbus_write_message32(0x74, eeprom_user_config_0.lower);
        mbus_write_message32(0x75, eeprom_user_config_0.upper);
...
static void data_buffer_update (uint32_t addr, uint32_t commit_crc) {
    ...
    // Calculate CRC
    dat_buf.crc32 = calc_crc32_128(/*src*/text, /*crc_prev*/crc32);
    #ifdef DEVEL
        mbus_write_message32(0xB7, dat_buf.crc32);
    #endif

    // Commit CRC
    if (commit_crc) crc32 = dat_buf.crc32;
...
static void all_buffer_commit(void) {
    ...
    if (cnt_buf.valid) {
        ...
        nfc_i2c_byte_write(/*e2*/ 0, /*addr*/ EEPROM_ADDR_SAMPLE_COUNT, /*data*/ cnt_buf.sample_info, /*nb*/ 4);
        // Measurement Count
        nfc_i2c_byte_write(/*e2*/ 0, /*addr*/ EEPROM_ADDR_MEAS_COUNT,   /*data*/ cnt_buf.meas_cnt,    /*nb*/ 3);
    ...
    if (dat_buf.valid) {
        // Data
        nfc_i2c_word_write(/*e2*/ 0, /*addr*/ dat_buf.addr,  /*data*/ (uint32_t *) buf_storage, /*nw*/ 4);
        // CRC
        nfc_i2c_byte_write(/*e2*/ 0, /*addr*/ EEPROM_ADDR_CRC,  /*data*/ dat_buf.crc32, /*nb*/ 4);
        ...
        if (WAKEUP_BY_NFC) {
            if (status.nfc_out_temp) {
                ...
                if (nfc_i2c_get_token(30)) {
                    ...
                    #ifndef DEVEL
                        // Command: DISPLAY (Available Only in *_nodevel and *_std)
