...
//-------------------------------------------------------------------
// GOC CRC IMPLEMENTATION
//-------------------------------------------------------------------
#define PTR_LAST_MEM_ADDR  ((volatile uint32_t *) 0x00000010)   // Pointer to store the Last Memory Address
...
// TIMEOUT CHECK
//-------------------------------------------------------------------
#define FAIL_ID_NONE    0           // No Fail (initial value)
#define FAIL_ID_EID     1           // Display Update
#define FAIL_ID_WUP     2           // SNT Timer Reading (Timeout)
#define FAIL_ID_PMU     4           // PMU Register Access
#define FAIL_ID_SNT     8           // SNT Temperature Measurement
#define FAIL_ID_I2C     16          // I2C NAK Failure
#define FAIL_ID_MET     32          // SNT Timer Reading (Meta-stability)
#define FAIL_ID_WTH     64          // SNT Timer Threshold Update (Timeout)
#define FAIL_ID_GEN     128         // Generic/Unknown
#define FAIL_ID_INIT    0x1010DEAD  // Timeout/Fail during Initialization
#define FAIL_ID_SOFT    999         // Soft Error
#define FAIL_ID_PEND    9999        // Pending Handling

#define TIMEOUT_1S      CPU_CLK_FREQ    // TIMER32 Threshold to measure 1 second
...
// Args    : None
// Description:
//           Resets/Initilalizes the EEPROM debug section
// Return  : None
//-------------------------------------------------------------------
void eeprom_init(void);

//-------------------------------------------------------------------
// Function: eeprom_increment_byte
// Args    : addr   : 16-bit byte address ( 2 LSBs are internally forced to 0 to make it word-aligned)
// Description:
//           Read the 1-byte value from 'addr'. Increment the value and write back into the same 'addr'.
//           If the value is already 0xFF, it does not write back (i.e., the value at 'addr' does not change.)
// Return  : Final value stored at 'addr' 
//-------------------------------------------------------------------
...
// Store number of EID Timer failures (#restarts)
#define __DEBUG_EID_TIMER__

// Max Num Retry for EID Update
#define __EID_NUM_RETRY__   5

// EID "Retry" delay. 
// When EID does not respond, it puts EID in sleep and wait for __EID_RETRY_DELAY__.
// Then, it wakes up the EID again, and wait for __EID_RETRY_DELAY__,
// then it retries the display update.
#define __EID_RETRY_DELAY__  60000  // ~2s
...
//-------------------------------------------------------------------
// Function: eid_stop_timer
// Args    : None
// Description:
//           Explicitly stop the EID timer.
// Return  : None
//-------------------------------------------------------------------
void eid_stop_timer(void);

//-------------------------------------------------------------------
// Function: eid_restart_timer
// Args    : None
// Description:
//           Explicitly restart the EID timer
// Return  : None
//-------------------------------------------------------------------
...
// Store number of SNT Timer failures (#restarts)
#define __DEBUG_SNT_TIMER__

// Max Num Retry for SNT Access
#define __SNT_NUM_RETRY__   5

// If SNT detects a timeout during hand-shake, it sends __SNT_WUP_TIMEOUT_FLAG__
#define __SNT_WUP_TIMEOUT_FLAG__ 0xAD00DEAD

// Indicates whether the SNT timer has been restarted or not.
volatile uint32_t __snt_timer_restarted__;
...
//           It makes up to __SNT_NUM_RETRY__ attempts.
//           If SNT timeouts or sends back the timeout flags,
//           it restarts the timer and retries.
//           If it still fails after the maximum number of attempts,
//           it triggers fail_handler() with FAIL_ID_WTH.
// Return  : threshold
//              - If the timer gets restarted, it sets __snt_timer_restarted__.
//-------------------------------------------------------------------
...
//                          1: Auto Reset is enabled
//           altval     - Alternative threshold value in case the SNT timer needs to restart.
// Description:
//           (Synchronously) set the threshold for the SNT wakeup timer,
...
// Return  : threshold
//              - If the timer gets restarted, it resets the theshold to altval.
//-------------------------------------------------------------------
...
// Args    : check_restart  - If 1, it does not change __snt_timer_restart__.
//                            If 0, it resets __snt_timer_restart__ to 0.
// Description:
