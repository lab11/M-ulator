...
uint32_t pmu_calc_new_sar_ratio(uint32_t adc_val, uint32_t offset, uint32_t curr_sar) {
    ...
    if (pmu_validate_adc_val(adc_val)) {
        ...
        if (
            ...
            // Limit the change in SAR 
            if      (new_val > curr_sar) new_val = curr_sar + 1;
            else if (new_val < curr_sar) new_val = curr_sar - 1;
...
uint32_t nfc_i2c_byte_core(uint8_t byte){
    uint32_t nak;
    if (!__nfc_i2c_nak__) {
...
uint32_t nfc_i2c_byte(uint8_t byte){
    uint32_t ack = nfc_i2c_byte_core(byte);
    __nfc_i2c_nak__ = !ack;
    #ifdef DEVEL
        if (!ack) mbus_write_message32(0xEF, 0x12C0DEAD);
    #endif
    return ack;
}

uint32_t nfc_i2c_byte_imm (uint8_t byte){
    return nfc_i2c_byte_core(byte);
}
...
uint32_t nfc_i2c_byte_write(uint32_t e2, uint32_t addr, uint32_t data, uint32_t nb){
    ...
    // Still NAK'd after __NFC_NUM_RETRY__ tries.
...
void eeprom_init(void) {
    #ifdef __DEBUG_SNT_TIMER__
        #ifdef __DEBUG_EID_TIMER__
            // Both __DEBUG_SNT_TIMER__ and __DEBUG_EID_TIMER__ are enabled
            nfc_i2c_byte_write(/*e2*/0, /*addr*/EEPROM_ADDR_SNT_RESTART_COUNT, /*data*/0,   /*nb*/2);
        #else
            // Only __DEBUG_SNT_TIMER__ is enabled
            nfc_i2c_byte_write(/*e2*/0, /*addr*/EEPROM_ADDR_SNT_RESTART_COUNT, /*data*/0,   /*nb*/1);
        #endif
    #else
        #ifdef __DEBUG_EID_TIMER__
            // Only __DEBUG_EID_TIMER__ is enabled
            nfc_i2c_byte_write(/*e2*/0, /*addr*/EEPROM_ADDR_EID_RESTART_COUNT, /*data*/0,   /*nb*/1);
        #endif
    #endif
}

uint32_t eeprom_increment_byte (uint32_t addr) {
    uint32_t val = nfc_i2c_byte_read(/*e2*/0, /*addr*/addr, /*nb*/1);
    if (val<0xFF) {
        val++;
        nfc_i2c_byte_write(/*e2*/0, /*addr*/addr, /*data*/val, /*nb*/1);
    }
    return val;
}
...
void eid_stop_timer(void){
    #ifndef __USE_HCODE__
        eid_r00.TMR_EN_CLK_DIV = 0;
        mbus_remote_register_write(EID_ADDR,0x00,eid_r00.as_int);
        eid_r01.TMR_ISOL_CLK = 1;
        mbus_remote_register_write(EID_ADDR,0x01,eid_r01.as_int);
        eid_r01.TMR_SEL_LDO     = 0;
        eid_r01.TMR_SELF_EN     = 0;
        eid_r01.TMR_EN_SELF_CLK = 0;
        eid_r01.TMR_RESETB_DCDC = 0;
        eid_r01.TMR_RESETB_DIV  = 0;
        eid_r01.TMR_RESETB      = 0;
        mbus_remote_register_write(EID_ADDR,0x01,eid_r01.as_int);
    #else
        // TMR_EN_CLK_DIV: 1 -> 0
        mbus_remote_register_write(EID_ADDR,0x00,(/*EN_WATCHDOG*/0<<0)|(/*TMR_EN_CLK_DIV*/0<<1)|(/*TMR_EN_CLK_OUT*/0<<2));
        // TMR_ISOL_CLK: 0 -> 1
...
void eid_restart_timer(void) {
    // Stop the EID Timer
    eid_stop_timer();
    delay(__EID_RETRY_DELAY__);
    // Turn on EID clock
    eid_enable_timer();

    #ifdef __DEBUG_SNT_TIMER__
        eeprom_increment_byte(/*addr*/EEPROM_ADDR_EID_RESTART_COUNT);
    #endif
}
...
void eid_enable_cp_ck(uint32_t te, uint32_t fd, uint32_t seg) {
    ...
    while (iter < __EID_NUM_RETRY__) {
        ...
        start_timeout32_check(/*id*/FAIL_ID_SOFT, /*val*/TIMEOUT_TH<<4); // ~16s timeout
        ...
        // If EID timeout occurs
        if (__wfi_id__==FAIL_ID_PEND) {
            __wfi_id__=FAIL_ID_NONE;
            #ifdef DEVEL
                mbus_write_message32(0xE1, (0x1<<28) | iter);
            #endif
    ...
    // Still timeouts after __EID_NUM_RETRY__ tries.
    // -> Call fail_handler() by using the TIMER32 timeout check.
    start_timeout32_check(/*id*/FAIL_ID_EID, /*val*/TIMEOUT_TH); // ~16s timeout
    while(1);
}
...
void snt_restart_timer(void) {
    ...
    // Set the flag
    __snt_timer_restarted__ = 1;

    #ifdef __DEBUG_SNT_TIMER__
        eeprom_increment_byte(/*addr*/EEPROM_ADDR_SNT_RESTART_COUNT);
    #endif
}

uint32_t snt_set_timer_threshold(uint32_t threshold) {

    // SNT sent back __SNT_WUP_TIMEOUT_FLAG__ if it detects an internal timeout. 
    // We will try one more time if we receive this pattern,
    // just in case it is a real counter value.
    // We will not increment 'iter' variable in this case.
    uint32_t suspected_timeout = 0; 

    uint32_t iter = 0;
    while (iter < __SNT_NUM_RETRY__) {

        mbus_remote_register_write(SNT_ADDR,0x19,(0x10<<16)|(WP1_TARGET_REG_IDX<<8)|((threshold>>24)&0xFF));
        enable_reg_irq(WP1_TARGET_REG_IDX);
        start_timeout32_check(/*id*/FAIL_ID_SOFT, /*val*/TIMEOUT_TH);
        ...
        uint32_t result = *WP1_TARGET_REG_ADDR;

        // If result=0xDEAD, we try one more time without incrementing 'iter'
        // just to make sure that it is a real counter value, not the 'timeout' flag.
        if ((result == 0xDEAD) && (suspected_timeout==0)) {
            suspected_timeout=1;
            #ifdef DEVEL
                mbus_write_message32(0xE1, (0x2<<28) | iter);
            #endif
        }
        // If timeout occurs, or SNT keeps sending the timeout flag (0xDEAD)
        else if ((__wfi_id__==FAIL_ID_PEND) 
                || ( (result == 0xDEAD) && (suspected_timeout==1) )
                || ( result != (threshold&0xFFFFFF) )
                ){
            __wfi_id__=FAIL_ID_NONE;

            #ifdef DEVEL
                mbus_write_message32(0xE1, (0x3<<28) | iter);
            #endif

            iter++;
            snt_restart_timer();
        }
        // If everytning went well
        else {
            return threshold;
        }
    }

    // Still timeouts after __SNT_NUM_RETRY__ tries.
    // -> Call fail_handler() by using the TIMER32 timeout check.
    start_timeout32_check(/*id*/FAIL_ID_WTH, /*val*/TIMEOUT_TH);
    while(1);
}
...
uint32_t snt_set_wup_timer(uint32_t auto_reset, uint32_t threshold, uint32_t altval){
    snt_enable_wup_timer(auto_reset);
    uint32_t new_val = snt_set_timer_threshold(/*threshold*/threshold);
...
uint32_t snt_sync_read_wup_timer(void) {
    ...
    while (iter < __SNT_NUM_RETRY__) {
        ...
        // If result=__SNT_WUP_TIMEOUT_FLAG__, we try one more time without incrementing 'iter'
        // just to make sure that it is a real counter value, not the 'timeout' flag.
        if ((result == __SNT_WUP_TIMEOUT_FLAG__) && (suspected_timeout==0)) {
            suspected_timeout=1;
            #ifdef DEVEL
                mbus_write_message32(0xE1, (0x4<<28) | iter);
            #endif
        }
        // If timeout occurs, or SNT keeps sending the timeout flag (__SNT_WUP_TIMEOUT_FLAG__)
        else if ((__wfi_id__==FAIL_ID_PEND) 
                || ( (result == __SNT_WUP_TIMEOUT_FLAG__) && (suspected_timeout==1) )
                ){
            __wfi_id__=FAIL_ID_NONE;
            #ifdef DEVEL
                mbus_write_message32(0xE1, (0x5<<28) | iter);
            #endif
            iter++;
            snt_restart_timer();
        }
...
uint32_t snt_read_wup_timer(uint32_t check_restart) {
    ...
    if (!check_restart) __snt_timer_restarted__ = 0;

    if (num_try >= SNT_WUP_READ_MAX_TRY) {
        // Trigger the timeout IRQ
        start_timeout32_check(/*id*/FAIL_ID_MET, /*val*/TIMEOUT_TH);
        while(1);
    }
    else {
        return temp;
