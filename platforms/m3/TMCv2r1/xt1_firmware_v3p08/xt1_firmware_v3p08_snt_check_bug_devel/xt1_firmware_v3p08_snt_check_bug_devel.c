...
void fail_handler(uint32_t id) {
    ...
    if (id==FAIL_ID_SNT) {
        nfc_i2c_byte_write(/*e2*/0, /*addr*/276, /*data*/0xDEAD7777, /*nb*/4);
    }
    else if (id==FAIL_ID_WUP) {
        nfc_i2c_byte_write(/*e2*/0, /*addr*/276, /*data*/0xDEAD8888, /*nb*/4);
    }
    else {
        nfc_i2c_byte_write(/*e2*/0, /*addr*/276, /*data*/(0xDEAD<<16)|id, /*nb*/4);
    }
...
int main(void) {
    ...
    *NVIC_ISER = (0x1 << IRQ_TIMER32);
    ...
    // Read & Store SNT Threhsold and the current Counter value
    if (state == 0) {
        ...
        // Marker
        nfc_i2c_byte_write(/*e2*/0, /*addr*/8, /*data*/0x22222222, /*nb*/4);
        ...
        // Marker
        nfc_i2c_byte_write(/*e2*/0, /*addr*/52, /*data*/0x33333333, /*nb*/4);

        // Synchronous: Read & Store SNT Counter Value (10 times)
        // Byte: 56, 60, ..., 92
        for(gidx=0; gidx<10; gidx++) {
            nfc_i2c_byte_write(/*e2*/0, /*addr*/56+(gidx<<2), /*data*/snt_sync_read_wup_timer(), /*nb*/4);
            delay(10000);
        }

        // Marker
        nfc_i2c_byte_write(/*e2*/0, /*addr*/96, /*data*/0x44444444, /*nb*/4);

        // Measure Temp Sensor (staying in Active)
        // Byte: 100, 104, ..., 136
        for(gidx=0; gidx<10; gidx++) {
            ...
            // Raw value
            data = (*SNT_TARGET_REG_ADDR) & 0xFFFF;

            // Reset & Turn off the temperature sensor
            snt_temp_sensor_power_off();

            nfc_i2c_byte_write(/*e2*/0, /*addr*/100+(gidx<<2), /*data*/data, /*nb*/4);
            delay(10000);
        }

        // Marker
        nfc_i2c_byte_write(/*e2*/0, /*addr*/140, /*data*/0x55555555, /*nb*/4);
    ...
    // Grab the temp result
        ...
        // Store the result
        // Byte: 144, ..., 148, ..., 180
        nfc_i2c_byte_write(/*e2*/0, /*addr*/144+(gidx<<2), /*data*/data, /*nb*/4);
        delay(10000);
    ...
    // Marker
    nfc_i2c_byte_write(/*e2*/0, /*addr*/184, /*data*/0x66666666, /*nb*/4);
    ...
    // Marker
    nfc_i2c_byte_write(/*e2*/0, /*addr*/228, /*data*/0x77777777, /*nb*/4);

    // Synchronous: Read & Store SNT Counter Value (10 times)
    // Byte: 232, 236, ..., 268
    for(gidx=0; gidx<10; gidx++) {
        nfc_i2c_byte_write(/*e2*/0, /*addr*/232+(gidx<<2), /*data*/snt_sync_read_wup_timer(), /*nb*/4);
        delay(10000);
    }

    // Marker
    nfc_i2c_byte_write(/*e2*/0, /*addr*/272, /*data*/0x88888888, /*nb*/4);
