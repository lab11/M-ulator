...
static void operation_init (void) {
    ...
    if (!get_flag(FLAG_ENUMERATED)) {
        ...
        *REG_SYS_CONF =       (0x0 << 9)    // 1'h1     NO_SLEEP_WITH_PEND_IRQ  #If 1, it replaces the sleep message with a selective wakeup message if there is a pending M0 IRQ. If 0, it goes to sleep even when there is a pending M0 IRQ.
        ...
        //--- SRAM Tuning                      Default
        *REG_SRAM0_TUNE =     (0x0  << 9)   // 5'h00    SRAM0_TUNE_ASO_DLY	        
                            | (0x2  << 6)   // 3'h2     SRAM0_TUNE_WL_WIDTH         
                            | (0x1F << 1)   // 5'h10    SRAM0_TUNE_DECODER_DLY      
                            | (0x0  << 0);  // 1'h0     SRAM0_USE_INVERTER_SA     

        *REG_SRAM1_TUNE =     (0x0  << 9)   // 5'h00    SRAM1_TUNE_ASO_DLY	        
                            | (0x2  << 6)   // 3'h2     SRAM1_TUNE_WL_WIDTH         
                            | (0x1F << 1)   // 5'h10    SRAM1_TUNE_DECODER_DLY      
                            | (0x0  << 0);  // 1'h0     SRAM1_USE_INVERTER_SA     
