...
// Version 3.19 (standard-short_interval)
//------------------------
#define HARDWARE_ID 0x01005843  // XT1r1 Hardware ID
#define FIRMWARE_ID 0x0313      // [15:8] Integer part, [7:0]: Non-Integer part
//-------------------------------------------------------------------------------------------
// < UPDATE HISTORY >
...
static void operation_sleep_snt_timer(uint32_t check_snt) {
    ...
    snt_threshold = snt_set_wup_timer(/*auto_reset*/0, /*threshold*/snt_threshold);
    #ifdef DEVEL
...
static void operation_init (void) {
    ...
    #ifdef ENABLE_GOC_CRC
        nfc_i2c_byte_write(/*e2*/0, /*addr*/EEPROM_ADDR_CRC, /*data*/0x0,   /*nb*/4);
...
static uint32_t check_snt_wakeup_near_threshold(void) {
    ...
    return ((snt_read_wup_timer()-snt_threshold)<(snt_freq<<5));
}
...
void fail_handler(uint32_t id) {
    ...
    // Display Patterns
    // -------------------------------------
    //          WUP PMU SNT I2C MET WTH GEN 
    // -------------------------------------
    // PLAY             v   v   v   v    
    // TICK                             
    // LOWBAT                           
    // BSLASH   v   v   v   v   v   v   v
    // SLASH                            
    // PLUS     v   v           v   v    
    // MINUS        v       v       v    
    // -------------------------------------
    ...
    // I2C NAK Failure
        ...
        disp_pattern = DISP_BACKSLASH|DISP_PLAY|DISP_MINUS;
    }
    // Meta-Stability (SNT Timer Reading)
    else if (id==FAIL_ID_MET) {
        disp_pattern = DISP_BACKSLASH|DISP_PLAY|DISP_PLUS;
    }
    // Timeout during SNT Timer Threshold Update
    else if (id==FAIL_ID_WTH) {
        disp_pattern = DISP_BACKSLASH|DISP_PLAY|DISP_PLUS|DISP_MINUS;
    }
