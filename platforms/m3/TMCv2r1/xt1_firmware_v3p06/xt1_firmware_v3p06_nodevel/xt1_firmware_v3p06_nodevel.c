...
// Version 3.06 (no_devel)
//------------------------
#define HARDWARE_ID 0x01005843  // XT1r1 Hardware ID
#define FIRMWARE_ID 0x0306      // [15:8] Integer part, [7:0]: Non-Integer part
//-------------------------------------------------------------------------------------------
// < UPDATE HISTORY >
...
volatile uint32_t eid_seg_in_eeprom;        // EID pattern status saved in EEPROM. This may not consistent with the actual display pattern on the e-ink, due to hibernation, etc.
...
static void eid_update_display(uint32_t seg) {
    ...
    if ((seg != eid_seg_in_eeprom) && !status.hbr_flag) {
