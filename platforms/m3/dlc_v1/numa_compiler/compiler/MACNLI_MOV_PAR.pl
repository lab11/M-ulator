...
use POSIX ;
print "\n";
print "READ MAC_PAR.log FOR MORE INFO OF GENERATED DATA SET\n";
print "\n";

###### Initial memory footprint: MACNLI_MEM_*.data
###### Final result: MOV_MEM_*.goldendata
###### Intermediate result: MACNLI_MEM_*.goldendata

my $OF_LOG = join "", "MACNLI_MOV", "_GEN.log";
open (OUT_LOG, ">$OF_LOG")
	or die "cannot open < ", $OF_LOG, ": $!";

my $INST = join "", "MACNLI_MOV", ".inst";
open (OUT_INST, ">$INST")
	or die "cannot open < ", $OUT_INST, ": $!";
################################################################################
### MACNLI
$inst = "MACNLI";
#my $M = $ARGV[0];
...
$MAC_COL_NUM_ORG == $line[0]
	or die "input file does not match weight file:", $MAC_COL_NUM_ORG, " vs ", $line[0];
...
printf OUT_DNN_PARA sprintf("#define HAS_NLI\n");
printf OUT_DNN_PARA sprintf("extern uint32_t NLI_X[%d][4][11];\n", $total_nli_no);	## inst_no, PE_no, ADDR_no
printf OUT_DNN_PARA sprintf("extern uint32_t NLI_A[%d][4][10];\n", $total_nli_no);	## inst_no, PE_no, ADDR_no
printf OUT_DNN_PARA sprintf("extern uint32_t NLI_B[%d][4][10];\n", $total_nli_no);	## inst_no, PE_no, ADDR_no
...
printf OUT_DNN_NLI sprintf("uint32_t NLI_X[%d][4][11];\n", $total_nli_no);	## inst_no, PE_no, ADDR_no
printf OUT_DNN_NLI sprintf("uint32_t NLI_A[%d][4][10];\n", $total_nli_no);	## inst_no, PE_no, ADDR_no
printf OUT_DNN_NLI sprintf("uint32_t NLI_B[%d][4][10];\n", $total_nli_no);	## inst_no, PE_no, ADDR_no
printf OUT_DNN_NLI sprintf("\n");
printf OUT_DNN_NLI sprintf("void set_nli_parameters() {\n"); # bracket closed at the end of the file	
...
for ($i = 0; $i < $NLI_SEGMENT + 1; $i++) { 
  printf OUT_DNN_NLI sprintf("NLI_X[%d][0][%d] = 0x%4.4x;\n", $inst_no, $i, $PE0_DATA_X[$i] & 0xFFFF);
  printf OUT_DNN_NLI sprintf("NLI_X[%d][1][%d] = 0x%4.4x;\n", $inst_no, $i, $PE1_DATA_X[$i] & 0xFFFF);
  printf OUT_DNN_NLI sprintf("NLI_X[%d][2][%d] = 0x%4.4x;\n", $inst_no, $i, $PE2_DATA_X[$i] & 0xFFFF);
  printf OUT_DNN_NLI sprintf("NLI_X[%d][3][%d] = 0x%4.4x;\n", $inst_no, $i, $PE3_DATA_X[$i] & 0xFFFF);
}
for ($i = 0; $i < $NLI_SEGMENT; $i++) { 
  printf OUT_DNN_NLI sprintf("NLI_A[%d][0][%d] = 0x%4.4x;\n", $inst_no, $i, $PE0_DATA_A[$i] & 0xFFFF);
  printf OUT_DNN_NLI sprintf("NLI_A[%d][1][%d] = 0x%4.4x;\n", $inst_no, $i, $PE1_DATA_A[$i] & 0xFFFF);
  printf OUT_DNN_NLI sprintf("NLI_A[%d][2][%d] = 0x%4.4x;\n", $inst_no, $i, $PE2_DATA_A[$i] & 0xFFFF);
  printf OUT_DNN_NLI sprintf("NLI_A[%d][3][%d] = 0x%4.4x;\n", $inst_no, $i, $PE3_DATA_A[$i] & 0xFFFF);
}
for ($i = 0; $i < $NLI_SEGMENT; $i++) { 
  printf OUT_DNN_NLI sprintf("NLI_B[%d][0][%d] = 0x%8.8x;\n", $inst_no, $i, $PE0_DATA_B[$i] & 0xFFFFFFFF);
  printf OUT_DNN_NLI sprintf("NLI_B[%d][1][%d] = 0x%8.8x;\n", $inst_no, $i, $PE1_DATA_B[$i] & 0xFFFFFFFF);
  printf OUT_DNN_NLI sprintf("NLI_B[%d][2][%d] = 0x%8.8x;\n", $inst_no, $i, $PE2_DATA_B[$i] & 0xFFFFFFFF);
  printf OUT_DNN_NLI sprintf("NLI_B[%d][3][%d] = 0x%8.8x;\n", $inst_no, $i, $PE3_DATA_B[$i] & 0xFFFFFFFF);
}
