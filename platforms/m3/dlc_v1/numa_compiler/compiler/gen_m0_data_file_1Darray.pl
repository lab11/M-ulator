...
# version: generate the data in a 1-D array format, skip "xxxxxxxxxxxxxxxxxxxxxxxx"
...
for ($q = 1; $q <=4; $q++){
	...
	for ($r = 0; $r < $NUM_LINES; $r++){
		...
		if($data ne "xxxxxxxxxxxxxxxxxxxxxxxx") {	#bug: perl string comparison can't use == or !=
			...
			$text = join "", "PE_MEM[", $array_size++, "] = ", $LINES, ";\n"; #bug: must have "", immediately after join
			printf DATA $text;
			$text = join "", "PE_MEM[", $array_size++, "] = 0x", $data_1, ";\n";
			printf DATA $text;
			$text = join "", "PE_MEM[", $array_size++, "] = 0x", $data_2, ";\n";
			printf DATA $text;
			$text = join "", "PE_MEM[", $array_size++, "] = 0x", $data_3, ";\n";
			printf DATA $text;
			#$array_size++;
		}
		$LINES++;
	...
	close(M);
	open(M, ">$DF");
	$text = join "", "uint32_t PE_MEM[", $array_size, "];\n\n";
	print M $text; 
