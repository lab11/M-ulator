#!/usr/bin/env perl
package MAC_gen_weight_face_M2;
#use strict;
use warnings;
use POSIX ;
use Exporter;
...
our @ISA=qw(Exporter);
our @EXPORT=qw(MAC_gen_weight_face_M2);
...
my $PE3_INST		= 1;
if (defined $argv[6]) {
  $PE0_INST	= (($argv[6] & 0b0001) != 0);
  $PE1_INST	= (($argv[6] & 0b0010) != 0);
  $PE2_INST	= (($argv[6] & 0b0100) != 0);
  $PE3_INST	= (($argv[6] & 0b1000) != 0);
}
#
my $PE0_MAC_OFFSET	= 1;
my $PE1_MAC_OFFSET	= 1;
...
my $PE3_MAC_OFFSET	= 1;
if (defined $argv[7]) {
  $PE0_MAC_OFFSET	= (($argv[7] & 0b0001) != 0);
  $PE1_MAC_OFFSET	= (($argv[7] & 0b0010) != 0);
  $PE2_MAC_OFFSET	= (($argv[7] & 0b0100) != 0);
  $PE3_MAC_OFFSET	= (($argv[7] & 0b1000) != 0);
}
...
my $ROW_NUM_MIN_FACTOR = 12;
#my $COL_NUM_MIN_FACTOR = 8; ## same as $PE_I_DATApWORD
my $COL_NUM_MIN_FACTOR = 6; ## same as $PE_I_DATApWORD
...
system ($^X,"gen_m0_data_file_mem.pl", $OF0);
system ($^X,"gen_m0_data_file_mem.pl", $OF1);
system ($^X,"gen_m0_data_file_mem.pl", $OF2);
system ($^X,"gen_m0_data_file_mem.pl", $OF3);
##########
} ## end FFT_GEN_WEIGHT
