...
if len(sys.argv) > 2:   ## todo: not useful 
  neurons[0] = int(sys.argv[1])
  for i in range(1, len(neurons)):
    neurons[i] = int(sys.argv[2])
if len(sys.argv) > 6:
...
if parallel:
  ...
  check_overlap(neurons[0], neurons[1] / 4, i_exp, o_exp, w_exp, to_exp)    ## TODO: just checked one layer
  ### check if the assignment can fit in one PE w/o bank overlap
else:
  check_overlap(neurons[0], neurons[1], i_exp, o_exp, w_exp, to_exp)        ## TODO: just checked one layer
  ### check if the assignment can fit in one PE w/o bank overlap
