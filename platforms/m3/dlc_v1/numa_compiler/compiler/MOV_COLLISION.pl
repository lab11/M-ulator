...
##########
######## configurations
my $input_size = 10; ## number of word lines to copy; careful if larger than 32
...
#
 ...
 my $MOV0_PE0_dest_addr = $MEM_H4B1_START + $input_size + 2; ## also MOV1_PE0_source addr
 my $MOV1_PE0_dest_addr = $MEM_H4B1_START + 2* ($input_size + 2); ## also MOV2_PE0_source addr
 my $MOV2_PE0_dest_addr = $MEM_H4B1_START + 3* ($input_size + 2);

 my $MOV0_PE1_source_addr = $MEM_H4B1_START;
 my $MOV0_PE1_dest_addr = $MEM_H4B2_START; ## also MOV1_PE2_source addr
 my $MOV1_PE2_dest_addr = $MEM_H4B3_START; ## also MOV2_PE3_source addr
 my $MOV2_PE3_dest_addr = $MEM_H4B4_START;
#########
######## init 
my @PE0_MEM_LINES 	= ();
my @PE1_MEM_LINES 	= ();
...
$PE1_O_MEM_START = $MOV0_PE1_dest_addr;
my $INPUT_MEM_FILE = "MOV_INIT";
@argument = ("MOV_0", $INPUT_MEM_FILE . "_MEM", 0b0011, 0, 0, 0, 0,  $PE0_I_MEM_START, $PE0_O_MEM_START, $input_size, $PE1_I_MEM_START, $PE1_O_MEM_START, $input_size, 0, 0, 0, 0, 0, 0); 
MOV_sub(@argument);

##### MOV1
$PE0_I_MEM_START = $MOV0_PE0_dest_addr;
$PE2_I_MEM_START = $MOV0_PE0_source_addr;
$PE0_O_MEM_START = $MOV1_PE0_dest_addr;
$PE2_O_MEM_START = $MOV1_PE2_dest_addr;
$INPUT_MEM_FILE = "MOV_0";
@argument = ("MOV_1", $INPUT_MEM_FILE . "_MEM", 0b0101, 0, 0, 0, 0,  $PE0_I_MEM_START, $PE0_O_MEM_START, $input_size, 0, 0, 0, $PE2_I_MEM_START, $PE2_O_MEM_START, $input_size, 0, 0, 0); 
#for ($ii = 0; $ii < 4; $ii++) { 
...
##### MOV2
$PE0_I_MEM_START = $MOV1_PE0_dest_addr;
$PE3_I_MEM_START = $MOV0_PE0_source_addr;
$PE0_O_MEM_START = $MOV2_PE0_dest_addr;
$PE3_O_MEM_START = $MOV2_PE3_dest_addr;
$INPUT_MEM_FILE = "MOV_1";
@argument = ("MOV_2", $INPUT_MEM_FILE . "_MEM", 0b1001, 0, 0, 0, 0,  $PE0_I_MEM_START, $PE0_O_MEM_START, $input_size, 0, 0, 0, 0, 0, 0, $PE3_I_MEM_START, $PE3_O_MEM_START, $input_size); 
#for ($ii = 0; $ii < 4; $ii++) { 
#  $original = join "", $INPUT_MEM_FILE, "_MEM_", $ii, ".goldendata";
#  $copy = join "", $INPUT_MEM_FILE, "_MEM_", $ii, ".data";
#  system ("cp", $original, $copy); 
