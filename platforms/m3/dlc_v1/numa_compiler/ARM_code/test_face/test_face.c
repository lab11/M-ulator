...
//      delay(3);
    ...
    // working sequence
      ...
      // b.) start MAC 
		write_instruction(0, 0, 0);   // write MAC
	  ...
	  *DNN_RE_INIT_3 = 0x00000000;
	  ...
	  //delay(3);
		write_instruction(1, 0, 0);   // write MAC
		start_pe_inst(0b0001);              // start MAC
		inst_no++;
		while (check_if_pe_finish(0b0001) != 1) { delay(5);}
	...
	out1= (y0 & 0xffff);
	out2= (y1<<8) | ((y0>>16) & 0x00ff);
	int face = (out1>out2)&0x00000001;
	//int face2 = (out1<out2)&0x00000001;
	a[0]=face;
	a[1]=out1;
	a[2]=out2;
	a[3]=*DNN_PG_0;
	...
	// mbus_write_message32(0x14,out1);
	// delay(7000);
	// mbus_write_message32(0x14,out2);
	// delay(7000);
	// mbus_write_message32(0x14,face);
	// delay(7000);
