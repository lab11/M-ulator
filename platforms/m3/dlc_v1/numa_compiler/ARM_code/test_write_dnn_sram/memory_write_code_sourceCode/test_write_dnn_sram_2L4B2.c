...
int main() {
	...
	*DNN_PG_3 = 0x000007ff;
	...
	{  
	  	*DNN_DATA_2_3   = (*(WRITE_SRAM_ADDR+3*i) >> 8) & 0x00ffffff;
	  	*DNN_DATA_2_2   = (*(WRITE_SRAM_ADDR+3*i) << 16) + ((*(WRITE_SRAM_ADDR+3*i+1) >> 16) & 0x0000ffff);
	  	*DNN_DATA_2_1   = ((*(WRITE_SRAM_ADDR+3*i+1) & 0xffff) << 8) + ((*(WRITE_SRAM_ADDR+3*i+2) >> 24) & 0x000000ff);
	  	*DNN_DATA_2_0   = *(WRITE_SRAM_ADDR+3*i+2) & 0xffffff;
	  	*DNN_CTRL_2 = (OUT_ADDR + i << 2) + 0b10;			// set CPU_ARB_DATA_WR to 1
	}
	// if(array_size > 1){
