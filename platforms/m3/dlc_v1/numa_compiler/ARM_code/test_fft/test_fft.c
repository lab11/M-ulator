...
//    if (*REG_CHIP_ID != 0x1234) while(1);
		...
		uint16_t inst_no;
		set_dnn_insts();
		set_nli_parameters();   // has to be here even not used: otherwise cannot compile
...
*/
    ...
    // working sequence
      ...
      inst_no = 0;
			...
			// b.) FFT
      // buffer 1
		  *PE_INST |= (0b1011 << 12);		// set all INST_SEL to buffer 1
      write_instruction(inst_no, 0, 1);
      //write_instruction(inst_no, 2, 1);
      // buffer 0
//		  *PE_INST |= (0b0000 << 12);		// set all INST_SEL to buffer 0
//      write_instruction(inst_no, 0, 0);
      write_instruction(inst_no, 2, 0);

			start_pe_inst(0b0101);
			inst_no++;
			wait_until_pe_start(0b0101);
      clock_gate();

			// d.) finish
			if (~check_if_pe_finish(0b0101)) { fail (0x0, 0x0); }
      signal_done();
      //clock_gate();
  		// done
		//////////////////////////////////////////////////////
