  ...
  while (dft_size < 8) {
    for (j = 0; j < FFT_SIZE; j += (dft_size << 1)) {
      ...
      for (loc = 0; loc < dft_size; loc++) {
        int tempr = fft_input_r[j + loc + dft_size] - fft_input_i[j + loc + dft_size];
        int tempi = fft_input_r[j + loc + dft_size] - fft_input_i[j + loc + dft_size];
...
// write PE fft input
  ...
  write_dnn_sram_6_fft_input(0x2000, (int32_t*)fft_input_r, (int32_t*)fft_input_i);
  *DNN_NLI_A_0 = 6;
  read_dnn_sram_6_fft_output(0x2000, (int32_t*)fft_input_r, (int32_t*)fft_input_i);
  *DNN_NLI_A_0 = 7;
	/////////////////////////////////////////
  // fft 
  /*
