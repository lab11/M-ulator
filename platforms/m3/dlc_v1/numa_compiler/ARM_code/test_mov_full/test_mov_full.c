...
//      delay(3);
    ...
    // working sequence
      ...
      write_instruction(inst_no, 1, 0); 
			start_pe_inst(0b0011);
			inst_no++;
			while (check_if_pe_finish(0b0011) != 1) { delay(5);}
      ...
      write_instruction(inst_no, 2, 0); 
			...
			inst_no++;
			while (check_if_pe_finish(0b0101) != 1) { delay(5);}

      // MOV2
      write_instruction(inst_no, 0, 0); 
      write_instruction(inst_no, 3, 0); 
			start_pe_inst(0b1001);
			inst_no++;
			while (check_if_pe_finish(0b1001) != 1) { delay(5);}
