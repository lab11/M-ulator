...
*/
    ...
    // working sequence
      ...
      inst_no = 0;
			...
			// b.) FFT
		  *PE_INST |= (0b1111 << 12);		// set all INST_SEL to buffer 1
      write_instruction(inst_no, 1, 1);
      write_instruction(inst_no, 3, 1);
      write_nli(inst_no, 1);
      write_nli(inst_no, 3);
			start_pe_inst(0b1010);
			inst_no++;
			wait_until_pe_start(0b1010);
      clock_gate();

			// d.) finish
			if (~check_if_pe_finish(0b1010)) { fail (0x0, 0x0); }
      signal_done();
      //clock_gate();
  		// done
		//////////////////////////////////////////////////////
