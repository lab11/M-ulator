...
int main() {
    ...
    *DNN_RAND_3 = 1;
	...
	{  
		...
		*DNN_DATA_0_0   = (3*i+2) & 0xffffff;																				// x0 last 24 bits
		*DNN_DATA_0_1   = (( (3*i+1) & 0xffff) << 8) + (( (3*i+2) >> 24) & 0x000000ff);			// x1 last 16 bits + x0 first 8 bits. the last AND op is to avoid x0 being signed 
		*DNN_DATA_0_2   = (( (3*i) & 0xff) << 16) + (( (3*i+1) >> 16) & 0x0000ffff);	  	// x2 last 8 bits + x1 first 16 bits 
		*DNN_DATA_0_3   = ( (3*i) >> 8) & 0x00ffffff; 															// x2 first 24 bits
		
	  	// *DNN_DATA_0_3   = data;
	  	// *DNN_DATA_0_2   = data;
	  	// *DNN_DATA_0_1   = data;
	  	// *DNN_DATA_0_0   = data;
	  	*DNN_CTRL_0 	= (i << 2) + 0b10;			// set CPU_ARB_DATA_WR to 1
		// data = ~data;
	}
	for(i=0;i<WRITE_SRAM_SIZE;i++)
	{  

		*DNN_DATA_1_0   = (3*i+2) & 0xffffff;																				// x0 last 24 bits
		*DNN_DATA_1_1   = (( (3*i+1) & 0xffff) << 8) + (( (3*i+2) >> 24) & 0x000000ff);			// x1 last 16 bits + x0 first 8 bits. the last AND op is to avoid x0 being signed 
		*DNN_DATA_1_2   = (( (3*i) & 0xff) << 16) + (( (3*i+1) >> 16) & 0x0000ffff);	  	// x2 last 8 bits + x1 first 16 bits 
		*DNN_DATA_1_3   = ( (3*i) >> 8) & 0x00ffffff; 															// x2 first 24 bits
	  	// *DNN_DATA_1_3   = data;
	  	// *DNN_DATA_1_2   = data;
	  	// *DNN_DATA_1_1   = data;
	  	// *DNN_DATA_1_0   = data;
	  	*DNN_CTRL_1 	= (i << 2) + 0b10;			// set CPU_ARB_DATA_WR to 1
		// data = ~data;
	}
	for(i=0;i<WRITE_SRAM_SIZE;i++)
	{  
		
		*DNN_DATA_2_0   = (3*i+2) & 0xffffff;																				// x0 last 24 bits
		*DNN_DATA_2_1   = (( (3*i+1) & 0xffff) << 8) + (( (3*i+2) >> 24) & 0x000000ff);			// x1 last 16 bits + x0 first 8 bits. the last AND op is to avoid x0 being signed 
		*DNN_DATA_2_2   = (( (3*i) & 0xff) << 16) + (( (3*i+1) >> 16) & 0x0000ffff);	  	// x2 last 8 bits + x1 first 16 bits 
		*DNN_DATA_2_3   = ( (3*i) >> 8) & 0x00ffffff; 															// x2 first 24 bits
	  
	  	// *DNN_DATA_2_3   = data;
	  	// *DNN_DATA_2_2   = data;
	  	// *DNN_DATA_2_1   = data;
	  	// *DNN_DATA_2_0   = data;
	  	*DNN_CTRL_2 	= (i << 2) + 0b10;			// set CPU_ARB_DATA_WR to 1
		// data = ~data;
	}
	for(i=0;i<WRITE_SRAM_SIZE;i++)
	{  

		*DNN_DATA_3_0   = (3*i+2) & 0xffffff;																				// x0 last 24 bits
		*DNN_DATA_3_1   = (( (3*i+1) & 0xffff) << 8) + (( (3*i+2) >> 24) & 0x000000ff);			// x1 last 16 bits + x0 first 8 bits. the last AND op is to avoid x0 being signed 
		*DNN_DATA_3_2   = (( (3*i) & 0xff) << 16) + (( (3*i+1) >> 16) & 0x0000ffff);	  	// x2 last 8 bits + x1 first 16 bits 
		*DNN_DATA_3_3   = ( (3*i) >> 8) & 0x00ffffff; 															// x2 first 24 bits
	  	// *DNN_DATA_3_3   = data;
	  	// *DNN_DATA_3_2   = data;
	  	// *DNN_DATA_3_1   = data;
	  	// *DNN_DATA_3_0   = data;
	  	*DNN_CTRL_3 	= (i << 2) + 0b10;			// set CPU_ARB_DATA_WR to 1
		// data = ~data;
	}
