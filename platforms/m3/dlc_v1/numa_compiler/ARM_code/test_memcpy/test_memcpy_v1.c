...
//    if (*REG_CHIP_ID != 0x1234) while(1);
    ...
    // working sequence
      ...
      for (ii = 0; ii < ARR_LENGTH; ii++)
        arr[ii] = ii * 1;
			for (addr = 0x0000; addr < 0x2000; addr += 5760 / (ARR_LENGTH / 3))
			  write_dnn_sram(addr, arr, ARR_LENGTH / 3);
      //
      for (ii = 0; ii < ARR_LENGTH; ii++)
        arr[ii] = ii * -1;
			for (addr = 0x2000; addr < 0x4000; addr += 5760 / (ARR_LENGTH / 3))
			  write_dnn_sram(addr, arr, ARR_LENGTH / 3);
      //
      for (ii = 0; ii < ARR_LENGTH; ii++)
        arr[ii] = ii * 65538;   // 2^16
			for (addr = 0x4000; addr < 0x6000; addr += 5760 / (ARR_LENGTH / 3))
			  write_dnn_sram(addr, arr, ARR_LENGTH / 3);
      //
      for (ii = 0; ii < ARR_LENGTH; ii++)
        arr[ii] = ii * -65538;    // -2^16
			for (addr = 0x6000; addr < 0x8000; addr += 5760 / (ARR_LENGTH / 3))
			  write_dnn_sram(addr, arr, ARR_LENGTH / 3);
      ...
      for (ii = 0; ii < ARR_LENGTH; ii++)
        arr[ii] = ii * 1;
			for (addr = 0x0000; addr < 0x2000; addr += 5760 / (ARR_LENGTH / 3)) {
			  read_dnn_sram(addr, ARR, ARR_LENGTH / 3);
				if (!check_array(arr, ARR, ARR_LENGTH / 3)) report_fail();
      }
      //
      for (ii = 0; ii < ARR_LENGTH; ii++)
        arr[ii] = ii * -1;
