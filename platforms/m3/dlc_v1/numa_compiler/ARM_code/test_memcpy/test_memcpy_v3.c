...
//      delay(3);
    ...
    ////
    // test
    int32_t input1[12] = {1, 2, 3,   1, 2, -3,   1, -2, 3,   1, -2, -3};
    int32_t input2[12] = {-1, 2, 3,  -1, 2, -3,  -1, -2, 3,  -1, -2, -3};
                            
//    int32_t result[24];
    int32_t result1[12];
    int32_t result2[12];
    uint8_t i;
    write_dnn_sram(2000, input1, 4); 
    write_dnn_sram(2010, input2, 4); 
    for (i = 0; i < 12; i++) { 
      *DNN_NLI_A_0 = input1[i]; 
      *DNN_NLI_X_0 = input2[i];
    }
//    read_dnn_sram(1000, result, 8); 
    read_dnn_sram(2000, result1, 4); 
    read_dnn_sram(2010, result2, 4); 
    for (i = 0; i < 12; i++) { 
///      *DNN_NLI_A_0 = result[i]; 
///      *DNN_NLI_X_0 = result[i+12];
      *DNN_NLI_A_0 = result1[i]; 
      *DNN_NLI_X_0 = result2[i];
    }
    signal_done();
    delay(7000);
    ///
