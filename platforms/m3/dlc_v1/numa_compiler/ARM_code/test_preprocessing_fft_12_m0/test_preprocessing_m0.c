...
#define FFT_SIZE 512
#define FFT_PREC 12     // essentially hard-coded: other precision won't work
#define DFT_SIZE 4
#define FFT_NUM 64
#define FFT_NUM_BITS 7  // log2(FFT_NUM) + 1
#define FFT_INST 7      // total number of FFT instructions
...
// prepare dft for fft
  ...
  for (loc = 0; loc < FFT_NUM * 2; loc++) {
    // DFT_SIZE = 4 only optimization
    base = bit_reverse_7_table[loc]; 
    for (j = 0; j < DFT_SIZE; j++) { 
//      fft_input_r[loc * DFT_SIZE + j] = raw_fft_input[base + j * FFT_NUM * 2];
      // DFT_SIZE = 4 only optimization
      fft_input_r[(loc << 2) + j] = raw_fft_input[base + (j << 7)];
    }
  }
...
// dft
  ...
  for (base = 0; base < FFT_SIZE; base += DFT_SIZE) {
    tempr[0] = fft_input_r[base+0] + fft_input_r[base+2];
    tempr[1] = fft_input_r[base+1] + fft_input_r[base+3];
    fft_input_r[base+0] = tempr[0] + tempr[1];
    fft_input_i[base+0] = 0;
    fft_input_r[base+2] = tempr[0] - tempr[1]; 
    fft_input_i[base+2] = 0;
    tempr[2] = fft_input_r[base+0] - fft_input_r[base+2];
    tempr[3] = fft_input_r[base+1] - fft_input_r[base+3];
    fft_input_r[base+1] = tempr[2];
    fft_input_i[base+1] = -tempr[3];
    fft_input_r[base+3] = tempr[2];
    fft_input_i[base+3] = tempr[3];
    /*
    for (j = 0; j < DFT_SIZE; j++) {
      tempr[j] = 0;
      tempi[j] = 0;
