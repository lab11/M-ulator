...
#define READ_SRAM_ADDR        ((volatile uint32_t *) 0x00002000)
#define READ_ADDR_RANG        1706
// #define READ_ADDR_RANG        1365
...
int main() {
	...
	// data=0xAAAAAAAA;
	data=0x55555555;
	for(i=0;i<READ_ADDR_RANG;i++)
	{	
		// face1 = ( data == *(READ_SRAM_ADDR+3*i  ) );
		// face2 = ( data == *(READ_SRAM_ADDR+3*i+1) );
		// face3 = ( data == *(READ_SRAM_ADDR+3*i+2) );

		if( data != *(READ_SRAM_ADDR+3*i  ) ){
			error_cnt++;
			error_addr = 2048 + 3*i;
			error_bit = *(READ_SRAM_ADDR+3*i  )^data;
			error_msg();
			delay(MBUS_DELAY);
		}
		if( data != *(READ_SRAM_ADDR+3*i+1) ){
			error_cnt++;
			error_addr = 2048 + 3*i + 1;
			error_bit = *(READ_SRAM_ADDR+3*i+1)^data;
			error_msg();
			delay(MBUS_DELAY);
		}
		if( data != *(READ_SRAM_ADDR+3*i+2) ){
			error_cnt++;
			error_addr = 2048 + 3*i + 2;
			error_bit = *(READ_SRAM_ADDR+3*i+2)^data;
			error_msg();
			delay(MBUS_DELAY);
		}
		data=~data;
