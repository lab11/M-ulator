    ...
    *DNN_RAND_3 = 1;
	...
	for(i=0;i<100;i++)
	{
		*DNN_CTRL_0 = (short_addr << 2) + 0b01;	// set CPU_ARB_DATA_RD to 1
		...
		// face = ( out1 == *(READ_SRAM_ADDR+3*i  ) ) & face;
		// face = ( out2 == *(READ_SRAM_ADDR+3*i+1) ) & face;
		// face = ( out3 == *(READ_SRAM_ADDR+3*i+2) ) & face;
		
		face1 = ( out1 == *(READ_SRAM_ADDR+3*i  ) );
		face2 = ( out2 == *(READ_SRAM_ADDR+3*i+1) );
		face3 = ( out3 == *(READ_SRAM_ADDR+3*i+2) );
	...
	// mbus_write_message(0x14,READ_SRAM_ADDR,1920);
	mbus_write_message(0x14,READ_SRAM_ADDR,12);
	// mbus_write_message(0x14,READ_SRAM_ADDR,300);
