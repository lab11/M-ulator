...
//        tempi[j] += input_r[k+base] * dft_i[j * dft_num + k] / shift; 
        tempr[j] += input_r[k+base] * cos(-2 * j * k * PI / dft_num);    // input_i is 0, no need to consider 
        tempi[j] += input_r[k+base] * sin(-2 * j * k * PI / dft_num); 
...
///////////////////////////////////////////
void main() {
  unsigned int fft_num = FFT_SIZE / 16;    // ** depends on precision **
//  unsigned int fft_zero_pad_size = FFT_SIZE * 2 * 3 / 2;
  unsigned int fft_zero_pad_size = FFT_SIZE;
//  fft_num = 2;
  ...
  int16_t window_parameter[FFT_SIZE];   
//  fft_iit(twidr_all, twidi_all);
  ...
  init_input(raw_fft_input, FFT_SIZE);
  ...
  prepare_dft_for_fft(FFT_SIZE, FFT_SIZE, raw_fft_input, input_r_dft, fft_num, fft_num_bits, dft_size);
  /////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////
  ...
  uint16_t loc;
  fft_num = FFT_SIZE / 2;
  fft_num_bits = log_2(fft_num) + 1;
  dft_size = FFT_SIZE / fft_num / 2;
  for (loc = 0; loc < fft_num * 2; loc++) {
    base = int_bit_reverse(loc, fft_num_bits); // TODO: use a lookup table
//    printf("base:%d loc:%d\n", base, loc);
    for (int j = 0; j < FFT_SIZE / fft_num / 2; j++) { 
      input_r_dft[loc * dft_size + j] = raw_fft_input[base + j * fft_num * 2];
    }
  }
  printf("## bit reversed inputs for fft ##\n"); 
  for (int i=0; i<FFT_SIZE; i++) {
    printf("%d\t", (int)input_r_dft[i]);
  }
  printf("\n"); 
  ...
  while (dft_size < 8) {    // depends on precision
    printf("dft_size:%d\n", dft_size);
...
//        printf("%.2f\t%.2f\n", twidr, twidi); 
        /// debug
        printf("r[idx]:%d\tr[idx+dft_size]:%d\ti[idx]:%d\ti[idx+dft_size]%d\n", input_r_dft[idx], input_r_dft[idx+dft_size], input_i_dft[idx], input_i_dft[idx+dft_size]);
        float tempr = input_r_dft[i+idx+dft_size] * twidr - input_i_dft[i+idx+dft_size] * twidi;
        float tempi = input_r_dft[i+idx+dft_size] * twidi + input_i_dft[i+idx+dft_size] * twidr;
        printf("i+idx:%d\tdft_size:%d\n", i+idx, dft_size);
        printf("idx:%d\ttwi_idx:%d\ttwidr:%.2f\ttwidi:%.2f\n", idx, twi_idx, twidr, twidi);
        printf("tempr:%.2f\ttempi:%.2f\n", tempr, tempi);
        input_r_dft[i+idx+dft_size] = input_r_dft[i+idx] - tempr; // has to compute this first
        input_r_dft[i+idx] += tempr;
        input_i_dft[i+idx+dft_size] = input_i_dft[i+idx] - tempi;
        input_i_dft[i+idx] += tempi;
        printf("r[idx]:%d\tr[idx+dft_size]:%d\ti[idx]:%d\ti[idx+dft_size]%d\n", input_r_dft[idx], input_r_dft[idx+dft_size], input_i_dft[idx], input_i_dft[idx+dft_size]);
        printf("\n");
    ...
    printf("## fft %d: ##\n", dft_size); 
    for (int i=0; i<FFT_SIZE; i++) {
      printf("%d\t%d\t%d\n", i, input_r_dft[i], input_i_dft[i]); 
    }
    printf("\n");
    /// debug
    dft_size *= 2;
  }
  printf("## fft (on m0) outputs ##\n"); 
  for (int i=0; i<FFT_SIZE; i++) {
    printf("%d\t", input_r_dft[i]);
    printf("%d\t", input_i_dft[i]);
  }
