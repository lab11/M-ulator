//*******************************************************************
//Author: Inhee Lee
//Description: Biofuel-Cell-Powered Temperature Sensing System
//            v1: Modified from 'Pstack_Ondemand_v2.0'
//            v2: Add a single tone test, Pull-down SOL when VBAT > 2.3V 
//*******************************************************************
#include "PRCv17.h"
#include "PRCv17_RF.h"
#include "mbus.h"
...
#define WAKEUP_PERIOD_RADIO_INIT 0x10 // About 2 sec (PRCv17)
...
inline static void pmu_set_clk_init(){
    ...
    pmu_reg_write(0x05, //default 12'h000
        ...
        | (0x56)        // Binary converter's conversion ratio (7'h00)
    ));
    pmu_reg_write(0x05, //default 12'h000
...
inline static void pmu_adc_read_latest(){
    ...
    if (read_data_batadc<PMU_ADC_2P2_VAL){
        read_data_batadc_diff = 0;
    }else{
        read_data_batadc_diff = read_data_batadc - PMU_ADC_2P2_VAL;
    }

}

inline static void pmu_battery_overcharge_protection(){
...
static void operation_init(void){
    ...
    *REG_CLKGEN_TUNE = prcv17_r0B.as_int;
    ...
    prcv17_r0D.SRAM_USE_INVERTER_SA = 0; // Default 0x2, 4 bits
...
*/
    ...
    // Adjust C
    srrv4_r07.RO_MOM = 0xF;
    srrv4_r07.RO_MIM = 0xF;
    mbus_remote_register_write(SRR_ADDR,0x07,srrv4_r07.as_int);

    // Current Limter set-up 
    srrv4_r00.SRR_CL_CTRL = 16; //16; //Set CL 1-finite 16-20uA

    // TX Setup Carrier Freq
    srrv4_r00.SRR_TRX_CAP_ANTP_TUNE = 0x1FFF;  //ANT CAP 14b unary 830.5 MHz
    ...
    mbus_remote_register_write(SRR_ADDR,0x00,srrv4_r00.as_int);
    srrv4_r01.SRR_TRX_CAP_ANTN_TUNE = 0x1FFF; //ANT CAP 14b unary 830.5 MHz
    ...
    pmu_setting_state = PMU_25C;
    PMU_10C_threshold_snt = 600; // Around 10C
    PMU_20C_threshold_snt = 1000; // Around 20C
    PMU_55C_threshold_snt = 4000; // Around 55C
    PMU_85C_threshold_snt = 9000; // Around 85C
    ...
    if (Tstack_state == TSTK_IDLE){
        ...
        *TIMER32_GO = 0;
        ...
        if (meas_count < NUM_TEMP_MEAS){    
            ...
            if (temp_storage_latest > PMU_85C_threshold_snt){
                if (pmu_setting_state != PMU_85C){
...
int main() {
    ...
    *NVIC_ISER = (1 << IRQ_WAKEUP) | (1 << IRQ_GOCEP) | (1 << IRQ_TIMER32) | (1 << IRQ_REG0)| (1 << IRQ_REG1)| (1 << IRQ_REG2)| (1 << IRQ_REG3);
    ...
    pmu_adc_read_latest();
    pmu_battery_overcharge_protection();
	...
	// Turn on cont mode w different SRR_TX_BIAS_TUNE

		disable_timerwd();
		*MBCWD_RESET = 1;
        
		srrv4_r03.SRR_TRX_ISOLATEN = 0;
		mbus_remote_register_write(SRR_ADDR,0x03,srrv4_r03.as_int);

		srrv4_r00.SRR_CL_CTRL = 16;
		mbus_remote_register_write(SRR_ADDR,0x00,srrv4_r00.as_int);


		srrv4_r00.SRR_CL_EN = 1;
		mbus_remote_register_write(SRR_ADDR,0x00,srrv4_r00.as_int);

    	delay(MBUS_DELAY*50); //800ms pulses
		
		srrv4_r00.SRR_CL_CTRL = 1;
		mbus_remote_register_write(SRR_ADDR,0x00,srrv4_r00.as_int);

		srrv4_r02.SRR_TX_EN_OW = 0x1;
		mbus_remote_register_write(SRR_ADDR,0x02,srrv4_r02.as_int);
    ...
    // Tune SNT Timer R for TC
        ...
        if (wakeup_data_field_0 == 0){
            ...
            PMU_ADC_2P2_VAL = read_data_batadc;
        }else{
            PMU_ADC_2P2_VAL = wakeup_data_field_0;
        }
        ...
        PMU_20C_threshold_snt = wakeup_data & 0xFFFF; // Around 20C
        // Go to sleep without timer
        operation_sleep_notimer();

    }else if(wakeup_data_header == 0x1C){
        PMU_55C_threshold_snt = wakeup_data & 0xFFFFFF; // Around 55C
        // Go to sleep without timer
        operation_sleep_notimer();

    }else if(wakeup_data_header == 0x1D){
        PMU_85C_threshold_snt = wakeup_data & 0xFFFFFF; // Around 85C
        // Go to sleep without timer
        operation_sleep_notimer();
...
*/        
    ...
    // Memory Tuning
        *REG_SRAM_TUNE = wakeup_data & 0xFFFF;
        operation_sleep_notimer();
