...
//			v4.2_HRV: Adding HRV light count, logging vbat adc, taking out high and low temp operations
//			v4.2a_HRV: TX HRV light count during radio tx mode
//			v4.2b_HRV: Change default HRV ratio and make it configurable 0x19
//*******************************************************************
#include "PRCv17.h"
#include "PRCv17_RF.h"
#include "mbus.h"
...
static void operation_init(void){
	...
	*REG_SRAM_TUNE = prcv17_r0D.as_int;
    ...
    // Harvester Settings --------------------------------------
	hrv_light_reset();
	mbus_remote_register_write(HRV_ADDR,0x00,15); // HRV_TOP_CONV_RATIO(0~15 >> 9x~23x); default: 14
	...
	if (Tstack_state == TSTK_IDLE){
		...
		*TIMER32_GO = 0;
		...
		if (meas_count < NUM_TEMP_MEAS){	
			...
			// FIXME: need to store light and batt

			// Store results in MEMv1; unless memory is full
			if ((temp_storage_count + 4) < TEMP_STORAGE_SIZE){
				...
				mem_write_data = (temp_storage_latest & 0xFFFF) | (read_data_batadc_diff<<16);
				mbus_copy_mem_from_local_to_remote_bulk(MEM_ADDR, (uint32_t*)((temp_storage_count>>1)<<2), (uint32_t*)&mem_write_data, 0);
				mem_write_data = hrv_light_count;
				mbus_copy_mem_from_local_to_remote_bulk(MEM_ADDR, (uint32_t*)(((temp_storage_count>>1)+1)<<2), (uint32_t*)&mem_write_data, 0);
				
				temp_storage_count = temp_storage_count + 4;
			}
			...
			if (radio_tx_option | (exec_count < TEMP_CYCLE_INIT)){
				...
				delay(RADIO_PACKET_DELAY);
				send_radio_data_srr(1,0xC3,*REG_CHIP_ID,((0xBB00|read_data_batadc_diff)<<8)|exec_count,hrv_light_count,0);
			}
	...
	*NVIC_ISER = (1 << IRQ_WAKEUP) | (1 << IRQ_GOCEP) | (1 << IRQ_TIMER32) | (1 << IRQ_REG0)| (1 << IRQ_REG1)| (1 << IRQ_REG2)| (1 << IRQ_REG3);
	...
	if (temp_running){
		...
		if (hrv_light_count_read == 0xFFFFFF){
			...
			hrv_light_count_old = 0;
			exec_count = 0;
			// Go to sleep to reset the counter
		...
		mbus_write_message32(0xC1,hrv_light_count);
		//#endif
	}
	...
	// Tune SNT Timer R for TC
		...
		mbus_remote_register_write(HRV_ADDR,0x00,wakeup_data & 0xF); // HRV_TOP_CONV_RATIO(0~15 >> 9x~23x); default: 14
		// Go to sleep without timer
		operation_sleep_notimer();
