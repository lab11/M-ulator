...
/*
 *
 * use the MBUS Reg #7 (IRQ#15) Interrupt as a soft-Halt
 *
 */

.global handler_ext_int_reg7      /* 31 External Interrupt(15) */
.type handler_ext_int_reg7, %function
handler_ext_int_reg7:
    ...
    */
    ...
    mov     r1, LR // SPECIAL LR
    tst     r0, r1
    beq     reg7_stacking_used_MSP

    reg7_stacking_used_PSP:
    mrs     r0, PSP         // first parameter - stacking was using PSP
    b       handler_ext_int_reg7_2 

    reg7_stacking_used_MSP:
    mrs     r0, MSP         // first parameter - stacking was using MSP
  
    handler_ext_int_reg7_2:
    /* ok, now we can actually call the c svc_handler */
