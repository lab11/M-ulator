...
int main() {
	...
	*NVIC_ISER = (1 << IRQ_WAKEUP) | (1 << IRQ_GOCEP) | (1 << IRQ_TIMER32) | (1 << IRQ_REG0)| (1 << IRQ_REG1)| (1 << IRQ_REG2)| (1 << IRQ_REG3);
		...
		*GOC_DATA_IRQ = 0;
		...
		if (exec_count_irq == 0){
			...
			if (temp_storage_count > 4000){
				if (read_data_batadc_diff > 0x2){ // 0x4D okay
					not_enough_batt = 1;
				}
				...
				if (read_data_batadc_diff > 0x4){ // 0x4F okay
					not_enough_batt = 1;
				}
			}
	...
	// Configure # of measurement
		...
		if (NUM_MEAS_USER == 0) NUM_MEAS_USER = 0;
        operation_sleep_notimer();
	...
	// Tune SNT Timer R for TC
		...
		srrv4_r07.RO_MOM = wakeup_data & 0x3F;
		srrv4_r07.RO_MIM = wakeup_data & 0x3F;
		mbus_remote_register_write(SRR_ADDR,0x07,srrv4_r07.as_int);
