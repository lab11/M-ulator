...
//                                                  g: Play
//
//  0xB0    pmu_adc_vbat_val                    PMU ADC Reading
//
...
#define DEBUG_ADC                   // ADC Debugging
...
static void snt_operation (void) {
    ...
    *NVIC_ISER = (0x1 << (SNT_TARGET_REG_IDX+8));
    ...
    uint32_t pmu_adc_vbat_val = pmu_reg_read(0x03) & 0xFF;

    #ifdef DEBUG_ADC
        mbus_write_message32(0xB0, (__PMU_TICK_ADC_CLK__<<24) | pmu_adc_vbat_val);
    #endif
...
static void update_system_configs(uint32_t use_default) {
    ...
    if (use_default) {
        ...
        eeprom_timer_calib_interval     =  0;   // (           10 ) Timer Calibration Interval. '0' means 'do not calibrate'. (unit: minute)
    ...
    if (get_flag(FLAG_ACTIVATED)) {
        if ((goc_raw==GOC_START_KEY) || (goc_raw==GOC_STOP_KEY) || (goc_raw==GOC_RESET_KEY)
            ...
            if (nfc_i2c_get_token(1)) {
                ...
                operation_sleep_snt_timer(/*auto_reset*/1, /*threshold*/6000);
            }
        }
    }
#endif
...
int main(void) {
    ...
    *NVIC_ISER = (0x1 << IRQ_TIMER32) | (0x1 << IRQ_XOT);
    ...
    if (get_flag(FLAG_ACTIVATED)) {
        ...
        //update_snt_timer_threshold();
        //operation_sleep_snt_timer(/*auto_reset*/0, /*threshold*/snt_timer_threshold);
        operation_sleep_snt_timer(/*auto_reset*/1, /*threshold*/1000);
