...
uint32_t pmu_adc_read_and_sar_ratio_adjustment() {
    ...
    if      (adc_vbat_dout < 109) { new_val = 0x60; upper_limit = 109; lower_limit =   0; }
    else if (adc_vbat_dout < 118) { new_val = 0x54; upper_limit = 118; lower_limit = 109; }
    else if (adc_vbat_dout < 127) { new_val = 0x4C; upper_limit = 127; lower_limit = 118; }
    else                          { new_val = 0x48; upper_limit = 256; lower_limit = 127; }
    ...
    if (((new_val > curr_val) && (adc_vbat_dout < (upper_limit - hyst))) 
     || ((new_val < curr_val) && (adc_vbat_dout > (lower_limit + hyst)))) {
        ...
        delay(40000);
        #ifdef DEBUG
            mbus_write_message32(0x90, (0xFF<<24)|(adc_vbat_dout<<16)|(curr_val<<8)|(new_val<<0));
        #endif
    }
    #ifdef DEBUG
    else {
        mbus_write_message32(0x90, (0x00<<24)|(adc_vbat_dout<<16)|(curr_val<<8)|(new_val<<0));
    }
    #endif
