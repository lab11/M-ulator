...
uint32_t pmu_adc_read_and_sar_ratio_adjustment(uint32_t offset) {
    ...
    if      (adc_vbat_dout < (109+offset)) { new_val = 0x60; upper_limit = 109 + offset; lower_limit =   0 + offset; }
    else if (adc_vbat_dout < (118+offset)) { new_val = 0x54; upper_limit = 118 + offset; lower_limit = 109 + offset; }
    else if (adc_vbat_dout < (127+offset)) { new_val = 0x4C; upper_limit = 127 + offset; lower_limit = 118 + offset; }
    ...
    if (((new_val > curr_val) && (adc_vbat_dout < (upper_limit - hyst))) 
     || ((new_val < curr_val) && (adc_vbat_dout > (lower_limit + hyst)))) {
        ...
        delay(40000);
        #ifdef DEBUG
            mbus_write_message32(0x90, (offset<<24)|(adc_vbat_dout<<16)|(curr_val<<8)|(new_val<<0));
        #endif
    }
    #ifdef DEBUG
    else {
        mbus_write_message32(0x91, (offset<<24)|(adc_vbat_dout<<16)|(curr_val<<8)|(new_val<<0));
    }
    #endif
