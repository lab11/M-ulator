...
uint32_t pmu_calc_new_sar_ratio(uint32_t adc_val, uint32_t offset) {
    ...
    if      (adc_val < (109+offset)) { new_val = 0x60; upper_limit = 109 + offset; lower_limit =   0 + offset; }
    else if (adc_val < (118+offset)) { new_val = 0x54; upper_limit = 118 + offset; lower_limit = 109 + offset; }
    else if (adc_val < (127+offset)) { new_val = 0x4C; upper_limit = 127 + offset; lower_limit = 118 + offset; }
    else                             { new_val = 0x48; upper_limit = 256 + offset; lower_limit = 127 + offset; }

    if (((new_val > curr_val) && (adc_val < (upper_limit - hyst))) 
     || ((new_val < curr_val) && (adc_val > (lower_limit + hyst)))) {
        #ifdef DEBUG
            mbus_write_message32(0x90, (offset<<24)|(adc_val<<16)|(curr_val<<8)|(new_val<<0));
        #endif
        return new_val;
    }
    else {
        #ifdef DEBUG
            mbus_write_message32(0x90, (offset<<24)|(adc_val<<16)|(curr_val<<8)|(curr_val<<0));
        #endif
        return curr_val;
    }
}
...
void nfc_power_off(void) {
    #ifdef DEBUG_LT
        nfc_i2c_byte_write(/*e2*/0, /*addr*/ EEPROM_ADDR_LT_I2C_TOKEN_STATUS, /*data*/ 0x0, /*nb*/ 1);
    #endif
    if (__nfc_on__) {
...
uint32_t nfc_i2c_get_token(uint32_t id, uint32_t max_num_try) {
#ifdef DEBUG_LT
    nfc_i2c_byte_write(/*e2*/0, /*addr*/ EEPROM_ADDR_LT_I2C_TOKEN_ID, /*data*/ id, /*nb*/ 1);
    nfc_i2c_byte_write(/*e2*/0, /*addr*/ EEPROM_ADDR_LT_I2C_TOKEN_STATUS, /*data*/ 0xF, /*nb*/ 1);
#endif
    if (!__nfc_i2c_token__) {
        ...
        while (num_try < max_num_try) {
            ...
            if (nfc_i2c_byte_imm(0xA6)) {
                ...
                if (__nfc_i2c_timeout__) {
                    #ifdef DEBUG_LT
                        nfc_i2c_byte_write(/*e2*/0, /*addr*/ EEPROM_ADDR_LT_I2C_TOKEN_STATUS, /*data*/ 0x3, /*nb*/ 1);
                    #endif
                    return 0;
                }
                // If everything went well, get the token
                    ...
                    __nfc_i2c_token__ = 1;
                    #ifdef DEBUG_LT
                        nfc_i2c_byte_write(/*e2*/0, /*addr*/ EEPROM_ADDR_LT_I2C_TOKEN_STATUS, /*data*/ 0x1, /*nb*/ 1);
                    #endif
                    return 1;
                }
            }
        ...
        #ifdef DEBUG_LT
            nfc_i2c_byte_write(/*e2*/0, /*addr*/ EEPROM_ADDR_LT_I2C_TOKEN_STATUS, /*data*/ 0x4, /*nb*/ 1);
        #endif
        return 0;
    }
    // You already got the token.
        ...
        #ifdef DEBUG_LT
            nfc_i2c_byte_write(/*e2*/0, /*addr*/ EEPROM_ADDR_LT_I2C_TOKEN_STATUS, /*data*/ 0x2, /*nb*/ 1);
        #endif
        return 1;
    }
}
