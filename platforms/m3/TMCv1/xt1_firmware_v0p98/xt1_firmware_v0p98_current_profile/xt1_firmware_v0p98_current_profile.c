...
static void set_system(uint32_t target, uint32_t reset_cmd, uint32_t reset_start_info) {
    ...
    //------------------------------------------
        ...
        eid_update_with_eeprom(DISP_PLAY|DISP_CHECK);
    }
    //------------------------------------------
    // FOR 'STARTED' TARGET
    //------------------------------------------
    ...
    *NVIC_ISER = (0x1 << IRQ_TIMER32) | (0x1 << IRQ_XOT);
    ...
    if (get_flag(FLAG_ACTIVATED)) {
        ...
        if (WAKEUP_BY_SNT) {
            ...
            mbus_write_message32(0xE0, (mode<<4)|iter);
...
//            }
        ...
        //operation_sleep_snt_timer(/*auto_reset*/0, /*threshold*/snt_timer_1min);
        operation_sleep_snt_timer(/*auto_reset*/1, /*threshold*/5*1500);
