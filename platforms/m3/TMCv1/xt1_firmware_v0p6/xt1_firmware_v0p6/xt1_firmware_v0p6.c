...
//                  - Removed the NFC 'UPDATE' command.
//                  - Removed ALSO_STORE_VBAT switch. 
//                          Instead, now it saves the latest ADC reading at EEPROM_ADDR_VBAT_INFO
...
//
//  0x82    {0x10, snt_running}                 snt_running variable (SNT State in snt_operation())
//
...
#ifdef DEBUG
    mbus_write_message32(0xE0, snt_temp_val);
    mbus_write_message32(0xE1, eeprom_eid_high_temp_threshold);
    mbus_write_message32(0xE2, eeprom_eid_low_temp_threshold);
    mbus_write_message32(0xE3, eeprom_eid_duration_high);
    mbus_write_message32(0xE4, eeprom_eid_fe_duration_high);
    mbus_write_message32(0xE5, eeprom_eid_duration_mid);
    mbus_write_message32(0xE6, eeprom_eid_fe_duration_mid);
    mbus_write_message32(0xE7, eeprom_eid_duration_low);
    mbus_write_message32(0xE8, eeprom_eid_fe_duration_low);
#endif

    // Update duration based on lastest temperature measurement
    if (snt_temp_val > eeprom_eid_high_temp_threshold) {
        #ifdef DEBUG
            mbus_write_message32(0xEF, 0x00000001);
        #endif
        ...
        #ifdef DEBUG
            mbus_write_message32(0xEF, 0x00000002);
        #endif
        ...
        #ifdef DEBUG
            mbus_write_message32(0xEF, 0x00000003);
        #endif
...
static void snt_operation (void) {
    ...
    #ifdef DEBUG
        mbus_write_message32(0x82, (0x10 << 24) | snt_running);
    #endif
...
static void update_system_configs(uint32_t use_default) {
    ...
    if (use_default) {
        ...
        eeprom_eid_high_temp_threshold  = 850;  // ( 850  (=15C)  ) Temperature Threshold (Raw Code) to determine HIGH and MID temperature for EID
        eeprom_eid_low_temp_threshold   = 300;  // ( 300  (=-5C)  ) Temperature Threshold (Raw Code) to determine MID and LOW temperature for EID
