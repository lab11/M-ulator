...
//                          Instead, it immediately calles main() again (i.e., it skips the sleep).
//-------------------------------------------------------------------------------------------
//
...
int main() {
    ...
    *NVIC_ISER = (0x1 << IRQ_TIMER32) | (0x1 << IRQ_XOT);
    ...
    if (get_flag(FLAG_ACTIVATED)) {
        ...
        // If snt_timer_threshold is too close to the current SNT counter value, directly call main() here.
        ...
        if (distance < eeprom_snt_base_freq) {
            #ifdef DEBUG
