...
#define NO_PMU
#define SIMPLE_WAKEUP_SLEEP // If enabled, it will do a simple wakeup/sleep
#define USE_SNT_TIMER       // If enabled, it runs the SNT timer, rather than the XO.
...
#define __GPO_MASK__ (0x1<<__NFC_GPO__)
#define __I2C_MASK__ (0x1<<__NFC_SCL__)|(0x1<<__NFC_SDA__)
...
#else // if SIMPLE_WAKEUP_SLEEP is defined
    delay(10000);
    reset_xo_cnt(); // Make counter value = 0
    operation_sleep_xo_timer(XOT_SLEEP_DURATION_LONG);
#endif
