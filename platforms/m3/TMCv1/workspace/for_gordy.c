...
static void nfc_power_on(void) {
    ...
    //gpio_set_dir_with_mask(GPO_MASK,(0<<NFC_GPO)); // This is not required, as the default direction is 0 (input)
    ...
    // NOTE: VNFC rising time is < 5us (XT1#4, MS612T, 22uF VBAT decap)
    //                           <15us (XT1#4, MS612T, no VBAT decap)
    set_cps(0x1 << NFC_CPS);
...
void handler_ext_int_gocep(void) { // GOCEP
    ...
    *NVIC_ICPR = (0x1 << IRQ_GOCEP);
    ...
    //--------------------------
    // Shorcuts (goc_header = 0x0)
    //--------------------------
    // goc_data = 0x0: Power on NFC
    // goc_data = 0x1:
    // goc_data = 0x2:
    // goc_data = 0x3:
    // goc_data = 0x4:
    ...
    // goc_data = 0xD:
    // goc_data = 0xE:
    // goc_data = 0xF: Power off NFC and go to sleep
    //--------------------------
    if (goc_header == 0x00) {
        ...
        // Go to sleep (after turning off NFC)
