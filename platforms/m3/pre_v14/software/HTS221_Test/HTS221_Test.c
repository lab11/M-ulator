//*******************************************************************
//Author: ZhiYoong Foo
//Description: Activity Detection System on 6socket. HTS221
//*******************************************************************
#include "PREv14.h"
#include "HTS221.h"
#include "mbus.h"

// AStack order PRE->PMU
#define PMU_ADDR    0x6
...
//GPIO[1] == CLK
//GPIO[2] == MOSI/MISO
static void HTS221_reg_wr (uint32_t reg_id, uint32_t reg_data){
  uint32_t test_msg;
  uint32_t test_i;
  test_msg = (reg_id<<8|reg_data)&0x3FFF;
  //Startup
  gpio_write_data(0<<2|1<<1|1<<0);
  set_gpio_pad(7);
  gpio_set_dir(7);
  freeze_gpio_pad(0);
  gpio_write_data(0<<2|1<<1|0<<0);
  //Message
  for(test_i=0; test_i<16; test_i++){
    gpio_write_data(((test_msg&(1<<(15-test_i)))>>(15-test_i))<<2|0<<1|0<<0);
    gpio_write_data(((test_msg&(1<<(15-test_i)))>>(15-test_i))<<2|1<<1|0<<0);
  }
  //EOM
  gpio_write_data(0<<2|0<<1|1<<0);
  freeze_gpio_pad(1);
}

static uint32_t HTS221_reg_rd (uint32_t reg_id){
  uint32_t test_msg;
  uint32_t test_i;
  uint32_t test_reply;
  test_reply = 0;
  test_msg = (1<<15|reg_id<<8)&0xBF00;
  //Startup
  gpio_write_data(0<<2|1<<1|1<<0);
  set_gpio_pad(7);
  gpio_set_dir(7);
  ...
  //Message
  for(test_i=0; test_i<16; test_i++){
    if(test_i==8){gpio_set_dir(3);}
    gpio_write_data(((test_msg&(1<<(15-test_i)))>>(15-test_i))<<2|0<<1|0<<0);
    gpio_write_data(((test_msg&(1<<(15-test_i)))>>(15-test_i))<<2|1<<1|0<<0);
    if(test_i>=8){
      test_reply = test_reply<<1;
      test_reply = test_reply|((*GPIO_DATA>>2)&0x00000001);
    }
  }
  //EOM
  gpio_write_data(0<<2|0<<1|1<<0);
...
static void HTS221_quit(){
  gpio_write_data(0<<2|0<<1|0<<0);
  set_gpio_pad(7);
  gpio_set_dir(7);
  freeze_gpio_pad(0);
  gpio_write_data(0<<2|0<<1|0<<0);
...
int main() {
  ...
  if(wkp_data_hdr == 0x00){
    ...
    *REG_CPS = 0;
    ...
    //HTS221 Write
    HTS221_reg_wr(wkp_data_fld_1,wkp_data_fld_0);
    operation_sleep_notimer();
  }else if(wkp_data_hdr == 0x13){
    mbus_write_message32(0xAA,0x13);
    //HTS221 Read
    mbus_write_message32(0xCC,HTS221_reg_rd(wkp_data_fld_0));
    operation_sleep_notimer();
  }else if(wkp_data_hdr == 0x14){
    mbus_write_message32(0xAA,0x14);
    //Activate Power Gate
    *REG_CPS = 0;
    operation_sleep_notimer();
  }else{
