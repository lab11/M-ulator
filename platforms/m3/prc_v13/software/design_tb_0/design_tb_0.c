...
void cycle2 (void) {
    if (do_cycle2 == 1) {
        ...
        // Copy Reg#1 in FLS and put them in Reg#0 in PRC.
        mbus_copy_registers_from_remote_to_local (FLS_ADDR, 0x01, 0x00, 0);

        // Copy Reg#1-4 in FLS and put them in Reg#0-3 in PRC.
        ...
        *NVIC_ISER =  (0 /*GPIO*/ << 16)    | (0 /*SPI*/ << 15)     | (0 /*GOCEP*/ << 14)  | (0 /*MBUS_FWD*/ << 13) 
        ...
        // Copy Reg#1-4 in FLS and put them in Reg#4-7 in PRC. It must generate 4 IRQs due to Reg#4-7 in PRC.
        ...
        *NVIC_ISER =  (0 /*GPIO*/ << 16)    | (0 /*SPI*/ << 15)     | (0 /*GOCEP*/ << 14)  | (0 /*MBUS_FWD*/ << 13) 
        ...
        // Copy Reg#1-4 in FLS and put them in Reg#4-7 in PRC. It must not generate REG IRQs
        mbus_copy_registers_from_remote_to_local (FLS_ADDR, 0x01, 0x04, 3);
...
void cycle3 (void) {
    if (do_cycle3 == 1) {
        ...
        // Copy MEM 0x00000000 in FLS and store those in PRC's 0x00001400~
        mbus_copy_mem_from_remote_to_any_bulk (FLS_ADDR, 0x00000000, PRC_ADDR, (uint32_t *) 0x00001400, 0);

        // Copy MEM 0x00000000~0x0000003C in FLS and store those in PRC's 0x00001500~
        mbus_copy_mem_from_remote_to_any_bulk (FLS_ADDR, 0x00000000, PRC_ADDR, (uint32_t *) 0x00001500, 9);

        // Before streaming, set the pointer to start from.
        *REG_STR0_OFF = 0x00000510;
        *REG_STR1_OFF = 0x00000500;

        // Copy MEM 0x00000000 in FLS and store those in PRC's 0x00000F9C~
        mbus_copy_mem_from_remote_to_any_stream (1, FLS_ADDR, 0x00000000, PRC_ADDR, 0);

        if (*REG_STR1_OFF == 0x501) pass (0x4, *REG_STR1_OFF);
        else fail (0x4, *REG_STR1_OFF);

        // Copy MEM 0x00000000~0x0000003C in FLS and store those in PRC's 0x00000FA0~
        mbus_copy_mem_from_remote_to_any_stream (0, FLS_ADDR, 0x00000000, PRC_ADDR, 9);
