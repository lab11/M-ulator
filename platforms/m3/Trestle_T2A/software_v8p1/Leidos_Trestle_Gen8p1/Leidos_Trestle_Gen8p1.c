...
static void FLASH_initialization (void) {
    ...
    // Tune Flash
    mbus_remote_register_write (FLP_ADDR , 0x26 , 0x0D7788); // Program Current (default) / Inside Macro
    mbus_remote_register_write (FLP_ADDR , 0x27 , 0x011BC8); // Erase Pump Diode Chain (default) / Inside Macro

    mbus_remote_register_write (FLP_ADDR , 0x01 , 0x0001FF); 
    	// Tprog idle time. Default value: 0x007F09
    	//Tcyc_prog Reg 0x01, Bit Field: [23:8], Default: 16¡¯h007F, W/R
	//	Number of CLKmain cycles to set Program cycle. The default value corresponds to ~128us.
	//Tprog Reg 0x01, Bit Field: [7:0], Default: 8¡¯h09, W/R
	//	Number of CLKmain cycles to set Program time. The default value corresponds to ~10us.

    mbus_remote_register_write (FLP_ADDR , 0x19 , 0x000F06); 
    	// Voltage Clamper Tuning. Default value: 0x000C03
	//DISABLE_BYPASS_MIRROR Reg 0x19, Bit Field: [11], Default: 1¡¯h1, W/R
	//	Enables/Disables a mirror circuit in Voltage Clamper.
	//	If 1, the current to charge the output capacitor is determined by a current reference and two current mirrors.
	//	If 0, the output capacitor is charged through one PMOS transistor directly from 3.6V supply. However, the
	//	output will be still regulated by a comparator.
	//COMP_CTRL_I_1STG Reg 0x19, Bit Field: [10:7], Default: 4¡¯h8, W/R
	//	It changes a current copy ratio of the NMOS current mirror (1x, 2x, 4x, 8x). The copied current is copied one
	//	more time through the PMOS mirror to charge the on-chip capacitor.
	//COMP_CTRL_I_2STG_BAR Reg 0x19, Bit Field: [6:3], Default: 4¡¯h0, W/R
	//	It changes a current copy ratio of the PMOS current mirror (1x, 2x, 4x, 8x). The control bits are inverted. This
	//	PMOS mirror copies current from the NMOS mirror to charge the on-chip capacitor.
	//COMP_CTRL_VOUT Reg 0x19, Bit Field: [2:0], Default: 3¡¯h3, W/R
...
inline static void comp_stream(void){
    ...
    delay(150000); // Compression-length control: ~10s
