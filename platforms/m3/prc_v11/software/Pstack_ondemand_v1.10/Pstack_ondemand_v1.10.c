...
//                              Revision 1.10
//                              - radio_power_on includes SCRO wakeup
//				Revision 1.9
//				- PRCv11
//				- SNSv6
...
static void operation_init(void){
  ...
  *((volatile uint32_t *) 0xA2000008) = 0x00202D08;
  ...
  // Configure SCRO
  radv8_r1.SCRO_FREQ_DIV = 3; //Default 2
  ...
  *((volatile uint32_t *) 0xE000E100) = 0xF;
  ...
  if(wakeup_data_header == 1){
    ...
    *((volatile uint32_t *) IRQ10VEC) = 0;
	...
	// radio
	send_radio_data_ppm(0,0xF00000+cdc_storage_cref_latest);
	// set timer 
	set_wakeup_timer (WAKEUP_PERIOD_CONT_INIT, 0x1, 0x0);
	// go to sleep and wake up with same condition
	operation_sleep_noirqreset();
      ...
      exec_count_irq = 0;
      // radio
      send_radio_data_ppm(1,0xF00000+cdc_storage_cref_latest);
      // Go to sleep without timer
      operation_sleep_notimer();
    }
