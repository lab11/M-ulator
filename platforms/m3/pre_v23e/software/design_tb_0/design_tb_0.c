...
//-------------------------------------------------------------------
// < UPDATE HISTORY >
//  Jul 16 2020 -   PRCv21 Family
//                      - Now gpio_get_data returns correct values for 'output' pads as well,
//                          so 'if (gpio_get_data()==0x0A)' in 'Basic GPIO In/Out Test'
//                          has been replaced with 'if (gpio_get_data()==0xEA)'.
...
volatile uint32_t do_cycle9  = 1; // GPIO
...
void cycle1 (void) {
    if (do_cycle1 == 1) {
        ...
        if ((get_flag(FLAG_PMU_SUB_1)==0) & (get_flag(FLAG_PMU_SUB_0)==0)) {
            ...
            // Below is an implementation of the PMU initialization part in Tstack_ondemand_v6.5.c

            // EN_SAR_TRIM_V3_ACTIVE
            mbus_remote_register_write(PMU_ADDR, 0x16, 0x00ca50);
            // EN_UPC_TRIM_V3_ACTIVE
            mbus_remote_register_write(PMU_ADDR, 0x18, 0x00ca30);
            // EN_DNC_TRIM_V3_ACTIVE
            mbus_remote_register_write(PMU_ADDR, 0x1a, 0x000a30);
            // EN_UPC_TRIM_V3_SLEEP
            mbus_remote_register_write(PMU_ADDR, 0x17, 0x00c621);
            // EN_SAR_TRIM_V3_SLEEP
            mbus_remote_register_write(PMU_ADDR, 0x15, 0x00c641);
            // EN_DNC_TRIM_V3_SLEEP
            mbus_remote_register_write(PMU_ADDR, 0x19, 0x000621);
            // EN_SAR_RATIO_OVERRIDE
            mbus_remote_register_write(PMU_ADDR, 0x05, 0x00082f);
            // EN_SAR_RATIO_OVERRIDE
            mbus_remote_register_write(PMU_ADDR, 0x05, 0x002aaf);
            // EN_CTRL_DESIRED_STATE_ACTIVE
            //mbus_remote_register_write(PMU_ADDR, 0x3c, 0x17c7fd);
            // EN_TICK_REPEAT_VBAT_ADJUST
            mbus_remote_register_write(PMU_ADDR, 0x36, 0x000001);
            // EN_TICK_ADC_RESET
            mbus_remote_register_write(PMU_ADDR, 0x33, 0x000002);
            // EN_TICK_ADC_CLK
            mbus_remote_register_write(PMU_ADDR, 0x34, 0x000002);
            // EN_CTRL_DESIRED_STATE_ACTIVE
            mbus_remote_register_write(PMU_ADDR, 0x3c, 0x17c7ff);
            // EN_VOLTAGE_CLAMP_TRIM
            mbus_remote_register_write(PMU_ADDR, 0x0e, 0x000e30);
            // EN_VOLTAGE_CLAMP_TRIM
            mbus_remote_register_write(PMU_ADDR, 0x0e, 0x000c30);
            // EN_VBAT_READ_TRIM
            mbus_remote_register_write(PMU_ADDR, 0x45, 0x000048);
            // EN_CTRL_STALL_STATE_ACTIVE
            mbus_remote_register_write(PMU_ADDR, 0x3a, 0x103800);
            // EN_CTRL_DESIRED_STATE_ACTIVE
            mbus_remote_register_write(PMU_ADDR, 0x3c, 0x17c7ff);
            // EN_CTRL_DESIRED_STATE_SLEEP
            mbus_remote_register_write(PMU_ADDR, 0x3b, 0x17efff);

            // Enable PMU Write Check
            set_halt_until_mbus_tx();
            PMUv13_REG_CONF.PMU_IRQ_EN = 0x1;
            PMUv13_REG_CONF.PMU_CHECK_WRITE = 0x1;
            mbus_remote_register_write(PMU_ADDR, 0x51, PMUv13_REG_CONF.as_int);
        ...
        // Sub-Test 1: PMU DIN Glitch
            ...
            set_halt_until_mbus_tx();
            PMUv13_REG_CONF.PMU_IRQ_EN = 0x1;
            PMUv13_REG_CONF.PMU_CHECK_WRITE = 0x0;
            mbus_remote_register_write(PMU_ADDR, 0x51, PMUv13_REG_CONF.as_int);
...
void cycle5 (void) {
    if (do_cycle5 == 1) {
        ...
        mbus_copy_mem_from_local_to_remote_bulk (FLP_ADDR, 0x00000000, 0x00000000, 4095);
...
void cycle9 (void) {
    if (do_cycle9 == 1) {
        ...
        //----------------------------------------------------------------------------------------------------------    
            ...
            if (gpio_get_data() == 0xEA) {pass (0xF, gpio_get_data());}
