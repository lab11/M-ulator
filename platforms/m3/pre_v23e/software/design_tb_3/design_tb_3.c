...
//                          SNTv5  -> SNTv6
//                          PMUv12 -> PMUv13
//                      - In initialization(), writing to PMU's HORIZON_CONFIG was added to enable the VCO clock monitoring.
//                      - For arb_debug_reg(0x61, 0x10000000): Changed VCO_CNT_THRESHOLD from 200 to 114.
//                      - For arb_debug_reg(0x61, 0x20000000): Changed VCO_CNT_THRESHOLD from 203 to 122.
//                      - For arb_debug_reg(0x62, 0x20000000): Changed VCO_CNT_THRESHOLD from 990 to 600.
//                      - For arb_debug_reg(0x62, 0x30000000): Changed VCO_CNT_THRESHOLD from 1002 to 602.
//-------------------------------------------------------------------
// < AUTHOR > 
...
volatile pmuv13_r53_t PMUv13_REG_VCO = PMUv13_R53_DEFAULT;
...
void initialization (void) {
    ...
    // Enable PMU SAR Clock Monitoring
    //---------------------------------------------------------------------------------------
    // HORIZON_CONFIG
    //---------------------------------------------------------------------------------------
    ...
    //---------------------------------------------------------------------------------------
        ...
        | (0x0 << 11)   // 0x0      // If 1, 'horizon' enqueues 'vbat_read_only'
        //------------------------------------------------------------------------
        ...
        | (0x0  << 8)   // 0x0      // Reserved
        | (0x48 << 0)   // 0x48     // Sets ADC_SAMPLING_BIT in 'vbat_read_only' task (0<=N<=255); ADC measures (N/256)xVBAT in Flip mode.
    ));
...
void cycle0 (void) {
    if (do_cycle0 == 1) {
        ...
        if (!get_flag(FLAG_VCO)) {
            ...
            PMUv13_REG_VCO.VCO_CNT_ENABLE    = 0x0;
            PMUv13_REG_VCO.VCO_CNT_MODE      = 0x0;
            PMUv13_REG_VCO.VCO_CNT_THRESHOLD = 0x0;
            mbus_remote_register_write(PMU_ADDR, 0x53, PMUv13_REG_VCO.as_int);
	        ...
	        *TIMER32_GO   = 0x1;
            ...
            set_halt_until_mbus_trx();
            PMUv13_REG_VCO.VCO_CNT_ENABLE    = 0x1;
            PMUv13_REG_VCO.VCO_CNT_MODE      = 0x1;
            PMUv13_REG_VCO.VCO_CNT_THRESHOLD = 1000;
            mbus_remote_register_write(PMU_ADDR, 0x53, PMUv13_REG_VCO.as_int);
            ...
            *TIMER32_GO = 0;
            ...
            PMUv13_REG_VCO.VCO_CNT_ENABLE    = 0x1;
            PMUv13_REG_VCO.VCO_CNT_MODE      = 0x0;
            PMUv13_REG_VCO.VCO_CNT_THRESHOLD = 0x1000;
            mbus_remote_register_write(PMU_ADDR, 0x53, PMUv13_REG_VCO.as_int);
...
void cycle1  (void) { 
    if (do_cycle1  == 1) { 
        ...
        if (get_mb_flag(FLAG_PMUWUP, FLAG_PMUWUP_LEN) == 0) {
            ...
            PMUv13_REG_VCO.VCO_CNT_ENABLE    = 0x1;
            PMUv13_REG_VCO.VCO_CNT_MODE      = 0x1;
            PMUv13_REG_VCO.VCO_CNT_THRESHOLD = 114;
            mbus_remote_register_write(PMU_ADDR, 0x53, PMUv13_REG_VCO.as_int);
        ...
        //          => PMU should first go into Sleep and then wake up (pending wakeup request)
            ...
            PMUv13_REG_VCO.VCO_CNT_ENABLE    = 0x1;
            PMUv13_REG_VCO.VCO_CNT_MODE      = 0x1;
            PMUv13_REG_VCO.VCO_CNT_THRESHOLD = 122;
            mbus_remote_register_write(PMU_ADDR, 0x53, PMUv13_REG_VCO.as_int);
...
void cycle2  (void) { 
    if (do_cycle2  == 1) { 
        ...
        if (get_mb_flag(FLAG_PMUGL, FLAG_PMUGL_LEN) == 0) {
            ...
            PMUv13_REG_VCO.VCO_CNT_ENABLE    = 0x1;
            PMUv13_REG_VCO.VCO_CNT_MODE      = 0x0;
            PMUv13_REG_VCO.VCO_CNT_THRESHOLD = 600;
            mbus_remote_register_write(PMU_ADDR, 0x53, PMUv13_REG_VCO.as_int);
            ...
            PMUv13_REG_VCO.VCO_CNT_ENABLE    = 0x1;
            PMUv13_REG_VCO.VCO_CNT_MODE      = 0x0;
            PMUv13_REG_VCO.VCO_CNT_THRESHOLD = 602;
            mbus_remote_register_write(PMU_ADDR, 0x53, PMUv13_REG_VCO.as_int);
