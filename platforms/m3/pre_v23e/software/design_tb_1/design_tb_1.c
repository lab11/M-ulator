...
void cycle1 (void) {
    if (do_cycle1 == 1) {
        ...
        if (!get_flag(FLAG_XO_SUB_2) && !get_flag(FLAG_XO_SUB_1) && !get_flag(FLAG_XO_SUB_0)) {
          ...
          //*REG_XO_CONF1 = 0x90A603; // Default Value
            *REG_XO_CONF1 = 0x90A613; // XO_RESETn = 1
            *REG_XO_CONF1 = 0x90A612; // XO_ISOL_CLK_LP = 0
        ...
        // 7: XO Timer: Use it as a normal timer (End of Testing)
          ...
          //*REG_XO_CONF1 = 0x90A603; // Power-gate and Isolate the XO driver (Default value)
