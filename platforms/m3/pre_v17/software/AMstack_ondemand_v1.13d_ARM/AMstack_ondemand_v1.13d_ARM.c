...
//			v1.13d_ARM: Send temp data during radio test mode
//*******************************************************************
#include "PREv17.h"
#include "PREv17_RF.h"
#include "mbus.h"
		...
		*TIMER32_GO = 0;
		...
		if (meas_count < TEMP_NUM_MEAS){	
			...
			if (astack_detection_mode > 0){
				...
				radio_power_on();
				// Send debug signal
				send_radio_data_mrr(1, 0x4D0000 | (*REG_CHIP_ID & 0xFFFF), 0xB00000| (read_data_batadc<<12),(0xF&radio_packet_count)<<20 | 0xA0000 | temp_storage_latest & 0xFFFF);
				set_wakeup_timer(WAKEUP_PERIOD_CONT, 0x1, 0x1);
			}
