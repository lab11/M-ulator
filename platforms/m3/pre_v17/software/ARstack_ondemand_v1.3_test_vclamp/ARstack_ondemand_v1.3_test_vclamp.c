...
*/
	...
	}else if(wakeup_data_header == 0x55){
		...
		mbus_remote_register_write(PMU_ADDR,0x0E, 
			...
			| (1 << 9)  // Enables override setting [8]
			| (0 << 8)  // Turn on the harvester-inhibiting switch
			| (wakeup_data_field_0&0xF0)  // clamp_tune_bottom (increases clamp thresh)
			| (wakeup_data_field_0&0x7) 		// clamp_tune_top (decreases clamp thresh)
		));
		delay(MBUS_DELAY);
		delay(MBUS_DELAY);
		mbus_remote_register_write(PMU_ADDR,0x0E, 
