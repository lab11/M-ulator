...
//				  Used for MOD Demo @ UK 2018 Feb
//				  Increasing default sleep clk for low temp
//			v1.3LT: Changing active settings for low temp
//				  Also adding 8us pulse option
//*******************************************************************
#include "PREv17.h"
#include "PREv17_RF.h"
#include "mbus.h"
...
inline static void pmu_set_sleep_low(){
    ...
    mbus_remote_register_write(PMU_ADDR,0x17, 
		( (3 << 14) // Desired Vout/Vin ratio; defualt: 0
		| (0 << 13) // Enable main feedback loop
		| (0xF << 9)  // Frequency multiplier R
		| (0x0 << 5)  // Frequency multiplier L (actually L+1)
...
static void radio_power_on(){
    ...
    mrrv5_r04.MRR_SCRO_RSTN_TIMER = 1;  //UNRST TIMER
    mbus_remote_register_write(MRR_ADDR,0x04,mrrv5_r04.as_int);
	delay(MBUS_DELAY*100); // Freq stab

    mrrv5_r04.MRR_SCRO_EN_CLK = 1;  //Enable clk
    mbus_remote_register_write(MRR_ADDR,0x04,mrrv5_r04.as_int);
	delay(MBUS_DELAY*100); // Freq stab
...
static void radio_power_off(){
    ...
    mrrv5_r0E.MRR_RAD_FSM_SLEEP = 1;
    mbus_remote_register_write(MRR_ADDR,0x0E,mrrv5_r0E.as_int);

    // Turn off Current Limter
    mrrv5_r00.MRR_CL_EN = 0;  //Enable CL
    mbus_remote_register_write(MRR_ADDR,0x00,mrrv5_r00.as_int);

    mrrv5_r04.MRR_SCRO_EN_TIMER = 0;
    mrrv5_r04.MRR_SCRO_RSTN_TIMER = 0;
    mrrv5_r04.MRR_SCRO_EN_CLK = 0;
    mbus_remote_register_write(MRR_ADDR,0x04,mrrv5_r04.as_int);
...
static void mrr_configure_pulse_width_long(){
    ...
    //mrrv5_r0F.MRR_RAD_FSM_TX_PW_LEN = 15; //64us PW
    //mrrv5_r10.MRR_RAD_FSM_TX_C_LEN = 512; // (PW_LEN+1):C_LEN=1:32
    //mrrv5_r0F.MRR_RAD_FSM_TX_PS_LEN = 15; // PW=PS   
    ...
    //mrrv5_r10.MRR_RAD_FSM_TX_C_LEN = 4000; // (PW_LEN+1):C_LEN=1:32
    //mrrv5_r0F.MRR_RAD_FSM_TX_PS_LEN = 124; // PW=PS   
    //mrrv5_r12.MRR_RAD_FSM_TX_HDR_CNST = 7; //8 bit shift in LFSR

    //mrrv5_r0F.MRR_RAD_FSM_TX_PW_LEN = 249; //1ms PW
    //mrrv5_r10.MRR_RAD_FSM_TX_C_LEN = 8000; // (PW_LEN+1):C_LEN=1:32
    //mrrv5_r0F.MRR_RAD_FSM_TX_PS_LEN = 249; // PW=PS
...
static void mrr_configure_pulse_width_short_8us(){

    mrrv5_r0F.MRR_RAD_FSM_TX_PW_LEN = 1; //4us PW
    mrrv5_r10.MRR_RAD_FSM_TX_C_LEN = 64; // (PW_LEN+1):C_LEN=1:32
    mrrv5_r0F.MRR_RAD_FSM_TX_PS_LEN = 3; // PW=PS guard interval betwen 0 and 1 pulse
    mrrv5_r12.MRR_RAD_FSM_TX_HDR_CNST = 0; //no shift in LFSR
...
static void send_radio_data_mrr(uint32_t last_packet, uint32_t radio_data_0, uint32_t radio_data_1, uint32_t radio_data_2){
    ...
    if (!radio_ready){
		...
		// Release FSM Reset
		mrrv5_r0E.MRR_RAD_FSM_RSTN = 1;  //UNRST BB
		mbus_remote_register_write(MRR_ADDR,0x0E,mrrv5_r0E.as_int);
		delay(MBUS_DELAY*10);
	...
	while (count < num_packets){
		...
		mrrv5_r00.MRR_TRX_CAP_ANTP_TUNE = mrr_cfo_vals[count]; 
		mrrv5_r01.MRR_TRX_CAP_ANTN_TUNE = mrr_cfo_vals[count];
		mbus_remote_register_write(MRR_ADDR,0x00,mrrv5_r00.as_int);
...
//    set_halt_until_mbus_rx();
	...
	mrr_freq_hopping = 1;
	mrr_cfo_vals[0] = 0x0001;//0x01FF;
	mrr_cfo_vals[1] = 0x0003;//0x03FF;
	mrr_cfo_vals[2] = 0x0000;//0x00FF;

	// TX Setup Carrier Freq
	mrrv5_r00.MRR_TRX_CAP_ANTP_TUNE = mrr_cfo_vals[0];  //ANT CAP 14b unary 830.5 MHz
	...
	mbus_remote_register_write(MRR_ADDR,0x00,mrrv5_r00.as_int);
	mrrv5_r01.MRR_TRX_CAP_ANTN_TUNE = mrr_cfo_vals[0]; //ANT CAP 14b unary 830.5 MHz
	...
	mrrv5_r0E.MRR_RAD_FSM_TX_D_LEN = RADIO_DATA_LENGTH; //0-skip tx data
	mbus_remote_register_write(MRR_ADDR,0x0E,mrrv5_r0E.as_int);
	...
	*NVIC_ISER = (1 << IRQ_WAKEUP) | (1 << IRQ_GOCEP) | (1 << IRQ_TIMER32) | (1 << IRQ_REG0)| (1 << IRQ_REG1)| (1 << IRQ_REG2)| (1 << IRQ_REG3);
    ...
    if(wakeup_data_header == 1){
		...
		*GOC_DATA_IRQ = 0;
		...
		operation_goc_trigger_radio(wakeup_data_field_0, wakeup_data_field_1, 0xB00000, read_data_batadc);
...
*/
		...
		mrrv5_r00.MRR_TRX_CAP_ANTP_TUNE = mrr_cfo_vals[0]; 
		mrrv5_r01.MRR_TRX_CAP_ANTN_TUNE = mrr_cfo_vals[0];
		mbus_remote_register_write(MRR_ADDR,0x00,mrrv5_r00.as_int);
		mbus_remote_register_write(MRR_ADDR,0x01,mrrv5_r01.as_int);
		...
		if (wakeup_data_field_0 == 0xFF) {
			mrr_configure_pulse_width_long();
			
		}else if (wakeup_data_field_0 == 0xF){
			mrr_configure_pulse_width_short_8us();
