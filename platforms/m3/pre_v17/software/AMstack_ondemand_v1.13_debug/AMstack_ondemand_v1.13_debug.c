...
static void operation_init(void){
	...
	*REG_SRAM_TUNE = prev17_r0D.as_int;
	...
	//Zhiyoong
	//Run LIGHT ONLY OPERATION
	//CAN SET LIGHT INTERVAL in CONFIG SECTION OF GUI
	//LIGHT DETECTION INTERVAL. SET TO 300s. Click the button to program it.
	//Because they're already programmed, you need to use a slower GOC speed. Typically, fresh out of the box, it's 50, then 25 for trigger, but since it's re-programming, we need to use 25.
	//Can use the GUI to reprogram too. Go to debug --> it'll let you choose the path for the hex file. On the bottom, there should be check boxes where you can put in goc speed
	//One text box for GOC speed, one goc speed for PR "programming". That should be currently set to 50, set it to 25.
	//Should be a button called "flash" or "load program" click that.
	//Wait a few minutes to try reprogramming again if fail. use goc speed 50 again
