...
volatile uint32_t read_data_state_adc;
...
inline static void set_pmu_clk_init(){
  ...
  set_pmu_adc_period(2); // 0x100 about 1 min for 1/2/1 1P2 setting
}
  ...
  if(wakeup_data_header == 1){
    ...
    *((volatile uint32_t *) IRQ14VEC) = 0;
    ...
    if (exec_count_irq == 0){
      ...
      mbus_remote_register_write(PMU_ADDR,0x00,0x03);
      read_data_batadc = *((volatile uint32_t *) REG0) & 0xFF;
      mbus_remote_register_write(PMU_ADDR,0x00,0x41);
      read_data_state_adc = *((volatile uint32_t *) REG0) & 0xFF;
      set_halt_until_mbus_tx();
      delay(10*MBUS_DELAY);
      mbus_write_message32(0xAB, read_data_batadc);
      delay(10*MBUS_DELAY);
      mbus_write_message32(0xAC, read_data_state_adc);
    ...
    //Auto Configure V3P6 Based on ADC Reading
    //Grab ADC Reading First
    if (exec_count_irq == 0){
      exec_count_irq++;
      set_halt_until_mbus_rx();
      mbus_remote_register_write(PMU_ADDR,0x33,0x1);
      mbus_remote_register_write(PMU_ADDR,0x34,0x1);
      set_wakeup_timer(4, 0x1, 0x1);
      // go to sleep and wake up with same condition
      operation_sleep_noirqreset();
    }else{
      set_halt_until_mbus_rx();
	...
	mbus_remote_register_write(PMU_ADDR,0x05, //default 12'h000
				   ( (1 << 13) // Enables override setting [12] (1'b1)
				     ...
				     | (0 << 8) // Switch input / output power rails for upconversion (1'h0)
				     | (1 << 7) // Enable override setting [6:0] (1'h0)
				     | (62) 		// Binary converter's conversion ratio (7'h00)
				     ));
      }
      //Battery 2.6V ~ 2.9V
      else if ((read_data_batadc >= 108) && (read_data_batadc <= 118)){
	mbus_remote_register_write(PMU_ADDR,0x05, //default 12'h000
				   ( (1 << 13) // Enables override setting [12] (1'b1)
				     | (0 << 12) // Let VDD_CLK always connected to vbat
				     | (1 << 11) // Enable override setting [10] (1'h0)
				     ...
				     | (0 << 8) // Switch input / output power rails for upconversion (1'h0)
				     | (1 << 7) // Enable override setting [6:0] (1'h0)
				     | (68) 		// Binary converter's conversion ratio (7'h00)
				     ));
      }
      //Battery 2.3V ~ 2.6V
      else if ((read_data_batadc >= 120) && (read_data_batadc <= 134)){
	mbus_remote_register_write(PMU_ADDR,0x05, //default 12'h000
				   ( (1 << 13) // Enables override setting [12] (1'b1)
				     | (0 << 12) // Let VDD_CLK always connected to vbat
				     | (1 << 11) // Enable override setting [10] (1'h0)
				     ...
				     | (0 << 8) // Switch input / output power rails for upconversion (1'h0)
				     | (1 << 7) // Enable override setting [6:0] (1'h0)
				     | (77) 		// Binary converter's conversion ratio (7'h00)
				     ));
      }
      //Battery 2.0V ~ 2.3V
      else if (read_data_batadc >= 136){
	mbus_remote_register_write(PMU_ADDR,0x05, //default 12'h000
				   ( (1 << 13) // Enables override setting [12] (1'b1)
				     | (0 << 12) // Let VDD_CLK always connected to vbat
				     | (1 << 11) // Enable override setting [10] (1'h0)
				     ...
				     | (0 << 8) // Switch input / output power rails for upconversion (1'h0)
				     | (1 << 7) // Enable override setting [6:0] (1'h0)
				     | (86) 		// Binary converter's conversion ratio (7'h00)
				     ));
      }
      set_halt_until_mbus_tx();
      exec_count_irq = 0;
	...
	mbus_remote_register_write(PMU_ADDR,0x05, //default 12'h000
      ...
      //Battery 2.6V ~ 2.9V
      else if ((read_data_batadc >= 114) && (read_data_batadc <= 124)){
	mbus_remote_register_write(PMU_ADDR,0x05, //default 12'h000
				   ( (1 << 13) // Enables override setting [12] (1'b1)
				     | (0 << 12) // Let VDD_CLK always connected to vbat
				     | (1 << 11) // Enable override setting [10] (1'h0)
      ...
      //Battery 2.3V ~ 2.6V
      else if ((read_data_batadc >= 126) && (read_data_batadc <= 140)){
	mbus_remote_register_write(PMU_ADDR,0x05, //default 12'h000
				   ( (1 << 13) // Enables override setting [12] (1'b1)
				     | (0 << 12) // Let VDD_CLK always connected to vbat
				     | (1 << 11) // Enable override setting [10] (1'h0)
