...
//
//			Version 3.1
//			- Removed part for Harvest Clamp
//			- Disabled PMU voltage clamp
//
//			Version 3.0
...
inline static void set_pmu_init(){
	...
	// Turn off voltage clamp
		...
		( (0 << 10) // When to turn on harvester-inhibiting switch(0: whenever PoR is on 1: only when VBAT is high); default: 0 
...
static void operation_init(void){
	...
	*((volatile uint32_t *) REG_CLKGEN_TUNE ) = prcv14_r0B.as_int;
	...
	// Mbus return address; Needs to be between 0x18-0x1F
    	...
    	WAKEUP_PERIOD_CONT_INIT = 2;   // 0x1E (30): ~1 min with PRCv9
  	...
  	config_timerwd(0x3FFFFF); // Config watchdog timer to about 10 sec (default: 0x02FFFFFF), 0xFFFFF about 13 sec with Y2 run default clock
		...
		*((volatile uint32_t *) IRQ14VEC) = 0;
		...
		Tstack_state = TSTK_IDLE;
        	...
        	// wakeup_data[15:0] is the # of data to transmit; if zero, all stored data is sent
        	// wakeup_data[23:16] is the user-specified period
		
        	WAKEUP_PERIOD_CONT_INIT = wakeup_data_field_2;
		radio_tx_numdata = wakeup_data_field_0 + (wakeup_data_field_1<<8);
		...
		// Change the RF frequency & Debug mode: Transmit something via radio and go to sleep w/o timer
        	// wakeup_data[7:0] is the # of transmissions
        	// wakeup_data[15:8] is the user-specified period
