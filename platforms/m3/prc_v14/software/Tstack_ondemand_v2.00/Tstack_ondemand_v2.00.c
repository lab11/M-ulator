...
//             PRCv14, SNSv7, RADv9, PMUv4, HRVv5
...
//#define DEF_HRV_ADDR 0x3
#define DEF_RAD_ADDR 0x4
#define DEF_SNS_ADDR 0x5
#define DEF_PMU_ADDR 0x6
...
// Tstack states
#define DEF_TSTK_IDLE       0x0
#define DEF_TSTK_LDO        0x1
#define DEF_TSTK_TEMP_RSTRL 0x3
#define DEF_TSTK_TEMP_START 0x2
#define DEF_TSTK_TEMP_READ  0x6

#define NUM_TEMP_MEAS 5 

// Radio configurations
#define DEF_RADIO_DATA_LENGTH 24
#define DEF_RADIO_PACKET_DELAY 5000
#define DEF_RADIO_TIMEOUT_COUNT 50
#define DEF_WAKEUP_PERIOD_RADIO_INIT 2

// PMU configurations
#define DEF_PMU_4P2_ADC 75

#define DEF_TEMP_STORAGE_SIZE 500 // Need to leave about 500 Bytes for stack

#define DEF_TIMERWD_VAL 0xFFFFF // 0xFFFFF about 13 sec with Y2 run default clock
...
volatile uint32_t temp_storage[DEF_TEMP_STORAGE_SIZE] = {0};
...
volatile uint32_t temp_running;  //zhiyoong bool?
...
inline static void set_pmu_adc_period(uint32_t val){
  ...
  mbus_remote_register_write(DEF_PMU_ADDR,0x3C,
  ...
  delay(MBUS_DELAY*10);
  //zhiyoong: DO WE REALLY NEED *10?!?!
  // Register 0x36: TICK_REPEAT_VBAT_ADJUST
  mbus_remote_register_write(DEF_PMU_ADDR,0x36,val); 
  delay(MBUS_DELAY*10);
...
inline static void set_pmu_sleep_clk_radio(){
  ...
  mbus_remote_register_write(DEF_PMU_ADDR,0x15, 
  ...
  mbus_remote_register_write(DEF_PMU_ADDR,0x17, 
...
inline static void set_pmu_clk_init(){
  set_halt_until_mbus_rx();
  //zhiyoong CHECK IF REGISTER GOT WRITTEN
  // Register 0x17: V3P6 Upconverter Sleep Settings
  mbus_remote_register_write(DEF_PMU_ADDR,0x17, 
			     ( (3 << 14)   // Desired Vout/Vin ratio; defualt: 0
			       | (0 << 13) // Enable main feedback loop
  ...
  mbus_remote_register_write(DEF_PMU_ADDR,0x18, 
			     ( (3 << 14)   // Desired Vout/Vin ratio; defualt: 0
			       | (0 << 13) // Enable main feedback loop
			       | (2 << 9)  // Frequency multiplier R
  ...
  mbus_remote_register_write(DEF_PMU_ADDR,0x19,
			     ( (0 << 13)  // Enable main feedback loop
			       | (1 << 9) // Frequency multiplier R
  ...
  mbus_remote_register_write(DEF_PMU_ADDR,0x1A,
			     ( (0 << 13)  // Enable main feedback loop
  ...
  mbus_remote_register_write(DEF_PMU_ADDR,0x16, 
			     ( (0 << 19)   // Enable PFM even during periodic reset
			       | (0 << 18) // Enable PFM even when Vref is not used as ref
			       | (0 << 17) // Enable PFM
			       | (3 << 14) // Comparator clock division ratio
  ...
  mbus_remote_register_write(DEF_PMU_ADDR,0x05,
...
inline static void pmu_adc_disable(){
  ...
  mbus_remote_register_write(DEF_PMU_ADDR,0x3B,
			     ((  1 << 0) //state_sar_scn_on
			      | (1 << 1) //state_wait_for_clock_cycles
			      | (1 << 2) //state_wait_for_time
			      | (1 << 3) //state_sar_scn_reset
...
inline static void reset_pmu_solar_short(){
  set_halt_until_mbus_rx();
  mbus_remote_register_write(DEF_PMU_ADDR,0x0E, 
...
static void radio_power_on(){
  // Requires >2s stabilization time
  ...
  mbus_remote_register_write(DEF_RAD_ADDR,0xD,radv9_r13.as_int);
  radv9_r2.SCRO_RESET = 0;
  mbus_remote_register_write(DEF_RAD_ADDR,0x2,radv9_r2.as_int);

  // Additional delay required after SCRO Reset release
  // zhiyoong do we really need this?
...
static void send_radio_data_ppm(bool last_packet, uint32_t radio_data){
  ...
  mbus_remote_register_write(DEF_RAD_ADDR,0x3,radv9_r3.as_int);

  if (radv9_r13.RAD_FSM_RESETn == 0){
  ...
  for( count=0; count<DEF_RADIO_TIMEOUT_COUNT; count++ ){
    if( mbus_msg_flag ){
      set_halt_until_mbus_tx();
      mbus_msg_flag = 0;
      if (last_packet){
...
static void temp_sensor_enable(){
  ...
  mbus_remote_register_write(DEF_SNS_ADDR,0xE,snsv7_r14.as_int);
}
...
static void ldo_power_off(){
  ...
  mbus_remote_register_write(DEF_SNS_ADDR,18,snsv7_r18.as_int);
}
...
static void operation_tx_stored(void){
  ...
  while(((!radio_tx_numdata)&&(radio_tx_count > 0)) | ((radio_tx_numdata)&&((radio_tx_numdata+radio_tx_count) > temp_storage_count))){
    ...
    config_timerwd(DEF_TIMERWD_VAL);

    // Radio out data
    send_radio_data_ppm(0, temp_storage[radio_tx_count]);
    delay(DEF_RADIO_PACKET_DELAY); //Set delays between sending subsequent packet

    radio_tx_count--;
  }

  delay(DEF_RADIO_PACKET_DELAY*2); //Set delays between sending subsequent packet
...
static void operation_init(void){
  ...
  *((volatile uint32_t *) REG_CLKGEN_TUNE ) = prcv14_r0B.as_int;
  ...
  //Enumeration
  set_halt_until_mbus_rx();
  mbus_enumerate(DEF_RAD_ADDR);
  mbus_enumerate(DEF_SNS_ADDR);
  //mbus_enumerate(DEF_HRV_ADDR);
  mbus_enumerate(DEF_PMU_ADDR);
  ...
  mbus_remote_register_write(DEF_PMU_ADDR,0x3A, 
  ...
  ///////////////////////
  // SNSv7 Configurations
  ///////////////////////
  set_halt_until_mbus_tx();
  // SNSv7 0x0E
  snsv7_r14.TEMP_SENSOR_BURST_MODE = 0x0;
  snsv7_r14.TEMP_SENSOR_DELAY_SEL  = 0x5;
  snsv7_r14.TEMP_SENSOR_R_tmod     = 0x0;
  snsv7_r14.TEMP_SENSOR_R_bmod     = 0x0;
  mbus_remote_register_write(DEF_SNS_ADDR,0x0E,snsv7_r14.as_int);
  // SNSv7 0x0F
  snsv7_r15.TEMP_SENSOR_AMP_BIAS   = 0x7;
  snsv7_r15.TEMP_SENSOR_CONT_MODEb = 0x0;
  snsv7_r15.TEMP_SENSOR_SEL_CT     = 0x6;
  mbus_remote_register_write(DEF_SNS_ADDR,0x0F,snsv7_r15.as_int);
  // SNSv7 0x12
  ...
  mbus_remote_register_write(DEF_SNS_ADDR,0x12,snsv7_r18.as_int);
  // SNSv7 0x18
  mbus_remote_register_write(DEF_SNS_ADDR,0x18,0x001800);
  // SNSv7 0x19
  mbus_remote_register_write(DEF_SNS_ADDR,0x19,0x001000);

  ///////////////////////
  // RADv9 Configurations
  ///////////////////////
  // RADv9 0x0
  radv9_r0.RADIO_TUNE_CURRENT_LIMITER = 0x2F; //0x2F = 30uA, 0x1F = 3uA
  radv9_r0.RADIO_TUNE_FREQ1           = 0x0;
  radv9_r0.RADIO_TUNE_FREQ2           = 0x0;
  radv9_r0.RADIO_TUNE_TX_TIME         = 0x6;
  mbus_remote_register_write(DEF_RAD_ADDR,0x00,radv9_r0.as_int);
  // RADv9 0xB
  radv9_r11.RAD_FSM_H_LEN = 16; // N
  radv9_r11.RAD_FSM_D_LEN = DEF_RADIO_DATA_LENGTH-1; // N-1
  radv9_r11.RAD_FSM_C_LEN = 10;
  mbus_remote_register_write(DEF_RAD_ADDR,0xB,radv9_r11.as_int);
  // RADv9 0x1
  radv9_r1.SCRO_FREQ_DIV        = 3;
  radv9_r1.SCRO_AMP_I_LEVEL_SEL = 2;
  radv9_r1.SCRO_I_LEVEL_SELB    = 0x60;
  mbus_remote_register_write(DEF_RAD_ADDR,0x1,radv9_r1.as_int);
  // RADv9 0xC
  radv9_r12.RAD_FSM_SEED = 4;
  mbus_remote_register_write(DEF_RAD_ADDR,0xC,radv9_r12.as_int);
  // RADv9 0xF
  mbus_remote_register_write(DEF_RAD_ADDR,0xF,0x1900);
  ...
  ///////////////////////
  // HRVv5 Configurations
  ///////////////////////
  //hrvv5_r0.HRV_TOP_CONV_RATIO = 0x6;
  //mbus_remote_register_write(DEF_HRV_ADDR,0,hrvv5_r0.as_int);
...
static void operation_temp_run(void){
  ...
  if (Tstack_state == DEF_TSTK_IDLE){
#ifdef DEBUG_MBUS_MSG 
    mbus_write_message32(0xBB, 0xFBFB0000);
    delay(MBUS_DELAY*10);
#endif
    Tstack_state = DEF_TSTK_LDO;
  ...
  }else if (Tstack_state == DEF_TSTK_LDO){
#ifdef DEBUG_MBUS_MSG
    mbus_write_message32(0xBB, 0xFBFB1111);
    delay(MBUS_DELAY*10);
#endif
    Tstack_state = DEF_TSTK_TEMP_RSTRL;
  ...
  }else if (Tstack_state == DEF_TSTK_TEMP_RSTRL){
#ifdef DEBUG_MBUS_MSG
    mbus_write_message32(0xBB, 0xFBFB2222);
    delay(MBUS_DELAY*10);
#endif
    Tstack_state = DEF_TSTK_TEMP_READ;
  ...
  }else if (Tstack_state == DEF_TSTK_TEMP_START){
    ...
    *TIMER32_GO = 0;
  ...
  }else if (Tstack_state == DEF_TSTK_TEMP_READ){
#ifdef DEBUG_MBUS_MSG
    ...
    if (meas_count < NUM_TEMP_MEAS){      
      ...
      temp_sensor_disable();
      Tstack_state = DEF_TSTK_TEMP_START;
      ...
      if (temp_storage_count < DEF_TEMP_STORAGE_SIZE){
...
static void operation_goc_trigger_init(void){
  ...
  //Check ADC Results during Normal Operations
  set_halt_until_mbus_rx();
  mbus_remote_register_write(DEF_PMU_ADDR,0x00,0x03);
  read_data_batadc = *((volatile uint32_t *) REG0) & 0xFF;
  set_halt_until_mbus_tx();
#ifdef DEBUG_MBUS_MSG
  mbus_write_message32(0xAC, read_data_batadc);
#endif
  if (read_data_batadc < (DEF_PMU_4P2_ADC + 2)){
    mbus_remote_register_write(DEF_PMU_ADDR,0x0E, 
			       ( (1 << 10)  // When to turn on harvester-inhibiting switch (0: PoR, 1: VBAT high)
				 | (1 << 9) // Enables override setting [8]
...
#ifdef DEBUG_MBUS_MSG
    mbus_write_message32(0xAD, 0x12345);
#endif
  }else if (read_data_batadc > (DEF_PMU_4P2_ADC + 6)){
    mbus_remote_register_write(DEF_PMU_ADDR,0x0E, 
			       ( (1 << 10)  // When to turn on harvester-inhibiting switch (0: PoR, 1: VBAT high)
				 | (1 << 9) // Enables override setting [8]
				 | (0 << 8) // Turn on the harvester-inhibiting switch
...
#ifdef DEBUG_MBUS_MSG
    mbus_write_message32(0xAD, 0x54321);
#endif
  }
}
...
int main() {
  ...
  if(wakeup_data_header == 1){
    ...
    // wakeup_data[ 7: 0] : # of transmissions
    // wakeup_data[15: 8] : user-specified period
    // wakeup_data[23:16] : MSB of # of transmissions
	...
	radio_power_on();
	// SCRO stabilitzation
	set_wakeup_timer(DEF_WAKEUP_PERIOD_RADIO_INIT, 0x1, 0x1);
	operation_sleep_noirqreset();
      }else{
    ...
    *((volatile uint32_t *) IRQ14VEC) = 0;
	...
	discharge_count++;
	delay(DEF_RADIO_PACKET_DELAY*2);
	// Prevent watchdog kicking in
	config_timerwd(DEF_TIMERWD_VAL);
	delay(DEF_RADIO_PACKET_DELAY*2);
    ...
    mbus_remote_register_write(DEF_RAD_ADDR,0,radv9_r0.as_int);
    delay(MBUS_DELAY);
