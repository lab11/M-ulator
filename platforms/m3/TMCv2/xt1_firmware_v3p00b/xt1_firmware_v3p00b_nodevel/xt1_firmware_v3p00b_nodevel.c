...
static void operation_init (void) {
    ...
    if (!get_flag(FLAG_ENUMERATED)) {
        ...
        *REG_WUPT_CONFIG = *REG_WUPT_CONFIG & 0xFF3FFFFF; // WUP_ENABLE=0, WUP_WREQ_EN=0
        ...
        // Sub-Versions
        nfc_i2c_byte_write(/*e2*/0, /*addr*/6, /*data*/0x000b, /*nb*/2);
        ...
        if (// If this is the very first sample (right after the start delay)
           ...
           || (sub_sample_cnt==eeprom_user_config.temp_meas_interval)) {
            ...
            if (status.sample_type==SAMPLE_RAW) {
                ...
                uint32_t byte_addr = (eeprom_sample_cnt<<3)+EEPROM_ADDR_DATA_RESET_VALUE;

                // Store the raw data
                //  Max Sample Count = (8192 - 128) / 8 = 1008
                //  However, we split this into two and store the raw data into the first half,
                //  and SAR/ADC values in to the second half.
                //  i.e., max sample count = 1008 / 2 = 504
                //      Byte# 128 - Byte#4159: SAR, ADC, Raw Temp Data, previous snt_threshold (each sample is 64-bit)
                //      Byte#4160 - Byte#8192: XO Information, xo_cnt          (each sample is 64-bit)
                if (eeprom_sample_cnt<504) {
                    sar_adc_addr = byte_addr + (504<<3);
