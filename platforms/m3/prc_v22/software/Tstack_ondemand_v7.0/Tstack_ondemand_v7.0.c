...
//            v6.4: PRCv20, PMUv11, SNTv4, HRVv8, SRRv7
...
//			  v6.5a: Send radio packet for configuration GOC messages
//			  v7.0: PRCv22, PMUv13r2, SNTv6r1, HRVv9A, SRRv7, take functions from XT1
//*******************************************************************
#include "PRCv22.h"
#include "PRCv22_RF.h"
#include "SNTv6_RF.h"
...
volatile snt_r08_t snt_r08 = SNT_R08_DEFAULT;
volatile snt_r09_t snt_r09 = SNT_R09_DEFAULT;
volatile snt_r0A_t snt_r0A = SNT_R0A_DEFAULT;
volatile snt_r0B_t snt_r0B = SNT_R0B_DEFAULT;
volatile snt_r17_t snt_r17 = SNT_R17_DEFAULT;

volatile srrv7_r00_t srrv7_r00 = SRRv7_R00_DEFAULT;
volatile srrv7_r01_t srrv7_r01 = SRRv7_R01_DEFAULT;
volatile srrv7_r02_t srrv7_r02 = SRRv7_R02_DEFAULT;
volatile srrv7_r03_t srrv7_r03 = SRRv7_R03_DEFAULT;
volatile srrv7_r04_t srrv7_r04 = SRRv7_R04_DEFAULT;
volatile srrv7_r07_t srrv7_r07 = SRRv7_R07_DEFAULT;
volatile srrv7_r11_t srrv7_r11 = SRRv7_R11_DEFAULT;
volatile srrv7_r12_t srrv7_r12 = SRRv7_R12_DEFAULT;
volatile srrv7_r13_t srrv7_r13 = SRRv7_R13_DEFAULT;
volatile srrv7_r14_t srrv7_r14 = SRRv7_R14_DEFAULT;
volatile srrv7_r15_t srrv7_r15 = SRRv7_R15_DEFAULT;
volatile srrv7_r1E_t srrv7_r1E = SRRv7_R1E_DEFAULT;
volatile srrv7_r1F_t srrv7_r1F = SRRv7_R1F_DEFAULT;

volatile hrvv5_r0_t hrvv5_r0 = HRVv5_R0_DEFAULT;

volatile prcv22_r0B_t prcv22_r0B = PRCv22_R0B_DEFAULT;
volatile prcv22_r1C_t prcv22_r1C = PRCv22_R1C_DEFAULT;
...
//***************************************************
// Temp Sensor Functions (SNTv6)
//***************************************************

static void temp_sensor_start(){
    snt_r01.TSNS_RESETn = 1;
    mbus_remote_register_write(SNT_ADDR,1,snt_r01.as_int);
}
...
static void sns_ldo_vref_on(){
    ...
    mbus_remote_register_write(SNT_ADDR,0,snt_r00.as_int);
}
...
static void snt_start_timer_presleep(){
    ...
    snt_r09.TMR_IBIAS_REF = 0x4; // Default : 4'h4
...
/*
Function from T6.5a
void snt_set_wup_timer(uint32_t sleep_count){
    ...
    snt_r17.WUP_ENABLE = 0x1; // Default : 0x
...
static void radio_power_on(){
    ...
    srrv7_r00.SRR_CL_EN = 1;  //Enable CL
    mbus_remote_register_write(SRR_ADDR,0x00,srrv7_r00.as_int);

    // Release timer power-gate
    srrv7_r04.RO_EN_RO_V1P2 = 1;  //Use V1P2 for TIMER
    mbus_remote_register_write(SRR_ADDR,0x04,srrv7_r04.as_int);
    delay(MBUS_DELAY);

    // Turn on timer
    srrv7_r04.RO_RESET = 0;  //Release Reset TIMER
    mbus_remote_register_write(SRR_ADDR,0x04,srrv7_r04.as_int);
    delay(MBUS_DELAY);

    srrv7_r04.RO_EN_CLK = 1; //Enable CLK TIMER
    mbus_remote_register_write(SRR_ADDR,0x04,srrv7_r04.as_int);
    delay(MBUS_DELAY);

    srrv7_r04.RO_ISOLATE_CLK = 0; //Set Isolate CLK to 0 TIMER
    mbus_remote_register_write(SRR_ADDR,0x04,srrv7_r04.as_int);

    // Release FSM Sleep
    srrv7_r11.SRR_RAD_FSM_SLEEP = 0;  // Power on BB
    mbus_remote_register_write(SRR_ADDR,0x11,srrv7_r11.as_int);
...
static void radio_power_off(){
    ...
    // Turn off everything
    srrv7_r03.SRR_TRX_ISOLATEN = 0;     //set ISOLATEN 0
    mbus_remote_register_write(SRR_ADDR,0x03,srrv7_r03.as_int);

    srrv7_r11.SRR_RAD_FSM_EN = 0;  //Stop BB
    srrv7_r11.SRR_RAD_FSM_RSTN = 0;  //RST BB
    srrv7_r11.SRR_RAD_FSM_SLEEP = 1;
    mbus_remote_register_write(SRR_ADDR,0x11,srrv7_r11.as_int);

    // Turn off Current Limter
    srrv7_r00.SRR_CL_EN = 0;  //Enable CL
    mbus_remote_register_write(SRR_ADDR,0x00,srrv7_r00.as_int);

    srrv7_r04.RO_RESET = 1;  //Release Reset TIMER
    srrv7_r04.RO_EN_CLK = 0; //Enable CLK TIMER
    srrv7_r04.RO_ISOLATE_CLK = 1; //Set Isolate CLK to 0 TIMER
    mbus_remote_register_write(SRR_ADDR,0x04,srrv7_r04.as_int);

    // Enable timer power-gate
    srrv7_r04.RO_EN_RO_V1P2 = 0;  //Use V1P2 for TIMER
    mbus_remote_register_write(SRR_ADDR,0x04,srrv7_r04.as_int);
...
static void srr_configure_pulse_width_short(){
    ...
    srrv7_r12.SRR_RAD_FSM_TX_PW_LEN = 0; //4us PW
    srrv7_r13.SRR_RAD_FSM_TX_C_LEN = 32; // (PW_LEN+1):C_LEN=1:32
    srrv7_r12.SRR_RAD_FSM_TX_PS_LEN = 1; // PW=PS guard interval betwen 0 and 1 pulse

    mbus_remote_register_write(SRR_ADDR,0x12,srrv7_r12.as_int);
    mbus_remote_register_write(SRR_ADDR,0x13,srrv7_r13.as_int);
}
...
static void send_radio_data_srr_sub1(){
    ...
    // Fire off data
    srrv7_r11.SRR_RAD_FSM_EN = 1;  //Start BB
    mbus_remote_register_write(SRR_ADDR,0x11,srrv7_r11.as_int);
...
static void send_radio_data_srr(uint32_t last_packet, uint8_t radio_packet_prefix, uint16_t radio_data_2, uint32_t radio_data_1, uint32_t radio_data_0, uint8_t num_tx_duplicate){
    ...
    if (!radio_ready){
        ...
        // Release FSM Reset
        srrv7_r11.SRR_RAD_FSM_RSTN = 1;  //UNRST BB
        mbus_remote_register_write(SRR_ADDR,0x11,srrv7_r11.as_int);
        delay(MBUS_DELAY);

    srrv7_r03.SRR_TRX_ISOLATEN = 1;     //set ISOLATEN 1, let state machine control
    mbus_remote_register_write(SRR_ADDR,0x03,srrv7_r03.as_int);
        delay(MBUS_DELAY);
...
static void operation_init(void){
    ...
    prcv22_r0B.CLK_GEN_RING = 0x1; // Default 0x1
    prcv22_r0B.CLK_GEN_DIV_MBC = 0x1; // Default 0x1
    prcv22_r0B.CLK_GEN_DIV_CORE = 0x2; // Default 0x3
    prcv22_r0B.GOC_CLK_GEN_SEL_FREQ = 0x5; // Default 0x6
    prcv22_r0B.GOC_CLK_GEN_SEL_DIV = 0x0; // Default 0x0
    *REG_CLKGEN_TUNE = prcv22_r0B.as_int;

    prcv22_r1C.SRAM0_TUNE_ASO_DLY = 31; // Default 0x0, 5 bits
    prcv22_r1C.SRAM0_TUNE_DECODER_DLY = 15; // Default 0x2, 4 bits
    prcv22_r1C.SRAM0_USE_INVERTER_SA = 0; // Default 0x2, 4 bits
    *REG_SRAM0_TUNE = prcv22_r1C.as_int;
    //*REG_SRAM_TUNE = 0x45; // use inverter SA
    //*REG_SRAM_TUNE = 0x44; // use default SA
  
    //Enumerate & Initialize Registers
    Tstack_state = TSTK_IDLE;    //0x0;
    enumerated = 0x54437000;
    exec_count = 0;
    exec_count_irq = 0;
...
*/
    ...
    // Config Register A
    snt_r0A.TMR_S = 0x1; // Default: 0x4, use 1 for good TC
    // Tune R for TC
    snt_r0A.TMR_DIFF_CON = 0x3FFF; // Default: 0x3FFB
    mbus_remote_register_write(SNT_ADDR,0x0A,snt_r0A.as_int);

	// TIMER CAP_TUNE  
    snt_r0B.TMR_TFR_CON = 0xF; // Default: 0xF
    mbus_remote_register_write(SNT_ADDR,0x0B,snt_r0B.as_int);

    // TIMER CAP_TUNE  
    // Tune C for freq
	snt_r09.TMR_SEL_CLK_DIV = 0x0; // Default : 1'h1
    snt_r09.TMR_SEL_CAP = 0x80; // Default : 8'h8
    snt_r09.TMR_SEL_DCAP = 0x3F; // Default : 6'h4

    snt_r09.TMR_EN_TUNE1 = 0x1; // Default : 1'h1
    snt_r09.TMR_EN_TUNE2 = 0x1; // Default : 1'h1

    // to reduce standby current
    snt_r09.TMR_IBIAS_REF = 0x0; // Default : 4'h4

    mbus_remote_register_write(SNT_ADDR,0x09,snt_r09.as_int);

    // Wakeup Counter
    snt_r17.WUP_CLK_SEL = 0x0; 
    snt_r17.WUP_AUTO_RESET = 0x0; // Automatically reset counter to 0 upon sleep 
    mbus_remote_register_write(SNT_ADDR,0x17,snt_r17.as_int);

    // Radio Settings (SRRv7) -------------------------------------------
    srrv7_r1F.LC_CLK_RING = 0x3;  // ~ 150 kHz
    srrv7_r1F.LC_CLK_DIV = 0x3;  // ~ 150 kHz
    mbus_remote_register_write(SRR_ADDR,0x1F,srrv7_r1F.as_int);
    ...
    // Adjust C
    srrv7_r07.RO_MOM = 0x10;
    srrv7_r07.RO_MIM = 0x10;
    mbus_remote_register_write(SRR_ADDR,0x07,srrv7_r07.as_int);

    // Current Limter set-up 
    srrv7_r00.SRR_CL_CTRL = 16; //Set CL 1-finite 16-20uA

    // TX Setup Carrier Freq
    srrv7_r00.SRR_TRX_CAP_ANTP_TUNE = 0x0000;  //ANT CAP 14b unary 830.5 MHz
    //srrv7_r00.SRR_TRX_CAP_ANTP_TUNE = 0x00FF;  //ANT CAP 14b unary 813.8 MHz
    //srrv7_r00.SRR_TRX_CAP_ANTP_TUNE = 0x0FFF;  //ANT CAP 14b unary 805.5 MHz
    mbus_remote_register_write(SRR_ADDR,0x00,srrv7_r00.as_int);
    srrv7_r01.SRR_TRX_CAP_ANTN_TUNE = 0x0000; //ANT CAP 14b unary 830.5 MHz
    //srrv7_r01.SRR_TRX_CAP_ANTN_TUNE = 0x00FF; //ANT CAP 14b unary 813.8 MHz
    //srrv7_r01.SRR_TRX_CAP_ANTN_TUNE = 0x0FFF;  //ANT CAP 14b unary 805.5 MHz
    mbus_remote_register_write(SRR_ADDR,0x01,srrv7_r01.as_int);
    srrv7_r02.SRR_TX_BIAS_TUNE = 0x0FFF;  //Set TX BIAS TUNE 13b // Set to max
    mbus_remote_register_write(SRR_ADDR,0x02,srrv7_r02.as_int);

    // RX Setup
    srrv7_r03.SRR_RX_BIAS_TUNE    = 0x000F;//  turn on Q_enhancement
    srrv7_r03.SRR_RX_SAMPLE_CAP    = 0x1;  // RX_SAMPLE_CAP
    mbus_remote_register_write(SRR_ADDR,3,srrv7_r03.as_int);

    srrv7_r14.SRR_RAD_FSM_RX_POWERON_LEN = 0x0;  //Set RX Power on length
    //srrv7_r14.SRR_RAD_FSM_RX_SAMPLE_LEN = 0x3;  //Set RX Sample length  16us
    srrv7_r14.SRR_RAD_FSM_RX_SAMPLE_LEN = 0x7;  //Set RX Sample length  4us
    srrv7_r14.SRR_RAD_FSM_GUARD_LEN = 0x001C; //Set TX_RX Guard length, TX_RX guard 32 cycle (28+5)
    mbus_remote_register_write(SRR_ADDR,0x14,srrv7_r14.as_int);

    srrv7_r15.SRR_RAD_FSM_RX_HDR_BITS = 0x00;  //Set RX header
    srrv7_r15.SRR_RAD_FSM_RX_HDR_TH = 0x00;    //Set RX header threshold
    srrv7_r15.SRR_RAD_FSM_RX_DATA_BITS = 0x1; //Set RX data 16b
    mbus_remote_register_write(SRR_ADDR,0x15,srrv7_r15.as_int);

    srrv7_r1E.SRR_IRQ_REPLY_PACKET = 0x061400; //Read RX data Reply
    mbus_remote_register_write(SRR_ADDR,0x1E,srrv7_r1E.as_int);

    mbus_remote_register_write(SRR_ADDR,0x15,srrv7_r15.as_int);
    
    // Use pulse generator
    srrv7_r02.SRR_TX_PULSE_FINE = 1;
    srrv7_r02.SRR_TX_PULSE_FINE_TUNE = 1;
    mbus_remote_register_write(SRR_ADDR,0x02,srrv7_r02.as_int);

    srrv7_r13.SRR_RAD_FSM_SEED = 1; //default
    srrv7_r14.SRR_RAD_FSM_TX_POWERON_LEN = 7; //3bits
    mbus_remote_register_write(SRR_ADDR,0x14,srrv7_r14.as_int);
    ...
    srrv7_r11.SRR_RAD_FSM_TX_H_LEN = 0; //31-31b header (max)
    srrv7_r11.SRR_RAD_FSM_TX_D_LEN = RADIO_DATA_LENGTH; //0-skip tx data
    mbus_remote_register_write(SRR_ADDR,0x11,srrv7_r11.as_int);

    srrv7_r13.SRR_RAD_FSM_TX_MODE = 3; //code rate 0:4 1:3 2:2 3:1(baseline) 4:1/2 5:1/3 6:1/4
    mbus_remote_register_write(SRR_ADDR,0x13,srrv7_r13.as_int);
...
//***************************************************
// Temperature measurement operation (SNTv6)
//***************************************************
    ...
    if (Tstack_state == TSTK_IDLE){
        ...
        if (meas_count == NUM_TEMP_MEAS){
            ...
            if (wfi_timeout_flag){
				...
				snt_wup_counter_cur = snt_sync_read_wup_timer();
            }else{
        ...
        //    }else{
			...
			// Use SNT Timer    
			snt_set_wup_timer(0,snt_wup_counter_cur+WAKEUP_PERIOD_CONT_USER);
...
static void operation_snt_calibration_radio_binary(uint32_t start_val, uint32_t settle_time){
    ...
    if ((snt_r0A.TMR_DIFF_CON == 0x3FFF) && ((exec_count_irq & 0xF) == 0xF)){
        // Stop condition
        exec_count_irq = 0;
        snt_r0A.TMR_DIFF_CON = 0x3FFF; // Default: 0x3FFF
        // radio
        send_radio_data_srr(1,0xB0,*REG_CHIP_ID,snt_r0A.TMR_DIFF_CON,exec_count_irq,0);    
        // Go to sleep without timer
        operation_sleep_notimer();
    }else{
        
        if ((exec_count_irq & 0xF) == 0xF){ // every 16 iterations
            snt_r0A.TMR_DIFF_CON = snt_r0A.TMR_DIFF_CON<<1 | 0x1; // Default: 0x3FFB
            mbus_remote_register_write(SNT_ADDR,0x0A,snt_r0A.as_int);
        }
        ...
        //pmu_set_sleep_radio();
            ...
            // Tune R for TC
            snt_r0A.TMR_DIFF_CON = start_val; // Default: 0x3FFB
            mbus_remote_register_write(SNT_ADDR,0x0A,snt_r0A.as_int);

			snt_start_timer(DLY_1S);
        ...
        //pmu_set_sleep_low();
            ...
            if (settle_time == 0){
                // set to 30s    
                snt_set_wup_timer(0,snt_wup_counter_cur+0xEA60);
            }else{
                snt_set_wup_timer(0,snt_wup_counter_cur+0x1D4C0*settle_time); // settling time in minutes; assumes 2khz clock
            }
            operation_sleep_noirqreset();
        }else{
            // radio
...
int main() {
    ...
    // Only enable relevant interrupts (PRCv22)
    //enable_reg_irq();
    //enable_all_irq();
    *NVIC_ISER = (1 << IRQ_WAKEUP) | (1 << IRQ_GOCEP) | (1 << IRQ_TIMER32) | (1 << IRQ_REG0)| (1 << IRQ_REG1)| (1 << IRQ_REG2)| (1 << IRQ_REG3);
    ...
    // Tune SNT Timer R for TC
        snt_r0A.TMR_DIFF_CON = wakeup_data & 0x3FFF; // Default: 0x3FFB
        mbus_remote_register_write(SNT_ADDR,0x0A,snt_r0A.as_int);
        operation_sleep_notimer();
        ...
        // Update GOC clock
		prcv22_r0B.GOC_CLK_GEN_SEL_FREQ = (wakeup_data >> 4)&0x7; // Default 0x0
		prcv22_r0B.GOC_CLK_GEN_SEL_DIV = wakeup_data & 0x3; // Default 0x6
		*REG_CLKGEN_TUNE = prcv22_r0B.as_int;
...
*/
        ...
        srrv7_r00.SRR_TRX_CAP_ANTP_TUNE = (wakeup_data & 0x3FFF);
        mbus_remote_register_write(SRR_ADDR,0x00,srrv7_r00.as_int);
        srrv7_r01.SRR_TRX_CAP_ANTN_TUNE = (wakeup_data & 0x3FFF);
        mbus_remote_register_write(SRR_ADDR,0x01,srrv7_r01.as_int);

    }else if(wakeup_data_header == 0x23){
        // Change the baseband frequency of SRR (SFO)
        srrv7_r07.RO_MOM = wakeup_data_field_0 & 0x3F;
        srrv7_r07.RO_MIM = wakeup_data_field_1 & 0x3F;
        mbus_remote_register_write(SRR_ADDR,0x07,srrv7_r07.as_int);
        ...
        // Use pulse generator
        srrv7_r02.SRR_TX_PULSE_FINE = wakeup_data_field_1;
        srrv7_r02.SRR_TX_PULSE_FINE_TUNE = wakeup_data_field_0 & 0xF;
        mbus_remote_register_write(SRR_ADDR,0x02,srrv7_r02.as_int);

    }else if(wakeup_data_header == 0x25){
        // Change the conversion time of the temp sensor
        snt_r03.TSNS_SEL_CONV_TIME = wakeup_data & 0xF; // Default: 0x6
        mbus_remote_register_write(SNT_ADDR,0x03,snt_r03.as_int);
        ...
        // Change SRR C_LEN
        srrv7_r13.SRR_RAD_FSM_TX_C_LEN = wakeup_data & 0xFFFF; // (PW_LEN+1):C_LEN=1:32
        mbus_remote_register_write(SRR_ADDR,0x13,srrv7_r13.as_int);

    }else if(wakeup_data_header == 0x2E){
        // Change SRR TX Amp Current
    srrv7_r02.SRR_TX_BIAS_TUNE = wakeup_data & 0x1FFF;  //Set TX BIAS TUNE 13b // Set to max
        mbus_remote_register_write(SRR_ADDR,0x02,srrv7_r02.as_int);

    }else if(wakeup_data_header == 0x2F){
        // Change SRR Current Limiter Settings
    srrv7_r00.SRR_CL_CTRL = wakeup_data & 0xFF; //Set CL 1-finite 16-20uA
        mbus_remote_register_write(SRR_ADDR,0x00,srrv7_r00.as_int);
