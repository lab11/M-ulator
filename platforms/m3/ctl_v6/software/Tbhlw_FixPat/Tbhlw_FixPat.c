...
//        Zhiyoong Foo
//Description: 	Tbhlw stack longterm test code
//              should survive under dimm light
//*******************************************************************
#include "m3_proc.h"
...
#define RAD_SAMPLE_DELAY 500  //~5 min??   //2       //Wake up timer tuning: # of wake up timer cycles to sleep
...
int main() {
  ...
  *((volatile uint32_t *) 0xE000E100) = 0xF;
  ...
  exec_marker = *((volatile uint32_t*) 0x00000710);
  if ( exec_marker != 0x12345678 ) {

    //Mark execution
    *((volatile uint32_t *) 0x00000710) = 0x12345678;
    *((volatile uint32_t *) 0x00000730) = 0x00000010;   // exec_count

    //Restore Sleep time PMU freq
    *((volatile uint32_t *) 0xA200000C) = 0x0F770029;  

    //Set Harvester Multiplication ratio
    _hrv_r0=0x0D;
    write_mbus_register(HRV_ADDR,0x00,_hrv_r0);

    //Enumeration - Tb: Enumerate RAD first
    enumerate(SNS_ADDR);
    asm ("wfi;");
    enumerate(HRV_ADDR);
    asm ("wfi;");
...
//    write_mbus_register(SNS_ADDR,3,_sns_r3);
    ...
    radio_data = gen_radio_data(temp_data);
