...
// Added for PMU Slow Loop Testing - Yejoong Kim (Jan 2020)
#define PMUS_VRATIO 0			// Vout/Vin ratio for V3P6 Converter
#define PMUS_LOOP_ACTIVE	1	// 0: OFF, 1: ON
#define PMUS_LOOP_SLEEP		1	// 0: OFF, 1: ON
#define PMUS_SETTING 		PMU_25C		// PMU_XXC, where XX = 10, 20, 25, 35, 55, 75, 95
#define PMUS_COMPCLKDIV		3	// Slow Loop Comparator Division Ratio (3 bit)
...
inline static void pmu_set_active_clk(uint8_t r, uint8_t l, uint8_t base, uint8_t l_1p2){
	...
	if (PMUS_LOOP_ACTIVE) {
		// Register 0x10: PMU_SAR_TRIM_ACTIVE
        pmu_reg_write(0x10, 
            ( (7 << 20) 	// Drop detector offset before voltage output (3'h7)
            | (0 << 16) 	// Disable reverse configuration for upconversion (1'h0)
            | (1 << 15)  	// Enable override setting [14] (1'h1)
            | (0 << 14)  	// Turn on drop detector (1'h0)
            | (1 << 13)  	// Enable override setting [13] (1'h1)
            | (1 << 12)  	// Use Vref as frequency controller reference (1'h0)
            | (2 << 8)  	// Drop detector offset (proportional to the value) (3'h2)
            | (4 << 0)  	// Bias current (current is proportional to the value) (4'h4)
        ));
	}

    // Register 0x18: V3P6 Active 
    pmu_reg_write(0x18, 
        ( (PMUS_VRATIO << 14) // Desired Vout/Vin ratio; defualt: 0
        | (PMUS_LOOP_ACTIVE << 13) // Enable main feedback loop
    ...
    pmu_reg_write(0x16, 
        ...
        | (0 << 17) // Enable PFM
        | (PMUS_COMPCLKDIV << 14) // Comparator clock division ratio
        | (PMUS_LOOP_ACTIVE << 13) // Enable main feedback loop
        | (r << 9)  // Frequency multiplier R
...
inline static void pmu_set_sleep_clk(uint8_t r, uint8_t l, uint8_t base, uint8_t l_1p2){
	...
	if (PMUS_LOOP_SLEEP) {
		// Register 0x0F: PMU_SAR_TRIM_SLEEP
        pmu_reg_write(0x0F, 
            ( (7 << 20) 	// Drop detector offset before voltage output (3'h7)
            | (0 << 16) 	// Disable reverse configuration for upconversion (1'h0)
            | (1 << 15)  	// Enable override setting [14] (1'h1)
    ...
    pmu_reg_write(0x17, 
        ( (PMUS_VRATIO << 14) // Desired Vout/Vin ratio; defualt: 0
        | (PMUS_LOOP_SLEEP << 13) // Enable main feedback loop
        | (r << 9)  // Frequency multiplier R
        | (l << 5)  // Frequency multiplier L (actually L+1)
        | (base)        // Floor frequency base (0-63)
    ));
...
static void operation_init(void){
    ...
    prcv20_r0B.CLK_GEN_DIV_MBC = 0x5; // Default 0x1 (orig code = 1)
    prcv20_r0B.CLK_GEN_DIV_CORE = 0x3; // Default 0x3 (orig code = 2)
...
//            }
			...
			pmu_setting_state = PMUS_SETTING;
