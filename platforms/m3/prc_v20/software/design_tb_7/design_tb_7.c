...
//NOTE: Tstack code delay issue
//  In general we have about 0.5ms delay between MBus messages.
//  The long delay in question happens at 100.994 second in the Saleae recording
//  at https://www.dropbox.com/s/i8thvb6xl841xjk/T6.2_correct.logicdata?dl=0
//  To reproduce this issue, you should trigger GOC with payload 0x020A000A
//  after programming.
//
// Originally, this code has been developed for PRCv18.
// This code is updated to use PRCv20 (by Yejoong Kim)
//
// LAYER ORDER:
//  PRC -> SRR -> SNT -> HRV -> PMU -> MEM 
//*******************************************************************
...
#define WAKEUP_PERIOD_RADIO_INIT 0x10 // About 2 sec (PRCv20)

#define TEMP_STORAGE_SIZE 8192 // MEMv1: 16kB, 8k 2-byte data

#define TIMERWD_VAL 0xFFFFF // 0xFFFFF about 13 sec with Y5 run default clock (PRCv20)
#define TIMER32_VAL 0x50000 // 0x20000 about 1 sec with Y5 run default clock (PRCv20)
...
volatile prcv20_r1C_t prcv20_r1C = PRCv20_R1C_DEFAULT;
...
static void operation_init(void){
    ...
    *REG_CLKGEN_TUNE = prcv20_r0B.as_int;
    ...
    prcv20_r1C.SRAM0_TUNE_DECODER_DLY = 15; // Default 0x2, 4 bits
    prcv20_r1C.SRAM0_USE_INVERTER_SA = 0; // Default 0x2, 4 bits
    *REG_SRAM0_TUNE = prcv20_r1C.as_int;
        ...
        *TIMER32_GO = 0;
        ...
        if (meas_count < NUM_TEMP_MEAS){    
            ...
            if (temp_storage_count < TEMP_STORAGE_SIZE){
                ...
                // NOTE: THIS IS WHERE THE DELAY HAPPENS
                set_halt_until_mbus_rx();
            ...
            // Test consecutive Mbus Messages
            mbus_write_message32(0xC0, 0x11111111);
            mbus_write_message32(0xD0, 0x22222222);
            mbus_write_message32(0xE0, 0x33333333);

            // Test consecutive Mbus Messages (direct accesses)
            *((volatile uint32_t *) 0xA0003800) = 0x44444444;
            *((volatile uint32_t *) 0xA0003900) = 0x55555555;
            *((volatile uint32_t *) 0xA0003A00) = 0x66666666;
...
int main() {
    ...
    *NVIC_ISER = (1 << IRQ_WAKEUP) | (1 << IRQ_GOCEP) | (1 << IRQ_TIMER32) | (1 << IRQ_REG0)| (1 << IRQ_REG1)| (1 << IRQ_REG2)| (1 << IRQ_REG3);
    ...
    wakeup_data = *GOC_DATA_IRQ; // wakeup_data = 0x020A000A
    uint32_t wakeup_data_header = (wakeup_data>>24) & 0xFF; // 0x02
    uint32_t wakeup_data_field_0 = wakeup_data & 0xFF;      // 0x0A
    uint32_t wakeup_data_field_1 = wakeup_data>>8 & 0xFF;   // 0x00
    uint32_t wakeup_data_field_2 = wakeup_data>>16 & 0xFF;  // 0x0A
