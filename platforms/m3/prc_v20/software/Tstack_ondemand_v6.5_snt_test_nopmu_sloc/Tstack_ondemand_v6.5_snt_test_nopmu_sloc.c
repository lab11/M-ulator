...
static void snt_start_timer_presleep(){
    ...
    sntv4_r08.TMR_EN_SELF_CLK = 0x0; // 0x0 for sloscillator
    mbus_remote_register_write(SNT_ADDR,0x08,sntv4_r08.as_int);
...
static void snt_start_timer_postsleep(){
    ...
    sntv4_r08.TMR_EN_OSC = 0x1; // Keep sloscillator turned on
    mbus_remote_register_write(SNT_ADDR,0x08,sntv4_r08.as_int);
}
...
*/
    ...
    // PMU_REFERENCE_TRIM Register
    pmu_reg_write(0xC,
        ( (0x3 << 5) // NMOS Pulldown: default 0x0
        | (0x1E) // PMOS Pullup: don't use max 0x1F, default 0x18
    ));
    pmu_reg_write(0xD,
        ( (0x3 << 5) // NMOS Pulldown
        | (0x1E) // PMOS Pullup: don't use max 0x1F, default 0x18
