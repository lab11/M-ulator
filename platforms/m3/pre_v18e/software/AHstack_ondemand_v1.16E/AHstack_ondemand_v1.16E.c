...
volatile prev18e_r0B_t prev18e_r0B = PREv18E_R0B_DEFAULT;
volatile prev18e_r1C_t prev18e_r1C = PREv18E_R1C_DEFAULT;
volatile prev18e_r1D_t prev18e_r1D = PREv18E_R1D_DEFAULT;
...
//    prev18e_r0B.CLK_GEN_RING = 0x1; // Default 0x1
//    prev18e_r0B.CLK_GEN_DIV_MBC = 0x2; // Default 0x1
//    prev18e_r0B.CLK_GEN_DIV_CORE = 0x4; // Default 0x3
    prev18e_r0B.GOC_CLK_GEN_SEL_DIV = 0x0; // Default 0x0
    prev18e_r0B.GOC_CLK_GEN_SEL_FREQ = 0x6; // Default 0x6
	*REG_CLKGEN_TUNE = prev18e_r0B.as_int;

    prev18e_r1C.SRAM0_TUNE_ASO_DLY = 31; // Default 0x0, 5 bits
    prev18e_r1C.SRAM0_TUNE_DECODER_DLY = 15; // Default 0x2, 4 bits
    //prev18e_r1C.SRAM0_USE_INVERTER_SA= 1; 
	*REG_SRAM0_TUNE = prev18e_r1C.as_int;
  
    prev18e_r1D.SRAM1_TUNE_ASO_DLY = 31; // Default 0x0, 5 bits
    prev18e_r1D.SRAM1_TUNE_DECODER_DLY = 15; // Default 0x2, 4 bits
    //prev18e_r1D.SRAM1_USE_INVERTER_SA= 1; 
	*REG_SRAM1_TUNE = prev18e_r1D.as_int;
  
  
    //Enumerate & Initialize Registers
    stack_state = STK_IDLE; 	//0x0;
    enumerated = 0x4148116E; // 0x4148 is AH in ascii
    exec_count = 0;
    exec_count_irq = 0;
...
mbus_write_message32(0xA0, *REG0);
mbus_write_message32(0xA1, *GOC_DATA_IRQ);
mbus_write_message32(0xA1, *GOC_DATA_IRQ);
mbus_write_message32(0xA2, *REG_GOCEP_FLAG);
	...
	mbus_write_message32(0xA3, wakeup_data);
	mbus_write_message32(0xA3, wakeup_data_header);
