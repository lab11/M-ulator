...
//                      - cycle9()
//                          Added XO Timer testing with DISABLE_AUTO_RESET=1
...
#define FLAG_XO_SUB_A_0     12
#define FLAG_XO_SUB_A_1     13
#define FLAG_XO_SUB_A_2     14
...
// Select Testing
volatile uint32_t do_cycle0  = 1; // Wakeup through GPIO
volatile uint32_t do_cycle1  = 1; // Wakeup through XO
...
volatile uint32_t do_cycle9  = 1; // Wakeup through XO (with DISABLE_AUTO_RESET=1)
...
void cycle1 (void) {
    if (do_cycle1 == 1) {
        ...
        // Enable the XO pad output
        *REG_XOT_CONFIG |= 0x3; // SEL_PAD_XO_CLK = 2'h3 (CLK_XO_TMR)
        ...
        if (!get_flag(FLAG_XO_SUB_2) && !get_flag(FLAG_XO_SUB_1) && !get_flag(FLAG_XO_SUB_0)) {
          ...
          //*REG_XO_CONF2 = 0x0A348A; // Default Value
            *REG_XO_CONF2 |=  (0x1 << 18); // XO_RESETn = 1
            *REG_XO_CONF2 &= ~(0x1 << 12); // XO_ISOL_CLK_LP = 0
          //*REG_XO_CONF1 = 0x004060; // Default Value
            *REG_XO_CONF1 &= ~(0x1 << 5); // XO_ISOL_CLK_HP = 0
        ...
        // 7: XO Timer: Use it as a normal timer (End of Testing)
          ...
          // Power-gate and Isolate the XO driver (Default value)
          //*REG_XO_CONF1 = 0x004060; // Default Value
          //*REG_XO_CONF2 = 0x0A348A; // Default Value
...
void cycle9 (void) { 
    if (do_cycle9 == 1) {
        arb_debug_reg(0x49, 0x00000000);

        //----------------------------------------------------------------------------------------------------------    
        // 0: XO Timer: Sleep Timer with DISABLE_AUTO_RESET=1 (Part I)
        if (!get_flag(FLAG_XO_SUB_A_2) && !get_flag(FLAG_XO_SUB_A_1) && !get_flag(FLAG_XO_SUB_A_0)) {

            set_flag(FLAG_XO_SUB_A_2, 0);
            set_flag(FLAG_XO_SUB_A_1, 0);
            set_flag(FLAG_XO_SUB_A_0, 1);

            // Configure XO
            // In reality, the XO driver must be properly configured and started
            arb_debug_reg(0x49, 0x00000001);
          //*REG_XO_CONF2 = 0x0A348A; // Default Value
            *REG_XO_CONF2 |=  (0x1 << 18); // XO_RESETn = 1
            *REG_XO_CONF2 &= ~(0x1 << 12); // XO_ISOL_CLK_LP = 0
          //*REG_XO_CONF1 = 0x004060; // Default Value
            *REG_XO_CONF1 &= ~(0x1 << 5); // XO_ISOL_CLK_HP = 0
            ...
            set_wakeup_timer (/*timestamp*/100, /*irq_en*/0, /*reset*/1);

            // XO Timer Configuration
            *REG_XOT_CONFIG =  ((0x0 << 23)   // XOT_ENABLE
                               |(0x0 << 22)   // XOT_MODE
                               |(0x1 << 21)   // XOT_WREQ_EN
                               |(0x0 << 20)   // XOT_IRQ_EN
                               |(0x0 << 19)   // XOT_FORCE_COUT
                               |(0x1 << 18)   // XOT_DISABLE_AUTO_RESET
                               |(0x3 <<  0)   // SEL_PAD_XO_CLK
                               );

            // Enable XO Timer and Start the counter
            *REG_XOT_CONFIG |= (0x1 << 23); // XOT_ENABLE = 1;
            start_xo_cnt();

            // Set the XO Timer Threshold
            *XOT_SAT = 1000;

            arb_debug_reg(0x49, (0x10<<24) | *XOT_VAL_ASYNC);
            arb_debug_reg(0x49, (0x20<<24) | *XOT_VAL);
            arb_debug_reg(0x49, (0x30<<24) | *XOT_SAT);

            mbus_sleep_all();

        }
        //----------------------------------------------------------------------------------------------------------    
        // 1: XO Timer: Sleep Timer with DISABLE_AUTO_RESET=1 (Part II)
        else if (!get_flag(FLAG_XO_SUB_A_2) && !get_flag(FLAG_XO_SUB_A_1) && get_flag(FLAG_XO_SUB_A_0)) {

            set_flag(FLAG_XO_SUB_A_2, 0);
            set_flag(FLAG_XO_SUB_A_1, 1);
            set_flag(FLAG_XO_SUB_A_0, 0);

            arb_debug_reg(0x49, (0x10<<24) | *XOT_VAL_ASYNC);
            arb_debug_reg(0x49, (0x20<<24) | *XOT_VAL);
            arb_debug_reg(0x49, (0x30<<24) | *XOT_SAT);

            arb_debug_reg(0x49, 0x00000002);
        ...
        //----------------------------------------------------------------------------------------------------------    
        // 3: XO Timer: Sleep Timer with DISABLE_AUTO_RESET=1 (Part III)
        else if (!get_flag(FLAG_XO_SUB_A_2) && get_flag(FLAG_XO_SUB_A_1) && !get_flag(FLAG_XO_SUB_A_0)) {
        ...
        //----------------------------------------------------------------------------------------------------------    
        // 4: XO Timer: Sleep Timer with DISABLE_AUTO_RESET=1 (Part IV)
        else if (!get_flag(FLAG_XO_SUB_A_2) && get_flag(FLAG_XO_SUB_A_1) && get_flag(FLAG_XO_SUB_A_0)) {

            set_flag(FLAG_XO_SUB_A_2, 1);
            set_flag(FLAG_XO_SUB_A_1, 0);
            set_flag(FLAG_XO_SUB_A_0, 0);
        ...
        //----------------------------------------------------------------------------------------------------------    
        // 5: XO Timer: Sleep Timer with DISABLE_AUTO_RESET=1 (End of Testing)
        else {
