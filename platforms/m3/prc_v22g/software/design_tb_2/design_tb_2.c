...
volatile uint32_t do_cycle3  = 1; // Pending M0 IRQ Handling
...
void snt_tmr_reset (void) {
    ...
    SNTv5_REG_TMR_CNF.as_int    = SNTv5_R08_DEFAULT_AS_INT;
    mbus_remote_register_write(SNT_ADDR, 0x08, SNTv5_REG_TMR_CNF.as_int);
}

void snt_wup_reset (void) {
    // Reset registers to default
    SNTv5_REG_WUP_CNF.as_int    = SNTv5_R17_DEFAULT_AS_INT;
    SNTv5_REG_WUP_THRESE.as_int = SNTv5_R19_DEFAULT_AS_INT;
    SNTv5_REG_WUP_THRES.as_int  = SNTv5_R1A_DEFAULT_AS_INT;
...
void snt_wup_start (uint32_t threshold, uint32_t auto_reset, uint32_t num_regs) {
    ...
    SNTv5_REG_WUP_THRESE.WUP_THRESHOLD_EXT = (threshold >> 24);
    mbus_remote_register_write(SNT_ADDR, 0x19, SNTv5_REG_WUP_THRESE.as_int);

    SNTv5_REG_WUP_THRES.WUP_THRESHOLD = threshold & 0x00FFFFFF;
    mbus_remote_register_write(SNT_ADDR, 0x1A, SNTv5_REG_WUP_THRES.as_int);

    SNTv5_REG_WUP_CNF.WUP_INT_RPLY_REG_ADDR   = 0x04;
    SNTv5_REG_WUP_CNF.WUP_INT_RPLY_SHORT_ADDR = 0x10;
    SNTv5_REG_WUP_CNF.WUP_INT_RPLY_NUM_REGS_1 = num_regs-1;
    SNTv5_REG_WUP_CNF.WUP_ENABLE_CLK_SLP_OUT  = 0;
    SNTv5_REG_WUP_CNF.WUP_CLK_SEL             = 0;
    SNTv5_REG_WUP_CNF.WUP_AUTO_RESET          = auto_reset;
    SNTv5_REG_WUP_CNF.WUP_IRQ_EN              = 1;
...
void cycle2 (void) {
    if (do_cycle2 == 1) { 
        ...
        if (get_mb_flag(FLAG_SNTWUP, FLAG_SNTWUP_LEN) == 0) {
            ...
            snt_wup_reset();
            snt_tmr_start();
            snt_wup_start (/*threshold*/37, /*auto_reset*/0, /*num_regs*/1);
        ...
        // WAKEUP_REQ happens after the sleep msg
            ...
            snt_wup_stop();
            snt_wup_start(/*threshold*/16, /*auto_reset*/1, /*num_regs*/1);
    ...
    if (do_cycle3 == 1) { 
        ...
        *NVIC_ISER = (0x1 << IRQ_REG4) | (0x1 << IRQ_REG5);
        ...
        if (get_mb_flag(FLAG_PENDIRQ, FLAG_PENDIRQ_LEN) == 0) {
            ...
            snt_wup_reset();
            snt_tmr_start();
            snt_wup_start (/*threshold*/3, /*auto_reset*/0, /*num_regs*/1);
        ...
        // M0 IRQ generated right before sending the sleep message
            ...
            snt_wup_stop();
            snt_wup_start(/*threshold*/32, /*auto_reset*/0, /*num_regs*/1);
