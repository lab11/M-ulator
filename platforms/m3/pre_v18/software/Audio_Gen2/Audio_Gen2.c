//*******************************************************************
//Author: Sechang Oh
//Description: NGC Audio Gen1 
//          v1.0: created   09/12/2019
//*******************************************************************
#include "PREv18.h"
#include "ADOv6VA_RF.h"
#include "PMUv7_RF.h"
#include "mbus.h"
...
volatile adov6va_r00_t adov6va_r00 = ADOv6VA_R00_DEFAULT;
volatile adov6va_r04_t adov6va_r04 = ADOv6VA_R04_DEFAULT;
volatile adov6va_r07_t adov6va_r07 = ADOv6VA_R07_DEFAULT;
volatile adov6va_r0B_t adov6va_r0B = ADOv6VA_R0B_DEFAULT;
volatile adov6va_r0D_t adov6va_r0D = ADOv6VA_R0D_DEFAULT;
volatile adov6va_r0E_t adov6va_r0E = ADOv6VA_R0E_DEFAULT;
volatile adov6va_r0F_t adov6va_r0F = ADOv6VA_R0F_DEFAULT;
volatile adov6va_r10_t adov6va_r10 = ADOv6VA_R10_DEFAULT;
volatile adov6va_r11_t adov6va_r11 = ADOv6VA_R11_DEFAULT;
volatile adov6va_r12_t adov6va_r12 = ADOv6VA_R12_DEFAULT;
volatile adov6va_r13_t adov6va_r13 = ADOv6VA_R13_DEFAULT;
volatile adov6va_r14_t adov6va_r14 = ADOv6VA_R14_DEFAULT;
volatile adov6va_r15_t adov6va_r15 = ADOv6VA_R15_DEFAULT;
volatile adov6va_r16_t adov6va_r16 = ADOv6VA_R16_DEFAULT;
volatile adov6va_r17_t adov6va_r17 = ADOv6VA_R17_DEFAULT;
volatile adov6va_r18_t adov6va_r18 = ADOv6VA_R18_DEFAULT;
volatile adov6va_r19_t adov6va_r19 = ADOv6VA_R19_DEFAULT;
volatile adov6va_r1A_t adov6va_r1A = ADOv6VA_R1A_DEFAULT;
volatile adov6va_r1B_t adov6va_r1B = ADOv6VA_R1B_DEFAULT;
volatile adov6va_r1C_t adov6va_r1C = ADOv6VA_R1C_DEFAULT;
...
inline static void pmu_set_active_clk(uint8_t r, uint8_t l, uint8_t base, uint8_t l_1p2){
    ...
    mbus_remote_register_write(PMU_ADDR,0x16, 
            ( (0 << 19) // Enable PFM even during periodic reset
              ...
              | (3) //base)      // Floor frequency base (0-63)
            ));
    delay(MBUS_DELAY);
    mbus_remote_register_write(PMU_ADDR,0x16, 
            ( (0 << 19) // Enable PFM even during periodic reset
...
inline static void ado_initialization(void){
    ...
    adov6va_r07.as_int = 0;
    adov6va_r0B.as_int = 0;
    adov6va_r10.as_int = 0;

    adov6va_r10.VAD_ADC_DOUT_ISOL = 1;
    mbus_remote_register_write(ADO_ADDR, 0x10, adov6va_r10.as_int);
 
    // AFE Initialization
    adov6va_r15.IB_GEN_CLK_EN = 1;
    adov6va_r15.LS_CLK_EN_1P2 = 1;
    adov6va_r15.REC_PGA_BWCON = 24;//24;
    adov6va_r15.REC_PGA_CFBADD = 1;//0;
    adov6va_r15.REC_PGA_GCON = 8;//2;
    mbus_remote_register_write(ADO_ADDR, 0x15, adov6va_r15.as_int); //1F8622

    adov6va_r16.IBC_MONAMP = 0;
    adov6va_r16.IBC_REC_LNA = 6;
    adov6va_r16.IBC_REC_PGA = 11;
    adov6va_r16.IBC_VAD_LNA = 7;
    adov6va_r16.IBC_VAD_PGA = 5;
    mbus_remote_register_write(ADO_ADDR, 0x16, adov6va_r16.as_int);//0535BD
 
    adov6va_r18.REC_LNA_N1_CON = 2;
    adov6va_r18.REC_LNA_N2_CON = 2;
    adov6va_r18.REC_LNA_P1_CON = 3;
    adov6va_r18.REC_LNA_P2_CON = 3;
    mbus_remote_register_write(ADO_ADDR, 0x18, adov6va_r18.as_int);//0820C3

    adov6va_r19.REC_PGA_P1_CON = 3;
    mbus_remote_register_write(ADO_ADDR, 0x19, adov6va_r19.as_int);//007064

    adov6va_r1B.VAD_LNA_N1_LCON = 5;
    adov6va_r1B.VAD_LNA_N2_LCON = 6;
    adov6va_r1B.VAD_LNA_P1_CON = 3;
    adov6va_r1B.VAD_LNA_P2_CON = 2;
    mbus_remote_register_write(ADO_ADDR, 0x1B, adov6va_r1B.as_int);//0A6062

    adov6va_r1C.VAD_PGA_N1_LCON = 0;
    adov6va_r1C.VAD_PGA_N1_MCON = 1;
    mbus_remote_register_write(ADO_ADDR, 0x1C, adov6va_r1C.as_int);//00079C

    adov6va_r11.LDO_CTRL_VREFLDO_VOUT_1P4HP = 4;
    mbus_remote_register_write(ADO_ADDR, 0x11, adov6va_r11.as_int);//84CAC9

    adov6va_r12.LDO_CTRL_VREFLDO_VOUT_0P6LP = 3;
    adov6va_r12.LDO_CTRL_VREFLDO_VOUT_0P9HP = 3;
    adov6va_r12.LDO_CTRL_VREFLDO_VOUT_1P4LP = 4;
    mbus_remote_register_write(ADO_ADDR, 0x12, adov6va_r12.as_int);//828283
    ...
    for(i= 0; i<LUT_DATA_LENGTH; i++){
        adov6va_r07.DSP_LUT_WR_ADDR = i;
        adov6va_r07.DSP_LUT_DATA_IN = LUT_DATA[i];
        mbus_remote_register_write(ADO_ADDR, 0x07, adov6va_r07.as_int);
    ...
    for(i= 0; i<PHS_DATA_LENGTH; i++){
        adov6va_r0B.DSP_PHS_WR_ADDR_FS = i;
        adov6va_r0B.DSP_PHS_DATA_IN_FS = PHS_DATA[i];
        mbus_remote_register_write(ADO_ADDR, 0x0B, adov6va_r0B.as_int);
        mbus_remote_register_write(ADO_ADDR, 0x0C, 0x000001);
        mbus_remote_register_write(ADO_ADDR, 0x0C, 0x000000);
    }
    
    //N_DCT: 128 points DCT 
    adov6va_r00.DSP_N_DCT = 2;
    mbus_remote_register_write(ADO_ADDR, 0x00, adov6va_r00.as_int);
    
    //WAKEUP REQ EN
    adov6va_r0E.DSP_WAKEUP_REQ_EN = 1;
    mbus_remote_register_write(ADO_ADDR, 0x0E, adov6va_r0E.as_int);
}


inline static void digital_set_mode(uint8_t mode){
    if(mode == 1){      //LP DSP
        adov6va_r0D.DSP_HP_ADO_GO = 0;
        adov6va_r0D.DSP_HP_DNN_GO = 0;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03BCB5
        adov6va_r0D.DSP_HP_RESETN = 0;
        adov6va_r0D.DSP_DNN_HP_MODE_EN = 0;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03BC15
        
        adov6va_r0D.DSP_DNN_RESETN_RF = 0;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03BC05
        adov6va_r0D.DSP_DNN_CLKENB_RF = 1;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03BC0D
        adov6va_r0D.DSP_DNN_ISOLATEN_RF = 0;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03BC09
        adov6va_r0D.DSP_DNN_PG_SLEEP_RF = 1;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03BC0B
        adov6va_r0D.DSP_DNN_CTRL_RF_SEL = 0;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03BC0A
        
        adov6va_r04.DSP_P2S_RESETN = 0;
        mbus_remote_register_write(ADO_ADDR, 0x04, adov6va_r04.as_int);//E00040
        adov6va_r04.DSP_P2S_RESETN = 1;
        mbus_remote_register_write(ADO_ADDR, 0x04, adov6va_r04.as_int);//E00040
        
        adov6va_r0D.DSP_LP_RESETN = 1;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03BC4A
        }
    else if(mode == 2){ // HP DSP
        adov6va_r0D.DSP_LP_RESETN = 0;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03B80A

        adov6va_r0D.DSP_DNN_CTRL_RF_SEL = 1;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03B80B
        adov6va_r0D.DSP_DNN_PG_SLEEP_RF = 0;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03B809
        adov6va_r0D.DSP_DNN_ISOLATEN_RF = 1;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03B80D
        adov6va_r0D.DSP_DNN_CLKENB_RF = 0;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03B805
        adov6va_r0D.DSP_DNN_RESETN_RF = 1;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03B815
        
        adov6va_r0D.DSP_DNN_HP_MODE_EN = 1;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03B835
        ...
        adov6va_r0D.DSP_HP_RESETN = 1;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03B8B5
        adov6va_r0D.DSP_HP_ADO_GO = 1;
        adov6va_r0D.DSP_HP_DNN_GO = 1;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03BBB5
        }
        ...
        adov6va_r04.DSP_P2S_MON_EN = 0;
        mbus_remote_register_write(ADO_ADDR, 0x04, adov6va_r04.as_int);//A00040
...
inline static void afe_set_mode(uint8_t mode){
    if(mode == 1){      //LP AFE
        ...
        //adov6va_r16.IBC_REC_LNA = 0;//6;
        //adov6va_r16.IBC_REC_PGA = 0;//11;
        //mbus_remote_register_write(ADO_ADDR, 0x16, adov6va_r16.as_int);

        adov6va_r0D.REC_ADC_RESETN = 0;
        adov6va_r0D.REC_ADCDRI_EN = 0;
        adov6va_r0D.REC_LNA_AMPEN = 0;
        adov6va_r0D.REC_LNA_AMPSW_EN = 0;
        adov6va_r0D.REC_LNA_FSETTLE = 1;
        adov6va_r0D.REC_LNA_OUTSHORT_EN = 0;
        adov6va_r0D.REC_PGA_AMPEN = 0;
        adov6va_r0D.REC_PGA_OUTSHORT_EN = 0;
        adov6va_r0D.VAD_LNA_AMPSW_EN = 1;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);
        
        adov6va_r13.VAD_PGA_FSETTLE = 1;
        adov6va_r13.VAD_PGA_OUTSHORT_EN = 0;
        adov6va_r13.VAD_LNA_FSETTLE = 0;
        adov6va_r13.VAD_LNA_OUTSHORT_EN = 0;
        adov6va_r13.LDO_PG_IREF = 0;
        adov6va_r13.LDO_PG_VREF_0P6LP = 0;
        adov6va_r13.LDO_PG_LDOCORE_0P6LP = 0;
        adov6va_r13.LDO_PG_VREF_1P4LP = 0;
        adov6va_r13.LDO_PG_LDOCORE_1P4LP = 0;
        adov6va_r13.LDO_PG_VREF_1P4HP = 1;
        adov6va_r13.LDO_PG_LDOCORE_1P4HP = 1;
        adov6va_r13.LDO_PG_VREF_0P9HP = 1;
        adov6va_r13.LDO_PG_LDOCORE_0P9HP = 1;
        mbus_remote_register_write(ADO_ADDR, 0x13, adov6va_r13.as_int);

//        adov6va_r0F.VAD_ADC_SDLY_SEL = 0;
//        adov6va_r0F.VAD_ADC_DLY_SEL = 4;
        adov6va_r0F.VAD_ADC_RESET = 0;
        mbus_remote_register_write(ADO_ADDR, 0x0F, adov6va_r0F.as_int);

        adov6va_r14.VAD_ADCDRI_EN = 1;
        adov6va_r14.VAD_LNA_AMPEN = 1;
        adov6va_r14.VAD_PGA_AMPEN = 1;
        adov6va_r14.VAD_PGA_BWCON = 13;//13;
        adov6va_r14.VAD_PGA_GCON = 8;//8;
        mbus_remote_register_write(ADO_ADDR, 0x14, adov6va_r14.as_int);

        ////VAD MON setting can come here
        //adov6va_r1A.VAD_MONSEL = 1;
        //adov6va_r1A.VAD_MONBUF_EN = 1;
        //adov6va_r1A.VAD_REFMONBUF_EN = 0;
        //adov6va_r1A.VAD_ADCDRI_OUT_OV_EN = 1;
        //mbus_remote_register_write(ADO_ADDR, 0x1A, adov6va_r1A.as_int);
        
        ////REC MON setting (debug)
        adov6va_r17.REC_MONSEL = 0;
        adov6va_r17.REC_MONBUF_EN = 0;
        //adov6va_r17.REC_REFMONBUF_EN = 0;
        mbus_remote_register_write(ADO_ADDR, 0x17, adov6va_r17.as_int);

        adov6va_r10.DO_LP_HP_SEL = 0;    //0: LP, 1: HP
        adov6va_r10.DIO_OUT_EN = 1;
        adov6va_r10.VAD_ADC_DOUT_ISOL = 0;
        mbus_remote_register_write(ADO_ADDR, 0x10, adov6va_r10.as_int);

        adov6va_r04.DSP_CLK_MON_SEL = 4; //LP CLK mon
        mbus_remote_register_write(ADO_ADDR, 0x04, adov6va_r04.as_int);
    }
        ...
        adov6va_r13.LDO_PG_VREF_1P4LP = 0;
        adov6va_r13.LDO_PG_LDOCORE_1P4LP = 0;
        adov6va_r13.LDO_PG_VREF_1P4HP = 0;
        adov6va_r13.LDO_PG_LDOCORE_1P4HP = 0;
        adov6va_r13.LDO_PG_VREF_0P9HP = 0;
        adov6va_r13.LDO_PG_LDOCORE_0P9HP = 0;
        mbus_remote_register_write(ADO_ADDR, 0x13, adov6va_r13.as_int);

        adov6va_r0D.REC_ADC_RESETN = 1;
        adov6va_r0D.REC_ADCDRI_EN = 1;
        adov6va_r0D.REC_LNA_AMPEN = 1;
        adov6va_r0D.REC_LNA_AMPSW_EN = 1;
        adov6va_r0D.REC_LNA_FSETTLE = 1;
        adov6va_r0D.REC_LNA_OUTSHORT_EN = 0;
        adov6va_r0D.REC_PGA_AMPEN = 1;
        adov6va_r0D.REC_PGA_OUTSHORT_EN = 0;
        adov6va_r0D.VAD_LNA_AMPSW_EN = 0;
        mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);
        delay(MBUS_DELAY*10);
        //adov6va_r0D.REC_LNA_FSETTLE = 0;
        //mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);

        adov6va_r10.DO_LP_HP_SEL = 1;    //HP
        adov6va_r10.DIO_OUT_EN = 1;
        mbus_remote_register_write(ADO_ADDR, 0x10, adov6va_r10.as_int);

        adov6va_r04.DSP_CLK_MON_SEL = 2; //HP clock mon
        mbus_remote_register_write(ADO_ADDR, 0x04, adov6va_r04.as_int);
    }
    else{       // AFE off
        ...
        adov6va_r13.VAD_PGA_FSETTLE = 1;
        adov6va_r13.VAD_LNA_FSETTLE = 1;
        adov6va_r13.LDO_PG_IREF = 0;
        adov6va_r13.LDO_PG_VREF_0P6LP = 1;
        adov6va_r13.LDO_PG_LDOCORE_0P6LP = 1;
        adov6va_r13.LDO_PG_VREF_1P4LP = 1;
        adov6va_r13.LDO_PG_LDOCORE_1P4LP = 1;
        adov6va_r13.LDO_PG_VREF_1P4HP = 1;
        adov6va_r13.LDO_PG_LDOCORE_1P4HP = 1;
        adov6va_r13.LDO_PG_VREF_0P9HP = 1;
        adov6va_r13.LDO_PG_LDOCORE_0P9HP = 1;
        ...
        adov6va_r0F.VAD_ADC_RESET = 1;
        mbus_remote_register_write(ADO_ADDR, 0x0F, adov6va_r0F.as_int);

        adov6va_r14.VAD_ADCDRI_EN = 0;
        adov6va_r14.VAD_LNA_AMPEN = 0;
        adov6va_r14.VAD_PGA_AMPEN = 0;
        mbus_remote_register_write(ADO_ADDR, 0x14, adov6va_r14.as_int);

        adov6va_r10.DIO_OUT_EN = 0;
        //adov6va_r10.DIO_DIR_IN1OUT0 = 1;
        adov6va_r10.VAD_ADC_DOUT_ISOL = 1;
        mbus_remote_register_write(ADO_ADDR, 0x10, adov6va_r10.as_int);
        ////VAD MON setting can come here
        //adov6va_r1A.VAD_MONSEL = 1;
        ...
        ////REC MON setting (debug)
        //adov6va_r17.REC_MONSEL = 1;
        //adov6va_r17.REC_MONBUF_EN = 1;
        //adov6va_r17.REC_REFMONBUF_EN = 0;
        //mbus_remote_register_write(ADO_ADDR, 0x17, adov6va_r17.as_int);
...
static void operation_init(void){
    ...
    *REG_CPS = 0x7; //PG control [2]=1: Test 1.2V on,  [0]=1: Test 0.6V on
    ...
    ado_initialization(); //ADOv6VA initialization
...
void handler_ext_int_gocep(void) { // GOCEP
    *NVIC_ICPR = (0x1 << IRQ_GOCEP);
    ...
    if(data_cmd == 0x01){       // Charge pump control
        if(data_val==1){        //ON
            adov6va_r14.CP_CLK_EN_1P2 = 1;
            adov6va_r14.CP_CLK_DIV_1P2 = 0;
            adov6va_r14.CP_VDOWN_1P2 = 0;
            mbus_remote_register_write(ADO_ADDR, 0x14, adov6va_r14.as_int);
            delay(CP_DELAY); 
            adov6va_r14.CP_VDOWN_1P2 = 1;
            mbus_remote_register_write(ADO_ADDR, 0x14, adov6va_r14.as_int);
        }
        else{                   //OFF
            adov6va_r14.CP_CLK_EN_1P2 = 0;
            adov6va_r14.CP_CLK_DIV_1P2 = 3;
            mbus_remote_register_write(ADO_ADDR, 0x14, adov6va_r14.as_int);
        }
    }
    else if(data_cmd == 0x02){  // SRAM Programming mode
        if(data_val==1){    //Enter
            adov6va_r0E.DSP_DNN_DBG_MODE_EN = 1;
            mbus_remote_register_write(ADO_ADDR, 0x0E, adov6va_r0E.as_int);//000054
            adov6va_r0D.DSP_DNN_CTRL_RF_SEL = 1;
            mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03B80B
            adov6va_r0D.DSP_DNN_PG_SLEEP_RF = 0;
            mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03B809
            ...
            adov6va_r0D.DSP_DNN_CTRL_RF_SEL = 0;
            mbus_remote_register_write(ADO_ADDR, 0x0D, adov6va_r0D.as_int);//03B80A
            adov6va_r0E.DSP_DNN_DBG_MODE_EN = 0;
            mbus_remote_register_write(ADO_ADDR, 0x0E, adov6va_r0E.as_int);//000050
        }
    }
    else if(data_cmd == 0x03){  // DSP Monitoring Configure
        //adov6va_r0E.DSP_MIX_MON_EN = 1;
        //mbus_remote_register_write(ADO_ADDR, 0x0E, adov6va_r0E.as_int);//000150
        //adov6va_r00.DSP_FE_SEL_EXT = 1;
        //mbus_remote_register_write(ADO_ADDR, 0x00, adov6va_r00.as_int);//9F1B28
        adov6va_r04.DSP_P2S_MON_EN = 1;
        adov6va_r04.DSP_CLK_MON_SEL = 4;
        mbus_remote_register_write(ADO_ADDR, 0x04, adov6va_r04.as_int);//A00040
        adov6va_r04.DSP_P2S_RESETN = 1;
        mbus_remote_register_write(ADO_ADDR, 0x04, adov6va_r04.as_int);//E00040
        //adov6va_r10.DIO_OUT_EN = 1;
        //adov6va_r10.DIO_IN_EN = 0;
        //adov6va_r10.DIO_DIR_IN1OUT0 = 0;
        //mbus_remote_register_write(ADO_ADDR, 0x10, adov6va_r10.as_int);//000008
    }
        ...
        *REG_CPS = 0x7 & data_val; //PG control [2]=1: Test 1.2V on,  [0]=1: Test 0.6V on
    }
