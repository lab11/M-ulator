...
volatile uint32_t mrr_cfo_count [5] = {};
...
static void send_radio_data_mrr_sub1_count(){
	...
	// FIXME: Read Current CFO counter value
    set_halt_until_mbus_rx();
    mbus_remote_register_read(MRR_ADDR,0x20,1); // CNT_VALUE_EXT
    set_halt_until_mbus_tx();
...
static void send_radio_data_mrr_count(uint32_t last_packet, uint8_t radio_packet_prefix, uint32_t radio_data){
	// Sends 192 bit packet, of which 96b is actual data
	// MRR REG_9: reserved for header
	// MRR REG_A: reserved for header
	// MRR REG_B: reserved for header
	...
	while (count < num_packets){
		...
		mrrv10_r01.MRR_TRX_CAP_ANTN_TUNE_FINE = mrr_cfo_val_fine;
		mbus_remote_register_write(MRR_ADDR,0x01,mrrv10_r01.as_int);
		send_radio_data_mrr_sub1_count();

		//FIXME: Store cfo count
    	mrr_cfo_count[count] = *REG1;
...
static void operation_init(void){
	...
	*REG_SRAM0_TUNE = prev20_r1C.as_int;
    ...
    enumerated = 0x5453104f; // 0x5453 is TS in ascii
    exec_count = 0;
    wakeup_count = 0;
    exec_count_irq = 0;
...
static void operation_sns_run(void){
    ...
    if (stack_state == STK_IDLE){
        ...
        if (meas_count < NUM_TEMP_MEAS){    
            ...
            // transmit the data
			// FIXME
            send_radio_data_mrr_count(0,0x1,(TEMP_CALIB_B<<16) | (temp_storage_latest&0xFFFF));

			disable_timerwd();
			delay(RADIO_PACKET_DELAY);
			send_radio_data_mrr_count(0,4,mrr_cfo_count[0]);
			delay(RADIO_PACKET_DELAY);
			send_radio_data_mrr_count(0,4,mrr_cfo_count[1]);
			delay(RADIO_PACKET_DELAY);
			send_radio_data_mrr_count(0,4,mrr_cfo_count[2]);
			delay(RADIO_PACKET_DELAY);
			send_radio_data_mrr_count(0,4,mrr_cfo_count[3]);
			delay(RADIO_PACKET_DELAY);
			send_radio_data_mrr_count(1,4,mrr_cfo_count[4]);
...
*/
    ...
    }else if(wakeup_data_header == 0x54){
		// Debug trigger for MRR CFO counting
		radio_power_on();
		disable_timerwd();
		send_radio_data_mrr_count(0,4,0x54);
		delay(RADIO_PACKET_DELAY*10);
		send_radio_data_mrr_count(0,4,mrr_cfo_count[0]);
		delay(RADIO_PACKET_DELAY*10);
		send_radio_data_mrr_count(0,4,mrr_cfo_count[1]);
		delay(RADIO_PACKET_DELAY*10);
