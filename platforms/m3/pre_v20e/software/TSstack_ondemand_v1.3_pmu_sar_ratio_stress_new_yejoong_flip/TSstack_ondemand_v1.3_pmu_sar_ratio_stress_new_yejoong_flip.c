...
volatile int32_t pmu_adc_out;
volatile int32_t pmu_adc_offset;
...
//************************************
// SAR RATIO change (in Active)
//************************************

//************************************
// CAUTION
// 1. You should have a valid ADC offset measurement before running set_sar_ratio(),
//    although you may not need to worry about this since the ADC automatically measures the offset right after booting.
// 1. Bit[1] in PMU Registers 59 (0x3B) and 60 (0x3C) must stay at 1 during the system lifetime.
// 2. Bit[7] in PMU Reigster 5 (0x05) must stay at 0 during the system lifetime if you use set_sar_ratio() function.

#define DEBUG_SET_SAR_RATIO // Display a few debug messages
...
void pmu_set_sar_ratio (uint32_t sar_ratio) {
    ...
    // Debug
    #ifdef DEBUG_SET_SAR_RATIO
        mbus_write_message32(0xD8, 0x01000000);
    #endif
    ...
    // Debug
    #ifdef DEBUG_SET_SAR_RATIO
        mbus_write_message32(0xD8, ((0x03 << 24) | adc_offset));
    #endif
    ...
    // Debug
    #ifdef DEBUG_SET_SAR_RATIO
        mbus_write_message32(0xD8, ((0x04 << 24) | 0x0));
    #endif
    ...
    // Set SAR_RATIO_OFFSET_DOWN=0 to enable the ratio override in sleep.
    //pmu_reg_write (6, // 0x7 (SAR_RATIO_OFFSET_DOWN_ACTIVE)
    //    ( (0 << 0)    // SAR_RATIO_OFFSET_DOWN_ACTIVE
    //));

    // Debug
    #ifdef DEBUG_SET_SAR_RATIO
        mbus_write_message32(0xD8, ((0x05 << 24) | 0x0));
    #endif
    ...
    // Debug
    #ifdef DEBUG_SET_SAR_RATIO
        mbus_write_message32(0xD8, ((0x06 << 24) | sar_ratio));
    #endif
    ...
    // Debug
    #ifdef DEBUG_SET_SAR_RATIO
        mbus_write_message32(0xD8, ((0x07 << 24) | 0x0));
    #endif

        pmu_reg_write(0x00,61); // state main
        // Debugg
        #ifdef DEBUG_SET_SAR_RATIO
            mbus_write_message32(0xD8, ((0xD5 << 24) | *REG0));
        #endif
    ...
    // Debug
    #ifdef DEBUG_SET_SAR_RATIO
        mbus_write_message32(0xD8, ((0x08 << 24) | 0x0));
    #endif
    ...
    do {
        ...
        // Debugg
        #ifdef DEBUG_SET_SAR_RATIO
            mbus_write_message32(0xD8, ((0x09 << 24) | pmu_sar_ratio));
        #endif
        ...
        // Debugg
        #ifdef DEBUG_SET_SAR_RATIO
            mbus_write_message32(0xD8, ((0xD1 << 24) | pmu_adc_out));
        #endif

        pmu_reg_write(0x00,65); //adc_state
        // Debugg
        #ifdef DEBUG_SET_SAR_RATIO
            mbus_write_message32(0xD8, ((0xD2 << 24) | *REG0));
        #endif

        pmu_reg_write(0x00,59); //desired state sleep
        // Debugg
        #ifdef DEBUG_SET_SAR_RATIO
            mbus_write_message32(0xD8, ((0xD3 << 24) | *REG0));
        #endif

        pmu_reg_write(0x00,60); // desired state active
        // Debugg
        #ifdef DEBUG_SET_SAR_RATIO
            mbus_write_message32(0xD8, ((0xD4 << 24) | *REG0));
        #endif
    ...
    // Debug
    #ifdef DEBUG_SET_SAR_RATIO
        mbus_write_message32(0xD8, ((0x0A << 24) | 0x0));
    #endif
    ...
    // Debug
    #ifdef DEBUG_SET_SAR_RATIO
        mbus_write_message32(0xD8, ((0x0B << 24) | 0x0));
    #endif
    ...
    // Debug
    #ifdef DEBUG_SET_SAR_RATIO
        mbus_write_message32(0xD8, ((0x0C << 24) | 0x0));
    #endif
...
inline static void pmu_adc_disable(){
	...
	pmu_reg_write(0x3B,
		...
		| (1 << 17) //state_vdd_1p2_turned_on
		| (1 << 18) //state_vdd_0P6_turned_on
		| (1 << 19) //state_vbat_read // Turn off for old adc
		| (1 << 20) //state_state_horizon
	));
}
...
static void operation_init(void){
	...
	*REG_SRAM0_TUNE = prev20_r1C.as_int;
    ...
    enumerated = 0x54531030; // 0x5453 is TS in ascii
    exec_count = 0;
    wakeup_count = 0;
    exec_count_irq = 0;
...
//    set_halt_until_mbus_rx();
	...
	// Updated for PMUv9
    pmu_reg_write(0x3A, 
		( (0 << 20) // ignore state_horizon; default 1
		| (0 << 19) // state_vbat_read
		| (0 << 13) // ignore sar_ratio_adjusted; default 0
		| (1 << 12) // ignore adc_adjusted; default 0
		| (0 << 11) // ignore adc_output_ready; default 0
	));
...
int main(){
	...
	*NVIC_ISER = (1 << IRQ_WAKEUP) | (1 << IRQ_GOCEP) | (1 << IRQ_TIMER32) | (1 << IRQ_REG0)| (1 << IRQ_REG1)| (1 << IRQ_REG2)| (1 << IRQ_REG3);
    ...
    }else if(wakeup_data_header == 0xD0){
		// Stress test PMU SAR Ratio Override
		// New FSM burst mode for MRR radio scanning
        // wakeup_data[15:0] = Test duration in minutes
		disable_timerwd();
		*MBCWD_RESET = 1;

		uint32_t pmu_delay = MBUS_DELAY*10; // About 60ms
//		if(wakeup_data_field_0==0x1) pmu_set_sar_ratio(50);
//		else if (wakeup_data_field_0==0x2) pmu_set_sar_ratio(52);
//		else if (wakeup_data_field_0==0x3) pmu_set_sar_ratio(57);
		while (1){
			pmu_set_sar_ratio(50);
			delay(pmu_delay);
			pmu_set_sar_ratio(52);
			delay(pmu_delay);
			pmu_set_sar_ratio(57);
			delay(pmu_delay);
			pmu_set_sar_ratio(50);
			delay(pmu_delay);
			pmu_set_sar_ratio(57);
    ...
    }else if(wakeup_data_header == 0xD1){
        pmu_reg_write(0x00,0x03);
        pmu_adc_out   = *REG0 & 0xFF;
        mbus_write_message32(0xDD, ((0x09 << 24) | pmu_adc_out));

		// Read the current ADC offset
  		pmu_adc_offset = (*REG0 & 0x007F00) >> 8;
        mbus_write_message32(0xDD, ((0x10 << 24) | pmu_adc_offset));
