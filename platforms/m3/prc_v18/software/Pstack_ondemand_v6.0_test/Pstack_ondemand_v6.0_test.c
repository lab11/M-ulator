...
void handler_ext_int_reg0(void) { // REG0
    *NVIC_ICPR = (0x1 << IRQ_REG0);
    //mbus_write_message32(0xB0,*REG0);
}
...
//    mbus_write_message32(0xED, base);
...
inline static void pmu_set_clk_init(){
    ...
    pmu_reg_write(0x05, //default 12'h000
                  ( (0 << 13) // Enables override setting [12] (1'b1)
                    ...
                    | (0 << 8) // Switch input / output power rails for upconversion (1'h0)
                    | (0 << 7) // Enable override setting [6:0] (1'h0)
                    | (47)        // Binary converter's conversion ratio (7'h00), original is 47
                  ));
    pmu_reg_write(0x05, //default 12'h000
                  ( (1 << 13) // Enables override setting [12] (1'b1)
                    | (0 << 12) // Let VDD_CLK always connected to vbat
...
//                #ifdef DEBUG_MBUS_MSG_1
//                mbus_write_message32(0xDD, 0xFFFFFFFF);
//                mbus_write_message32(0xDD, mem_write_data);
//                mbus_write_message32(0xDD, 0xFFFFFFFF);
//                #endif
...
static void operation_goc_trigger_init(void){
    ...
    //////////////////////////////////reset the counter in pmu//////////////////////////////////////
    mbus_remote_register_write(PMU_ADDR,0x53,
                               ( (0 << 23) // 1'h0 W/R VCO_CNT_ENABLE
                                 | (1 << 22) // 1'h0 W/R VCO_CNT_MODE "0: Sleep Measurement, 1: Active Measurement"
                                 | (20000) 	// 22'h0 W/R VCO_CNT_THRESHOLD
                               ));
    delay(MBUS_DELAY*10);
...
//    }else if(wakeup_data_header == 0x06){
    if(wakeup_data_header == 0x06){
        /////////////////////////////////////////////// new code for counter/////////////////////////////////
//        static const uint8_t pmusetting[5][4] = { //(uint8_t r, uint8_t l, uint8_t base, uint8_t l_1p2
//                {0x2,0x0,0x10,0x4}, // default
//                {0x1,0x0,0x10,0x4},
//                {0x2,0x0,0x10,0x4},
//                {0x2,0x1,0x2,0x4},// problematic setting
//                {0x0,0x2,0x10,0x4}// problematic setting
//
//        };
//
//        mbus_write_message32(0xE8, *REG_GOCEP_FLAG);

//        pmu_set_active_clk(pmusetting[i][0], pmusetting[i][1], pmusetting[i][2], pmusetting[i][3]/*V1P2*/);
        uint8_t r = wakeup_data & 0xF;
        uint8_t l = wakeup_data >> 4 & 0xF;
        uint8_t base = wakeup_data >> 16 & 0x1F;
        uint8_t l_1p2 = wakeup_data >> 8 & 0xF;
//        uint8_t r = 0x2; // 0 - 15
//        uint8_t l = 0x1; // 0 - 15
//        uint8_t base = 0x10; // 0 - 31
//        uint8_t l_1p2 = 0x4;
        wfi_timeout_flag = 0;
        // Disable Watchdog
        disable_timerwd();
        ...
        // Disable MBus Watchdog
        *MBCWD_RESET = 1;
        mbus_write_message32((0xE0), r);
        mbus_write_message32((0xE0), l);
        mbus_write_message32((0xE0), base);
        mbus_write_message32((0xE0), l_1p2);
        pmu_set_active_clk(r,l,base,l_1p2/*V1P2*/);
        delay(MBUS_DELAY * 10);
        set_counter();
        mbus_write_message32(0xE1, 0x00000000);
        config_timer32(0xFFFFFFFF, 1, 0, 0); // start the timer
        WFI();
        if (wfi_timeout_flag){
            mbus_write_message32(0xEA, 0xFAFAFAFA);
//            mbus_write_message32(0xFA, i);
            *TIMER32_GO = 0; // stop the timer
            *TIMER32_CNT = 0; // reset the timer
            delay(MBUS_DELAY);
            wfi_timeout_flag = 0;
//            continue;
        }
        *TIMER32_GO = 0; // stop the timer
        mbus_write_message32(0xEE, *TIMER32_CNT);
        delay(MBUS_DELAY);
//        mbus_write_message32(0xFF, i);
        *TIMER32_CNT = 0; // reset the timer
        ...
        uint8_t base = wakeup_data >> 16 & 0x1F;
        uint8_t l_1p2 = wakeup_data >> 8 & 0xF;
		pmu_set_sleep_clk(r,l,base,l_1p2/*V1P2*/);
		operation_sleep_notimer();
	}
    ////////////////////////////////////////////////////end////////////////////////////////////////////
