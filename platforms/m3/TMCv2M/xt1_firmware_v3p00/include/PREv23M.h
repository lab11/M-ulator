...
//Description: PREv23M header file
//*******************************************************************
...
// GOC Data Address
//---------------------------------------------------------
// WARNING: This address conflicts with R_GPIO_* address.
//*********************************************************
...
#define REG_GOC_CONFIG  ((volatile uint32_t *) 0xA0000020)
#define REG_GOC_CHIP_ID ((volatile uint32_t *) 0xA0000020) // For backward-compatibility
...
#define REG_XO_CONF3    ((volatile uint32_t *) 0xA00000E0)
...
#define XOT_SAT         ((volatile uint32_t *) 0xA0001430)
#define XOT_VAL_ASYNC   ((volatile uint32_t *) 0xA0001440)
...
//*********************************************************
#define R_GPIO_DATA     ((volatile uint32_t *) 0x00000034)
#define R_GPIO_DIR      ((volatile uint32_t *) 0x00000038)
#define R_GPIO_IRQ_MASK ((volatile uint32_t *) 0x0000003C)

//*********************************************************
// PRCv23 Family System Registers
//*********************************************************
...
/**
 * @brief   Enable REGn interrupt
 *
 * @param   reg     REG ID which the interrupt is enabled for. Range: [0, 7]
 *
 * @usage   enable_reg_irq(0);
 */
void enable_reg_irq(uint32_t reg);

/**
 * @brief   Disable REGn interrupt
 *
 * @param   reg     REG ID which the interrupt is disabled for. Range: [0, 7]
 *
 * @usage   disable_reg_irq(0);
 */
void disable_reg_irq(uint32_t reg);

/**
 * @brief   Set REGn PEND interrupt
 *
 * @param   reg     REG ID which the PEND interrupt is set for. Range: [0, 7]
 *
 * @usage   set_reg_pend_irq(0);
 */
...
/**
 * @brief   Immediately put CPU in halt. CPU resumes its operation when the event specified in SREG_CONF_HALT occurs.
 *
 * @param   N/A
 *
 * @usage   halt_cpu();
...
//-------------------------------------------------------------------
// Function: set_timeout16_check
// Args    : var - Threshold
// Description:
//          set_timeout16_check() initializes and starts the TIMER 16
...
//-------------------------------------------------------------------
// Function: stop_timeout16_check
// Args    : code - Error code
// Description:
//          stop_timeout16_check() stops the TIMER16.
// Return  : 1  - there is no timeout.
//           0  - there is a timeout.
//-------------------------------------------------------------------
...
// Return  : None
//-------------------------------------------------------------------
volatile uint32_t __wfi_id__;
void set_timeout32_check(uint32_t val);

//-------------------------------------------------------------------
// Function: start_timeout32_check
// Args    : id  - ID to be used for error message
//           var - Threshold
// Description:
//          start_timeout32_check() initializes and starts the TIMER 32
//          for a time-out check
//          This sets __wfi_id__ = id
// Return  : None
//-------------------------------------------------------------------
...
/**
 * @brief   Selects what to output from XO_PAD_CLK
 *	    SEL_PAD_XO_CLK: 2'h0: Tie-Lo, 2'h1: CLK_XO_LP, 2'h2: CLK_XO_HP, 2'h3: CLK_XO_TMR"
 */
void set_xo_pad (uint8_t sel_pad);
 ...
 * @param   auto_reset  Auto Reset Enable
 */
...
// Return  : None
    //--------------------------------------------------------------------------
    // XO Driver (XO_DRV_V6_TSMC180) Start-Up Sequence (Start with HP mode & transitions to LP mode)
    //--------------------------------------------------------------------------
    // RESETn_*     __|*********************************************************
    // EN_HP_LDO    __|***************************|_____________________________
    // EN_HP_IREF   __|***************************|_____________________________
    // EN_HP        __|***************************|_____________________________
    // XO_MODE      **************************|_________________________________
    // ISOL_CLK_HP  **********|_________________|*******************************
    // ISOL_CLK_LP  ******************|_________________________________________
    //                |<--A-->|<--B-->|<--C-->|.|.|<-- Low Power Operation -->
    //--------------------------------------------------------------------------
    // A: ~1s  (XO HP Start-Up): NOTE: You may need more time here due to the weak power-gate switch.
...
// Args    : None
// Description:
//           Changes XO mode from LP to HP
// Return  : None
//-------------------------------------------------------------------
void xo_lp2hp(uint32_t delay_a);

//-------------------------------------------------------------------
// Function: xo_stop
// Args    : None
// Description:
//           Changes XO mode from HP to LP
// Return  : None
//-------------------------------------------------------------------
void xo_hp2lp( void );
