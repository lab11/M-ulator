...
//  Apr 06 2023 - PRCv23M added
//                  - Added set_xo_pad() to support the new PAD_XO_CLK selections.
//                  - Updated xo_start() and xo_stop()
//                  - Added xo_lp2hp() and xo_hp2lp()
//*******************************************************************
...
void set_xo_pad (uint8_t sel_pad) {
    uint32_t regval = *REG_XOT_CONFIG;
    regval &= 0xFFFFFC; // SEL_PAD_XO_CLK: 2'h0: Tie-Lo, 2'h1: CLK_XO_LP, 2'h2: CLK_XO_HP, 2'h3: CLK_XO_TMR"
    regval |= sel_pad; 
    *REG_XOT_CONFIG = regval;
}
...
void xo_start(uint32_t delay_a, uint32_t delay_b) {
    ...
    prev23m_r19_t xo_control;
    xo_control.as_int = *REG_XO_CONF1;

    prev23m_r1A_t xo_control2;
    xo_control2.as_int = *REG_XO_CONF2;

    prev23m_r38_t xo_control3;
    xo_control3.as_int = *REG_XO_CONF3;

    xo_control.XO_RESETn_XO_MODE    = 1;
    xo_control.XO_RESETn_HP_CONF    = 1;
    xo_control.XO_EN_HP_LDO = 1;
    xo_control.XO_EN_HP_IREF = 1;
    *REG_XO_CONF1 = xo_control.as_int;

    xo_control3.XO_EN_HP = 1;
    *REG_XO_CONF3 = xo_control3.as_int;
    ...
    *REG_XO_CONF1 = xo_control.as_int;
    ...
    xo_control2.XO_ISOL_CLK_LP = 0;
    *REG_XO_CONF2 = xo_control2.as_int;
    
    delay(100); // Delay C (~1ms; SCN Output Generation)

    xo_control.XO_XO_MODE = 0;
    *REG_XO_CONF1 = xo_control.as_int;
    
    xo_control.XO_ISOL_CLK_HP = 1;
    *REG_XO_CONF1 = xo_control.as_int;

    xo_control3.XO_EN_HP = 0;
    *REG_XO_CONF3 = xo_control3.as_int;

    xo_control.XO_EN_HP_LDO = 0;
    xo_control.XO_EN_HP_IREF = 0;
    *REG_XO_CONF1 = xo_control.as_int;
...
void xo_stop( void ) {
    ...
    xo_control2.XO_ISOL_CLK_LP = 1;
    *REG_XO_CONF2 = xo_control2.as_int;

    xo_control.XO_RESETn_XO_MODE   = 0;
    xo_control.XO_XO_MODE = 1;
    *REG_XO_CONF1 = xo_control.as_int;
    ...
    *REG_XO_CONF1 = xo_control.as_int;
    ...
    xo_control.XO_EN_HP_LDO = 0;
    xo_control.XO_EN_HP_IREF = 0;
    xo_control.XO_RESETn_HP_CONF   = 0;
    *REG_XO_CONF1 = xo_control.as_int;
...
void xo_lp2hp(uint32_t delay_a) {
    ...
    //--------------------------------------------------------------------------
    // XO Driver (XO_DRV_V6_TSMC180) Transition Sequence LP to HP
    //--------------------------------------------------------------------------
    // EN_HP_LDO    __|**************************************
    // EN_HP_IREF   __|**************************************
    // EN_HP        __|**************************************
    // ISOL_CLK_HP  **********|______________________________
    // XO_MODE      ____________|****************************
...
void xo_hp2lp(void) {
    ...
    //--------------------------------------------------------------------------
    // XO Driver (XO_DRV_V6_TSMC180) Transition Sequence HP to LP
    //--------------------------------------------------------------------------
    // EN_HP_LDO    ********|_____________________________
    // EN_HP_IREF   ********|_____________________________
    // EN_HP        ********|_____________________________
    // XO_MODE      ****|_________________________________
    // ISOL_CLK_HP  ______|*******************************
    //              --->|.|.|<-- Low Power Operation -->
    //--------------------------------------------------------------------------
