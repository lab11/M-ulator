...
static void operation_init (void) {
    ...
    if (!get_flag(FLAG_ENUMERATED)) {
        ...
        *REG_SYS_CONF =       (0x0 << 9)    // 1'h1     NO_SLEEP_WITH_PEND_IRQ  #If 1, it replaces the sleep message with a selective wakeup message if there is a pending M0 IRQ. If 0, it goes to sleep even when there is a pending M0 IRQ.
        ...
        //-------------------------------------------------
        //--- XO Settings
        *REG_XO_CONF1 =             // Default  // Description
            //-----------------------------------------------------------------------------------------------------------
		( ( 0x3 << 15)     // 3'h0      XO_CAP_TUNE       (General  ) Adjusts load capacitance on XO nodes"
		| ( 0x1 << 14)     // 1'h1      XO_XO_MODE        (General  ) Controls XO mode"
		| ( 0x0 << 13)     // 1'h0      XO_RESETn_XO_MODE (General  ) Resets XO mode control"
		| ( 0x0 << 12)     // 1'h0      XO_RESETn_HP_CONF (HP XO    ) Resets configurations for HP mode"
		| ( 0x0 << 11)     // 1'h0      XO_EN_HP_IREF     (HP LDO   ) Power gating signal for LDO current reference"
		| ( 0x0 << 10)     // 1'h0      XO_EN_HP_LDO      (HP LDO   ) Power gating signal for LDO core"
		| ( 0x1 <<  6)     // 4'h1      XO_SEL_HP_VLDO    (HP LDO   ) Adjusts LDO output voltage level"
		| ( 0x1 <<  5)     // 1'h1      XO_ISOL_CLK_HP    (HP Driver) Isolates HP XO output"
		| ( 0x0 <<  1)     // 4'h0      XO_SEL_DRIV       (HP Driver) Adjusts driver strength"
		| ( 0x0 <<  0)     // 1'h0      XO_SEL_OSC_IN     (HP Driver) Selects input source for xo clk output"
		);

        *REG_XO_CONF2 =             // Default  // Description
            //-----------------------------------------------------------------------------------------------------------
		( ( 0x1		<< 19)     // 2'h1       XO_SEL_IBIAS_DLY    "(Pulse Gen) Adjusts bias current for delay gen used in the pulse gen"
		| ( 0x0		<< 18)     // 1'h0       XO_RESETn           "(Pulse Gen) Reset signal "
		| ( 0x0		<< 16)     // 2'h2       XO_INJ              "(Pulse Gen) Adjust injection period"
		| ( 0x1		<< 13)     // 3'h1       XO_SEL_DLY          "(Pulse Gen) Adjust pulse delay"
		| ( 0x1		<< 12)     // 1'h1       XO_ISOL_CLK_LP      "(Pulse Gen) Isolates LP clk output"
		| ( 0x1		<< 10)     // 2'h1       XO_SEL_CLK_SCN      "(Pulse Gen) Selects division ratio for SCN CLK"
		| ( XO_FREQ_SEL <<  7)     // 3'h1       XO_SEL_CLK_OUT_DIV  "(Pulse Gen) Selects division ratio for the XO CLK output"
		| ( 0x0		<<  4)     // 3'h0       XO_I_AMP            "(VREF     ) Adjust VREF body bias buffer current"
		| ( 0x2		<<  2)     // 2'h2       XO_BIAS_SEL         "(LP Driver) Adjusts bias level for capacitively coupled LP driver inputs"
		| ( 0x2		<<  0)     // 2'h2       XO_SEL_IBIAS        "(LP Driver) Adjusts bias current for the LP driver"
		);

        *REG_XO_CONF3 =             // Default  // Description
            //-----------------------------------------------------------------------------------------------------------
		( ( 0x0 << 17)     // 1'h0       XO_EN_HP       (HP Driver) Enables HP Drivier"
		| ( 0x1 << 16)     // 1'h1       XO_EN_FS       (HP Driver) Feedback resistance no longer a pseudo R"
		| ( 0x0 << 15)     // 1'h0       XO_EN_FAST     (HP Driver) Feedback resistance for fast corner"
		| ( 0x1 << 14)     // 1'h1       XO_EN_NOM      (HP Driver) Feedback resistance for nominal corner"
		| ( 0x0 << 13)     // 1'h0       XO_EN_SLOW     (HP Driver) Feedback resistance for slow cornor"
		| ( 0x1 << 12)     // 1'h1       XO_SEL_LDO     (SCN      ) Selects LDO output as an input to SCN"
		| ( 0x0 << 11)     // 1'h0       XO_SEL_0P6     (SCN      ) Selects V0P6 as an input to SCN"
		| ( 0x0 <<  4)     // 7'h0       XO_REF_TUNEB   (VREF     ) Adjust LP VREF temperature coefficient"
		| ( 0x0 <<  2)     // 2'h2       XO_SEL_LP_VREF (VREF     ) Adjust LP VREF output level"
		| ( 0x1 <<  0)     // 2'h1       XO_SEL_HP_VREF (VREF     ) Adjust HP VREF output level"
		);

	set_xo_pad(1); // SEL_PAD_XO_CLK: 2'h0: Tie-Lo, 2'h1: CLK_XO_LP, 2'h2: CLK_XO_HP, 2'h3: CLK_XO_TMR"
