...
//    uint32_t upper = var >> 32;
//    uint32_t lower = var & 0xFFFFFFFF;
//    uint64_t result;
//    lower = (upper << (32-nb)) | (lower >> nb);
//    result = (upper >> nb);
//    result = (result << 32) | lower;
//    return result;
//}
...
//*******************************************************************
// PRE FUNCTIONS (Specific to TMCvA)
//*******************************************************************
...
uint32_t pmu_calc_new_sar_ratio(uint32_t adc_val, uint32_t offset) {
    ...
    if (pmu_validate_adc_val(adc_val)) {
        ...
        if (
            ...
            || (adc_val > (__pmu_last_effective_adc_val__ + __PMU_ADC_SAR_HYSTERESIS__))    // ADC values went up (+ __PMU_ADC_SAR_HYSTERESIS__)
        ) {
            ...
            #ifdef __PMU_SAR_MARGIN_5_PERCENT__
                c0 = 0x536778; c1 = 0x0D5DB; c2 = 0x1E5; // 5%  
            #endif
            #ifdef __PMU_SAR_MARGIN_7_PERCENT__
                c0 = 0x5516E1; c1 = 0x0DC1D; c2 = 0x1F5; // 7%  
            #endif
            #ifdef __PMU_SAR_MARGIN_8_PERCENT__
                c0 = 0x559E24; c1 = 0x0DC93; c2 = 0x1F5; // 8%
            #endif
            #ifdef __PMU_SAR_MARGIN_9_PERCENT__
                c0 = 0x56A605; c1 = 0x0E0B7; c2 = 0x1FF; // 9%
            #endif
            #ifdef __PMU_SAR_MARGIN_10_PERCENT__
                c0 = 0x57A761; c1 = 0x0E760; c2 = 0x212; // 10% 
            #endif
            #ifdef __PMU_SAR_MARGIN_12_PERCENT__
                c0 = 0x594870; c1 = 0x0EA5A; c2 = 0x217; // 12% (interpolated)
            #endif
            #ifdef __PMU_SAR_MARGIN_15_PERCENT__
                c0 = 0x5BBA07; c1 = 0x0EED2; c2 = 0x220; // 15%
            #endif
...
uint32_t pmu_check_low_vbat (uint32_t adc_val, uint32_t offset, uint32_t threshold) {
    if (pmu_validate_adc_val(adc_val)) {
        if (adc_val < (threshold + offset)) {
            if (__pmu_low_vbat_streak__ < __PMU_NUM_CONS_MEAS__) __pmu_low_vbat_streak__++;
        }
        else {
            __pmu_low_vbat_streak__ = 0;
        }
    }
    return (__pmu_low_vbat_streak__>=__PMU_NUM_CONS_MEAS__);
}
...
uint32_t pmu_check_crit_vbat (uint32_t adc_val, uint32_t offset, uint32_t threshold) {
    if (pmu_validate_adc_val(adc_val)) {
        if (adc_val < (threshold + offset)) {
            if (__pmu_crit_vbat_streak__ < __PMU_NUM_CONS_MEAS__) __pmu_crit_vbat_streak__++;
        }
        else {
            __pmu_crit_vbat_streak__ = 0;
        }
    }
    return (__pmu_crit_vbat_streak__>=__PMU_NUM_CONS_MEAS__);
}
...
void nfc_init(void){
    ...
    //-------------------------------------------------------------------
    //[NOTE] __i2c_password__ has been commented out, 
    //      since the firmware does not change the password.
    //-------------------------------------------------------------------
...
uint32_t nfc_i2c_present_password(void) {
    ...
    //-------------------------------------------------------------------
    //[NOTE] __i2c_password__ is fixed at 0
    //       For the full implementation of this function,
    //       see xt1_firmware_v3p05
    //-------------------------------------------------------------------
    for(idx=0; idx<8; idx++) nfc_i2c_byte(0x00);
    nfc_i2c_byte(0x09);
    for(idx=0; idx<8; idx++) nfc_i2c_byte(0x00);
    nfc_i2c_stop();
...
void eid_init(void) {
    ...
    #ifndef __USE_HCODE__
        __crsh_seqa__.value = (/*vin*/ 0x0 << 11) | (/*te*/ 0x0 << 10) | (/*fd*/ 0x1 << 9) | (/*seg*/ 0x07F << 0);
    ...
    mbus_remote_register_write(EID_ADDR,0x13,
        ...
        mbus_remote_register_write(EID_ADDR,0x15,
                                      (  1              << 23)  // WCTR_RESETB_CP_A
                                    | (  0              << 22)  // WCTR_VIN_CP_A
                                    | ((~0x27F & 0x7FF) << 11)  // WCTR_EN_CP_PD_A
                                    | ( 0x27F           <<  0)  // WCTR_EN_CP_CK_A
                                    );
...
void sat_init(void) {
...
#ifndef __USE_HCODE__
    sat_r00.as_int = SAT_R00_DEFAULT_AS_INT;
    sat_r01.as_int = SAT_R01_DEFAULT_AS_INT;
    sat_r08.as_int = SAT_R08_DEFAULT_AS_INT;
    sat_r09.as_int = SAT_R09_DEFAULT_AS_INT;
#endif
    sat_r0C.as_int = SAT_R0C_DEFAULT_AS_INT;
    sat_r0F.as_int = SAT_R0F_DEFAULT_AS_INT;
    sat_r13.as_int = SAT_R13_DEFAULT_AS_INT;
    sat_r17.as_int = SAT_R17_DEFAULT_AS_INT;

#ifdef __USE_HCODE__
    // Temp Sensor Initialization
    //sat_r01.TSNS_RESETn     = 0;
    //sat_r01.TSNS_EN_IRQ     = 1;
    //sat_r01.TSNS_BURST_MODE = 0;
    //sat_r01.TSNS_CONT_MODE  = 0;
    mbus_remote_register_write(SAT_ADDR,0x01,0x000902);
#else
    // Temp Sensor Initialization
    sat_r01.TSNS_RESETn     = 0;
    sat_r01.TSNS_EN_IRQ     = 1;
    sat_r01.TSNS_BURST_MODE = 0;
    sat_r01.TSNS_CONT_MODE  = 0;
    mbus_remote_register_write(SAT_ADDR,0x01,sat_r01.as_int);
#endif

    //---------------------------------------------------------------------------------------
    // Set temp sensor conversion time
    //---------------------------------------------------------------------------------------
    mbus_remote_register_write(SAT_ADDR,0x03,
    //---------------------------------------------------------------------------------------
                        // Default  // Description
        ( (0x4 << 16)   // 0x4      // TSNS_MIM              
        | (0x4 << 13)   // 0x4      // TSNS_MOM              
    ...
    sat_r17.WUP_INT_RPLY_REG_ADDR = PLD_TARGET_REG_IDX;
    sat_r17.WUP_INT_RPLY_SHORT_ADDR = 0x10;
    sat_r17.WUP_CLK_SEL = 0x0; // 0: Use CLK_TMR, 1: Use CLK_TSNS
    sat_r17.WUP_AUTO_RESET = 0x0; // Automatically reset counter to 0 upon sleep 
    mbus_remote_register_write(SAT_ADDR,0x17,sat_r17.as_int);

    // Turn on LDO VREF (previously sat_ldo_vref_on())
#ifdef __USE_HCODE__
    //sat_r00.LDO_EN_VREF = 1;
    mbus_remote_register_write(SAT_ADDR,0x00,0x000024);
#else
    sat_r00.LDO_EN_VREF = 1;
    mbus_remote_register_write(SAT_ADDR,0x00,sat_r00.as_int);
#endif
...
void sat_acc_ldo_power_on(void) {
    sat_r0C.ACC_ENb_LDO = 0x0; 
    mbus_remote_register_write(SAT_ADDR,0x0C,sat_r0C.as_int);
}
void sat_acc_ldo_power_off(void) {
    sat_r0C.ACC_ENb_LDO = 0x1; 
    mbus_remote_register_write(SAT_ADDR,0x0C,sat_r0C.as_int);
}

void sat_acc_cp_power_on(void) {
    sat_r0C.ACC_SEL_CP_CLK = 0x0; // 0:div0, 1:div2, 2:div4, 3:div8
    sat_r0C.ACC_ISOL_CP_CLK = 0x0; 
    mbus_remote_register_write(SAT_ADDR,0x0C,sat_r0C.as_int);
}
void sat_acc_cp_power_off(void) {
    sat_r0C.ACC_ISOL_CP_CLK = 0x1; 
    mbus_remote_register_write(SAT_ADDR,0x0C,sat_r0C.as_int);
}

void sat_acc_safr_clk_on(void) {
    sat_r0F.ACC_RESETB_SAFR = 0x1; 
    sat_r0F.ACC_ISOL_SAFR_CLK = 0x0; 
    sat_r0F.ACC_DSM_ENB = 0x0; 
    mbus_remote_register_write(SAT_ADDR,0x0F,sat_r0F.as_int);
}
void sat_acc_safr_clk_off(void) {
    sat_r0F.ACC_RESETB_SAFR = 0x0; 
    sat_r0F.ACC_ISOL_SAFR_CLK = 0x1; 
    sat_r0F.ACC_DSM_ENB = 0x1; 
    mbus_remote_register_write(SAT_ADDR,0x0F,sat_r0F.as_int);
}

void sat_acc_md_enable(void) {
    sat_r0F.ACC_ISOL_MD = 0x0; 
    sat_r0F.ACC_RESETB_MD = 0x1; 
    mbus_remote_register_write(SAT_ADDR,0x0F,sat_r0F.as_int);
}
void sat_acc_md_disable(void) {
    sat_r0F.ACC_ISOL_MD = 0x1; 
    sat_r0F.ACC_RESETB_MD = 0x0; 
    mbus_remote_register_write(SAT_ADDR,0x0F,sat_r0F.as_int);
}
void sat_acc_get_md_cnt(void) {
    // ACC_READ_MD_PRE
    mbus_remote_register_write(SAT_ADDR,0x10,0x1);

    // Delay (~1ms) @ 100kHz clock speed)
    delay(100); 

    // ACC_READ_MD
    mbus_remote_register_write(SAT_ADDR,0x11,0x1);
}
void sat_acc_scan_phi_h(void) {
    sat_r13.ACC_SCAN_PHI = 0x1; 
    sat_r13.ACC_SCAN_PHI_BAR = 0x1; 
    sat_r13.ACC_SCAN_DATA_IN = 0x1; 
    sat_r13.ACC_SCAN_LOAD_CHIP = 0x1; 
    mbus_remote_register_write(SAT_ADDR,0x10,sat_r13.as_int);
}
...
#ifdef __USE_HCODE__
    // Turn on LDO
    //sat_r00.LDO_EN_IREF = 1;
    //sat_r00.LDO_EN_LDO  = 1;
    mbus_remote_register_write(SAT_ADDR,0x00,0x000027);

    // Delay (~10ms) @ 100kHz clock speed)
    delay(1000); 

    // Turn on digital block
    //sat_r01.TSNS_SEL_LDO  = 1;
    mbus_remote_register_write(SAT_ADDR,0x01,0x00090A);

    // Turn on analog block
    //sat_r01.TSNS_EN_SENSOR_LDO  = 1;
    mbus_remote_register_write(SAT_ADDR,0x01,0x00092A);
...
#ifdef __USE_HCODE__
    //sat_r01.TSNS_RESETn = 0;
    mbus_remote_register_write(SAT_ADDR,0x01,0x000928);

    //sat_r01.TSNS_RESETn         = 0;
    //sat_r01.TSNS_ISOLATE        = 1;
    mbus_remote_register_write(SAT_ADDR,0x01,0x00092A);

    //sat_r01.TSNS_SEL_V1P2       = 0;
    //sat_r01.TSNS_EN_SENSOR_V1P2 = 0;
    //sat_r01.TSNS_SEL_LDO        = 0;
    //sat_r01.TSNS_EN_SENSOR_LDO  = 0;
    mbus_remote_register_write(SAT_ADDR,0x01,0x000902);

    // Turn off LDO
    //sat_r00.LDO_EN_IREF   = 0;
    //sat_r00.LDO_EN_LDO    = 0;
    mbus_remote_register_write(SAT_ADDR,0x00,0x000024);
#else
    sat_r01.TSNS_RESETn = 0;
    mbus_remote_register_write(SAT_ADDR,0x01,sat_r01.as_int);
...
void sat_temp_sensor_start(void){
    ...
    //sat_r01.TSNS_RESETn  = 1;
    //sat_r01.TSNS_ISOLATE = 0;
    mbus_remote_register_write(SAT_ADDR,0x01,0x000929);
#else
    sat_r01.TSNS_RESETn = 0;
    mbus_remote_register_write(SAT_ADDR,0x01,sat_r01.as_int);
...
void sat_start_timer(uint32_t wait_time){
    ...
    //sat_r08.TMR_SLEEP = 0x0;
    mbus_remote_register_write(SAT_ADDR,0x08,0x000020);

    // [5] TMR_ISOLATE: 1'h1 -> 1'h0
    //sat_r08.TMR_ISOLATE = 0x0;
    mbus_remote_register_write(SAT_ADDR,0x08,0x000000);

    // [21] TMR_SELF_EN: 1'h1 -> 1'h0
    //sat_r09.TMR_SELF_EN = 0x0;
    mbus_remote_register_write(SAT_ADDR,0x09,0x4880FF);

    // [3] TMR_EN_OSC: 1'h0 -> 1'h1
    //sat_r08.TMR_EN_OSC = 0x1;
    mbus_remote_register_write(SAT_ADDR,0x08,0x000008);

    // Release Reset 
    // [4] TMR_RESETB: 1'h0 -> 1'h1
    // [2] TMR_RESETB_DIV: 1'h0 -> 1'h1
    // [1] TMR_RESETB_DCDC: 1'h0 -> 1'h1
    //sat_r08.TMR_RESETB = 0x1;
    //sat_r08.TMR_RESETB_DIV = 0x1;
    //sat_r08.TMR_RESETB_DCDC = 0x1;
    mbus_remote_register_write(SAT_ADDR,0x08,0x00001E);

    // Delay (must be >400ms at RT)
    delay(wait_time);

    // [0] TMR_EN_SELF_CLK: 1'h0 -> 1'h1
    //sat_r08.TMR_EN_SELF_CLK = 0x1;
    mbus_remote_register_write(SAT_ADDR,0x08,0x00001F);

    // [21] TMR_SELF_EN: 1'h0 -> 1'h1
    //sat_r09.TMR_SELF_EN = 0x1;
    mbus_remote_register_write(SAT_ADDR,0x09,0x6880FF);

    // [3] TMR_EN_OSC: 1'h1 -> 1'h0
    //sat_r08.TMR_EN_OSC = 0x0; // Default : 0x0
    mbus_remote_register_write(SAT_ADDR,0x08,0x000017);
#else
    // [6] TMR_SLEEP: 1'h1 -> 1'h0
    sat_r08.TMR_SLEEP = 0x0;
    mbus_remote_register_write(SAT_ADDR,0x08,sat_r08.as_int);
    ...
    // [21] TMR_SELF_EN: 1'h1 -> 1'h0
    sat_r09.TMR_SELF_EN = 0x0;
    mbus_remote_register_write(SAT_ADDR,0x09,sat_r09.as_int);
...
#ifdef __USE_HCODE__
    //sat_r08.TMR_EN_OSC = 0x0; // Default : 0x0
    //sat_r08.TMR_EN_SELF_CLK = 0x0; // Default : 0x0
    //sat_r08.TMR_RESETB = 0x0;// Default : 0x0
    //sat_r08.TMR_RESETB_DIV = 0x0; // Default : 0x0
    //sat_r08.TMR_RESETB_DCDC = 0x0; // Default : 0x0
    mbus_remote_register_write(SAT_ADDR,0x08,0x000000);

    sat_r17.WUP_ENABLE = 0x0; // Default : 0x
    mbus_remote_register_write(SAT_ADDR,0x17,sat_r17.as_int);

	// New for SNTv3
	//sat_r08.TMR_SLEEP = 0x1; // Default : 0x1
	//sat_r08.TMR_ISOLATE = 0x1; // Default : 0x1
    mbus_remote_register_write(SAT_ADDR,0x08,0x000060);
#else
    // EN_OSC
    sat_r08.TMR_EN_OSC = 0x0; // Default : 0x0
...
uint32_t sat_set_timer_threshold(uint32_t threshold){
    mbus_remote_register_write(SAT_ADDR,0x19,(0x10<<16)|(WP1_TARGET_REG_IDX<<8)|((threshold>>24)&0xFF));
    enable_reg_irq(WP1_TARGET_REG_IDX);
    start_timeout32_check(/*id*/FAIL_ID_WUP, /*val*/TIMEOUT_TH);
    mbus_remote_register_write(SAT_ADDR,0x1A,threshold & 0xFFFFFF);
    WFI();
...
void sat_enable_wup_timer (uint32_t auto_reset) {
    sat_r17.WUP_AUTO_RESET = auto_reset;
    sat_r17.WUP_ENABLE = 0x1;
    mbus_remote_register_write(SAT_ADDR,0x17,sat_r17.as_int);
}

void sat_disable_wup_timer (void) {
    sat_r17.WUP_ENABLE = 0x0;
    mbus_remote_register_write(SAT_ADDR,0x17,sat_r17.as_int);
}

uint32_t sat_set_wup_timer(uint32_t auto_reset, uint32_t threshold){
    sat_enable_wup_timer(auto_reset);
    return sat_set_timer_threshold(threshold);
}

uint32_t sat_sync_read_wup_timer(void) {
    enable_reg_irq(WP1_TARGET_REG_IDX);
    start_timeout32_check(/*id*/FAIL_ID_WUP, /*val*/TIMEOUT_TH);
    mbus_remote_register_write(SAT_ADDR,0x14,
...
uint32_t sat_read_wup_timer(void) {
    ...
    uint32_t temp = sat_sync_read_wup_timer();
    uint32_t temp_prev;
