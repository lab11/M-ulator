...
//
//  -----------------------------------------------------------------------------------------
//  < XO and SNT Timer - calibrate_sat_timer() >
//  -----------------------------------------------------------------------------------------
...
//                                                  0x3: 'skip_calib' or 'MAIN_CALLED' or 'FLAG_INVLD_XO_PREV' is 1. Will use the previous sat_freq as it is.
...
//                                                  0x8: Skipping the sat_threshold calculation (usually due to a new START/STOP/RESET command)
//                                                  0x9: Calculated sat_threshold was 0, so it is forced to 1.
...
//                                                          It uses the previous sat_freq to calculate the next sat_threshold.
...
//                                                          It uses the previous sat_freq to calculate the next sat_threshold.
//  0x84    sat_duration                        SNT counter value that corresponds to the given interval (previous sleep duration)
//  0x85    XO_FREQ                             XO frequency
//  0x86    sat_freq                            SNT frequency that is used for the previous sleep.
//  0x87    delta_xo_val                        xo_val_curr - xo_val_prev
//  0x88    a                                   Calculated a (Custom fixed-point, N=30)
//  0x89    temp_sat_freq                       Newly Calculated SNT frequency
//  0x8A    wakeup_interval_sec                 New wakeup interval (in seconds)
//  0x8B    sat_duration                        Newly calculated sat_duration
//  0x8C    sat_threshold                       Newly calculated sat_threshold
//  0x8D    sat_threshold_prev                  Previous sat_threshold
//  0x8E    sat_threshold                       sat_threshold
//  0x8F    sat_threshold                       Going into sleep with sat_threshold
//  
//  -----------------------------------------------------------------------------------------
...
//  0x91    value                               meas_temp_adc(), sat_operation() information
...
//
//  0xCA    sat_threshold                       sat_threshold has been updated with sat_read_wup_timer() + sat_duration (xo fail w/ skip_calib)
//  0xCB    sat_threshold                       sat_threshold has been updated with sat_read_wup_timer() + sat_duration (xo pass w/ skip_calib)
//  0xCC    sat_threshold_prev                  sat_threshold_prev in calc_ssls()
//  0xCD    sat_curr_val                        sat_curr_val in calc_ssls()
//  0xCE    sat_delta                           sat_delta in calc_ssls()
...
volatile uint32_t sat_freq;                         // SNT Timer Frequency (Hz)
volatile uint32_t sat_duration;                     // SNT counter value that corresponds to the given interval
volatile uint32_t sat_threshold;                    // SNT Timer Threshold to wake up the system
volatile uint32_t sat_threshold_prev;               // Previous SNT Timer Threshold
...
volatile uint32_t sat_skip_calib;                   // If 1, it skips the SNT calibration and uses the previous sat_freq to calculate the next sat_threshold.
...
volatile uint32_t pretend_wakeup_by_sat;    // 1 indicates that the current wakeup must be handled as if it was triggered by the SNT timer, 
...
static void operation_sleep (uint32_t check_sat);
static void operation_sleep_sat_timer(uint32_t check_sat);
...
static void sat_operation (void);
...
// Args    : check_sat  - Check the current SNT counter value
//                        and decide whether to bypass the sleep
// Description:
//           Sends the MBus Sleep message
//           If check_sat=1, it first checks the SNT counter value.
// Return  : None
//-------------------------------------------------------------------
static void operation_sleep (uint32_t check_sat) {
    ...
    if (check_sat) {
        ...
        uint32_t dist_min = get_high_power_history()  
                            ? (sat_freq<<3)+(sat_freq<<1) // 10s
                            : (sat_freq<<2);              // 4s
        uint32_t dist_max = (sat_freq<<18);               // 262144s (= 4369min = 73hrs); Also see calc_ssls()
        uint32_t sat_val = sat_read_wup_timer();
        uint32_t dist = sat_threshold - sat_val;

        //uint32_t dist;
        //if (sat_threshold >= sat_val) { dist = sat_threshold - sat_val;                    } 
        //else                          { dist = sat_threshold + (0xFFFFFFFF - sat_val) + 1; }
...
//-------------------------------------------------------------------
// Function: operation_sleep_sat_timer
// Args    : check_sat  - Check the current SNT counter value
//                        and decide whether to bypass the sleep
// Description:
//           Goes into sleep with SNT Timer enabled
//           Auto Reset is disabled.
//           SNT threshold is set to sat_threshold.
//           If check_sat=1, it first checks the SNT counter value.
// Return  : None
//-------------------------------------------------------------------
static void operation_sleep_sat_timer(uint32_t check_sat) {
    if (!sat_set_wup_timer(/*auto_reset*/0, /*threshold*/sat_threshold)) {
        __wfi_id__ = FAIL_ID_GEN;
        fail_handler(/*id*/__wfi_id__);
    }
    #ifdef DEVEL
        mbus_write_message32(0x8F, sat_threshold);
    #endif
    operation_sleep(/*check_sat*/check_sat);
}
...
static void operation_dead(void) {
    ...
    sat_temp_sensor_power_off();

    // Disable SNT timer, XO timer
    sat_stop_timer();
    sat_disable_wup_timer();
    xo_stop();
...
static void operation_init (void) {
    ...
    if (!get_flag(FLAG_ENUMERATED)) {
        ...
        *REG_SYS_CONF =       (0x0 << 9)    // 1'h1     NO_SLEEP_WITH_PEND_IRQ  #If 1, it replaces the sleep message with a selective wakeup message if there is a pending M0 IRQ. If 0, it goes to sleep even when there is a pending M0 IRQ.
        ...
        mbus_remote_register_write(SAT_ADDR, 0x07, (0x10 << 8) | SNT_TARGET_REG_IDX);
        ...
        sat_start_timer(/*wait_time*/DLY_1S);

        // Start the SNT counter
        sat_enable_wup_timer(/*auto_reset*/0);
        ...
        *REG_WUPT_CONFIG = *REG_WUPT_CONFIG & 0xFF3FFFFF; // WUP_ENABLE=0, WUP_WREQ_EN=0
        ...
        sat_freq = 1400;
        sat_duration = 30000;
        sat_threshold_prev = sat_read_wup_timer();
        sat_threshold = sat_threshold_prev + sat_duration;
        xo_val_curr = get_xo_cnt();
        operation_sleep_sat_timer(/*check_sat*/0);
    }
}
    ...
    if (!meas.valid) {
        ...
        if (get_flag(FLAG_INITIALIZED)) {
            ...
            if (meas.crit_vbat) {
                ...
                sat_set_wup_timer(/*auto_reset*/1, /*threshold*/0);
                operation_sleep(/*check_sat*/0);
            }
            /////////////////////////////////////////////////////////////////
...
int main(void) {
    ...
    *NVIC_ISER = (0x1 << IRQ_TIMER32);
    ...
    if (!WAKEUP_BY_NFC && !WAKEUP_BY_SNT) {
        operation_sleep(/*check_sat*/1);
    }

    sat_acc_ldo_power_on();
    sat_acc_safr_clk_on();
    sat_acc_scan_phi_h();
    //sat_acc_cp_power_on();
